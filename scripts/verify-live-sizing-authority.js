...
const paper_engine_1 = require("../src/engine/paper-engine");
const engine_v2_1 = require("../src/engine-v2");
...
async function runVerification() {
    ...
    console.log("TEST_CONFIG_MAX_ADDON:", (0, env_1.getEngineConfig)(explicitEnv).okxLiveMaxAddonNotionalUsdt);
    const config = {
        ...(0, env_1.getEngineConfig)(explicitEnv),
        baseSizeUsd: 100,
        ...
        okxBalanceOk: true,
        reports: {
            ...((0, env_1.getEngineConfig)(explicitEnv).reports || {}),
    ...
    let mockOpenPositions = [];
    let mockClosedPositions = [];
    let mockPendingOrders = [];
    const mockStore = {
        readPositionsOpenAll: async () => mockOpenPositions,
        writePositionsOpenAll: async (positions) => { mockOpenPositions = positions; return ""; },
        writeOpenPositions: async (positions) => { mockOpenPositions = positions; },
        writeClosedPosition: async (position) => { mockClosedPositions.push(position); },
        readPositionsHistory: async () => mockClosedPositions,
        readPendingEntryOrders: async () => mockPendingOrders,
        writePendingEntryOrders: async (orders) => { mockPendingOrders = orders; },
    ...
    const paperEngine = new paper_engine_1.PaperEngine(config, dummyLogger, mockOkxDemoClient, mockStore);
    paperEngine.instrumentCache = new Map([
        ["ETH-USDT-SWAP", { lotSz: 0.1, minSz: 0.1, ctVal: 0.1, ctValCcy: "ETH" }]
    ]);
    paperEngine.okxPublic = mockOkxDemoClient;
    paperEngine.okxDemo = mockOkxDemoClient;
    paperEngine.serverTradeControlState = { server_trade_enabled: true, close_only_mode: false, kill_switch_active: false };
    paperEngine.loadServerTradeControlState = async () => { };
    ...
    paperEngine.submitOkxOrder = async (input) => {
    ...
    paperEngine.cancelOrder = async (symbol, ordId, algoId) => {
    ...
    paperEngine.tryPaperPositionClose = async (input) => {
    ...
    paperEngine.writeOpenPositions = async (positions) => {
    ...
    paperEngine.writeClosedPositions = async (positions) => {
    ...
    paperEngine.removeClosedPositionFromLedger = async (symbol) => {
    ...
    paperEngine.ensureProtectiveStopOrder = async (open, flowId, pricingLastInput, protectionSource) => {
    ...
    async function runFullPipelineTest(args) {
        paperEngine.paperExecutionReady = true;
        paperEngine.signedExecutionReady = true;
        paperEngine.okxSmokeTestPerformed = true;
        paperEngine.okxSignedRestReady = true;
        paperEngine.liveBalanceReady = args.liveBalanceReady ?? true;
        paperEngine.okxWalletBalanceUsdt = args.okxWalletBalanceUsdt ?? 69;
        paperEngine.okxAvailableBalanceUsdt = args.okxAvailableBalanceUsdt ?? 69;
        paperEngine.okxPositionsOk = args.okxPositionsOk ?? true;
        paperEngine.okxOrderSubmitOk = true;
        paperEngine.okxPendingOrdersReady = args.okxPendingOrdersReady ?? true;
        paperEngine.lastLivePositionsPayload = args.lastLivePositionsPayload ?? [];
        mockOpenPositions = args.opensAfterClose ?? [];
        const lastRisk = args.lastRisk ?? null;
        const bridgeState = (0, paper_engine_1.buildV2StateBridge)(mockOpenPositions, lastRisk, config, true, true, false, false, 1, 1, defaultQualityProfiles, { server_trade_enabled: true, close_only_mode: false, kill_switch_active: false, authority_source: "server_state", updated_at: now, reason: null }, false, args.lastLivePositionsPayload ?? [], args.liveBalanceReady ?? true, args.okxWalletBalanceUsdt ?? 69, args.okxAvailableBalanceUsdt ?? 69, args.okxPositionsOk ?? true, args.okxPendingOrdersReady ?? true, "pendingOrdersNotionalUsdt" in args ? args.pendingOrdersNotionalUsdt : 0, "pendingSymbolNotionalUsdt" in args ? args.pendingSymbolNotionalUsdt : 0, "balanceFetchedAt" in args ? args.balanceFetchedAt : now, "positionsFetchedAt" in args ? args.positionsFetchedAt : now, "pendingOrdersFetchedAt" in args ? args.pendingOrdersFetchedAt : now);
        // REMOVED ALL FORCED addOnAllowed OVERRIDES (Requirement 8)
        const v2Input = (0, engine_v2_1.adaptV2Input)(args.symbol, now, snapshotAdapter, config, bridgeState, args.customV1Result ?? v1Result);
        if (Array.isArray(args.opensAfterClose) && args.opensAfterClose.length > 0) {
            v2Input.state.currentPositions = args.opensAfterClose;
        }
        const v2Outcome = (0, engine_v2_1.runEngineV2)(v2Input);
    ...
    const conservativeConfig = {
        ...(0, env_1.getEngineConfig)(conservativeEnv),
        baseSizeUsd: 100,
        okxAuthMode: "live",
        okxExchangeAuthOptIn: true,
        okxLiveEnabled: true,
    ...
    const bridgeState2a = (0, paper_engine_1.buildV2StateBridge)([initialOpenRecord], { directionalShockState: "NONE", longAllow: true, shortAllow: true }, conservativeConfig, true, true, false, false, 1, 1, defaultQualityProfiles, { server_trade_enabled: true, close_only_mode: false, kill_switch_active: false, authority_source: "server_state", updated_at: now, reason: null }, false, [{ instId: "ETH-USDT-SWAP", symbol: "ETHUSDT", side: "long", posSide: "long", entryPrice: 2400, avgPx: "2400", sizeUsd: 45 }], true, 69, 69, true, true, 0, 0, now, now, now);
    const v2Input2a = (0, engine_v2_1.adaptV2Input)("ETHUSDT", now, snapshotAdapter, conservativeConfig, bridgeState2a, {
        ...v1Result,
        decision: "ENTER",
        isAddOn: true,
        addOnPolicyAllowed: true,
        lifecycleAuthority: { ...v1Result.lifecycleAuthority, addOnAllowed: true, addOnReason: "TEST_ADDON_POLICY_APPROVED" }
    });
    v2Input2a.state.currentPositions = [initialOpenRecord];
    const v2Outcome2a = (0, engine_v2_1.runEngineV2)(v2Input2a);
    const bridgeResult2a = await paperEngine.executeAuthorizedV2Action({
        symbol: "ETHUSDT",
        v2Decision: v2Outcome2a.decision,
        lastPrice: 3000,
        committedRiskPlan: v2Outcome2a.decision.committedRiskPlan
    });
    ...
    spies.reset();
    const holdInput = (0, engine_v2_1.adaptV2Input)("ETHUSDT", now, snapshotAdapter, config, (0, paper_engine_1.buildV2StateBridge)([], null, config, true, true, false, false, 1, 1, defaultQualityProfiles, { server_trade_enabled: false, close_only_mode: false, kill_switch_active: false, authority_source: "server_state", updated_at: now, reason: null }, false, [], true, 69, 69, true, true, 0, 0, now, now, now), { ...v1Result, decision: { final_decision: "HOLD", regime_state: "RANGE" } });
    const holdOutcome = (0, engine_v2_1.runEngineV2)(holdInput);
    ...
    const holdBridgeRes = await paperEngine.executeAuthorizedV2Action({
        ...
        lastPrice: 3000,
        committedRiskPlan: holdOutcome.decision.risk
    });
    ...
    const paperEngineInstance = new paper_engine_1.PaperEngine(paperConfig, dummyLogger, mockOkxDemoClient, mockStore);
    paperEngineInstance.instrumentCache = new Map([
        ["ETH-USDT-SWAP", { lotSz: 0.1, minSz: 0.1, ctVal: 0.1, ctValCcy: "ETH" }]
    ]);
    paperEngineInstance.paperExecutionReady = true;
    paperEngineInstance.serverTradeControlState = { server_trade_enabled: true, close_only_mode: false, kill_switch_active: false };
    paperEngineInstance.loadServerTradeControlState = async () => { };
    const paperBridgeState = (0, paper_engine_1.buildV2StateBridge)([], null, paperConfig, true, false, false, false, 1, 1, defaultQualityProfiles, { server_trade_enabled: true, close_only_mode: false, kill_switch_active: false, authority_source: "server_state", updated_at: now, reason: null }, false, [], false, null, null, false, false, 0, 0, now, now, now);
    const paperV2Input = (0, engine_v2_1.adaptV2Input)("ETHUSDT", now, snapshotAdapter, paperConfig, paperBridgeState, v1Result);
    const paperOutcome = (0, engine_v2_1.runEngineV2)(paperV2Input);
    ...
    mockOkxDemoClient.tryGetCandles = async (symbol, interval, limit) => ({
        ok: true,
        value: mockCandlesArray,
        diagnostics: { httpStatus: 200 }
    });
    mockOkxDemoClient.tryGetTicker = async (symbol) => ({
        ok: true,
        value: { last: 3000, bid: 2999, ask: 3001 },
        diagnostics: { httpStatus: 200 }
    });
    mockOkxDemoClient.tryGetFundingRate = async (symbol) => ({
        ok: true,
        value: { rate: 0.0001 },
        diagnostics: { httpStatus: 200 }
    });
    ...
    paperEngine.okxSmokeTestPerformed = true;
    paperEngine.okxSignedRestReady = true;
    paperEngine.liveBalanceReady = true;
    paperEngine.okxWalletBalanceUsdt = 69;
    paperEngine.okxAvailableBalanceUsdt = 69;
    paperEngine.okxPositionsOk = true;
    paperEngine.okxPendingOrdersReady = true;
    const testNow = Date.now();
    paperEngine.balanceFetchedAt = testNow;
    paperEngine.positionsFetchedAt = testNow;
    paperEngine.pendingOrdersFetchedAt = testNow;
    paperEngine.pendingOrdersNotionalUsdt = 0;
    paperEngine.pendingSymbolNotionalUsdt = 0;
    paperEngine.okxOrderSubmitOk = true;
    paperEngine.okxAccountConfigOk = true;
    paperEngine.okxBalanceOk = true;
    paperEngine.bundleWriterReady = true;
    paperEngine.publicMarketDataReady = true;
    paperEngine.positionTrackingAlive = true;
    paperEngine.engineLastTickAt = Date.now();
    paperEngine.freshTickRequiredAfterReadiness = false;
    paperEngine.evaluateReadinessTransition = () => { };
    paperEngine.applyStartupRecoveryBarrier = () => { };
    const originalPoll = paperEngine.pollSymbol.bind(paperEngine);
    paperEngine.pollSymbol = async (...args) => {
