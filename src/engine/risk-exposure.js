...
exports.evaluateRiskExposure = evaluateRiskExposure;
...
function shortBlockedReason(input) {
    if (!input.riskShortAllow)
        return "RISK_SHORT_DISALLOWED";
    if (input.routingPaused)
        return "ROUTING_PAUSED";
    if (!(input.openPositionCount < input.maxSlots))
        return "MAX_SLOTS_REACHED";
    if (input.riskMode === "HALT")
        return "RISK_MODE_HALT";
    return null;
}
function longBlockedReason(input) {
    if (!input.riskLongAllow)
        return "RISK_LONG_DISALLOWED";
    if (input.routingPaused)
        return "ROUTING_PAUSED";
    if (!(input.openPositionCount < input.maxSlots))
        return "MAX_SLOTS_REACHED";
...
function sessionRiskBias(sessionProfile, volHigh, utcHour, utcMin) {
 ...
 */
function evaluateRiskExposure(input) {
    ...
    let riskMode = "NORMAL";
    if (risk.engineBlocked === true || risk.dailyLossGuardTriggered)
        riskMode = "HALT";
    else if (throttle > 0.72 || volHigh || risk.crashState === "CRASH_EXIT")
        riskMode = "DEFENSIVE";
    else if (throttle > 0.45 || risk.riskStatus === "LIMITED" || risk.crashState !== "NONE")
        riskMode = "REDUCED";
    const baseSize = risk.riskStatus === "LIMITED"
    ...
    if (riskMode === "NORMAL" && throttle < 0.48) {
        if (ae === "TREND" && (mm === "TREND" || mm === "MIXED"))
            opportunityBias *= 1.07;
        if (ae === "RANGE" && (mm === "RANGE" || mm === "TRANSITION"))
    ...
    const perSlot = Math.max(1, (0, live_position_sizing_1.computePaperSizingAnchorUsd)(config));
    ...
    let alignmentBlockProtectedReason = null;
    ...
    const riskReasonLabel = riskMode === "HALT"
        ? "일일 손실 한도 또는 엔진 차단"
        : riskMode === "DEFENSIVE"
    ...
    if (riskMode === "HALT" || riskMode === "DEFENSIVE")
        riskStanceLabel = "보수·축소";
    else if (opportunityBias >= 1.08 && throttle < 0.52)
        riskStanceLabel = "기회·확대";
    else if (opportunityBias <= 0.98 || throttle > 0.58)
        riskStanceLabel = "주의·보수";
    return {
        riskMode,
        opportunityBias,
        riskStanceLabel,
