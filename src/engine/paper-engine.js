...
exports.OKX_INSTRUMENT_SPECS = exports.PaperEngine = exports.normalizeOkxSwapContractsFromNotional = void 0;
exports.normalizeEntryStageFromSizeEvidence = normalizeEntryStageFromSizeEvidence;
exports.buildV2ConfigBridge = buildV2ConfigBridge;
exports.buildV2StateBridge = buildV2StateBridge;
exports.computeOkxFilledNotionalUsdt = computeOkxFilledNotionalUsdt;
exports.getOkxInstrumentMeta = getOkxInstrumentMeta;
const path = __importStar(require("node:path"));
const fs = __importStar(require("node:fs/promises"));
const node_crypto_1 = require("node:crypto");
const types_1 = require("../models/types");
const json_store_1 = require("../storage/json-store");
const okx_demo_1 = require("../exchange/okx-demo");
const okx_position_sync_1 = require("../exchange/okx-position-sync");
const position_ops_monitor_1 = require("./position-ops-monitor");
const trend_filter_1 = require("../strategy/trend-filter");
const entry_signal_1 = require("../strategy/entry-signal");
const regime_exit_1 = require("../strategy/regime-exit");
const live_partial_exit_policy_1 = require("../strategy/live-partial-exit-policy");
const live_position_sizing_1 = require("../strategy/live-position-sizing");
const entry_gate_1 = require("../strategy/entry-gate");
const paper_entry_quality_1 = require("../strategy/paper-entry-quality");
const market_regime_detector_1 = require("../strategy/market-regime-detector");
const position_manager_1 = require("./position-manager");
const risk_manager_1 = require("./risk-manager");
const risk_control_layer_1 = require("../strategy/risk-control-layer");
const range_executor_1 = require("../strategy/executors/range-executor");
const ai_highway_filter_1 = require("./ai-highway-filter");
const highway_exit_engine_1 = require("./highway-exit-engine");
const executor_normalize_1 = require("../strategy/executors/executor-normalize");
const entry_approval_1 = require("../ai/entry-approval");
const decision_funnel_1 = require("./decision-funnel");
const paper_symbol_decision_1 = require("./paper-symbol-decision");
const directional_routing_1 = require("./directional-routing");
const paper_close_finalize_1 = require("./paper-close-finalize");
const mode_selector_1 = require("./mode-selector");
const risk_exposure_1 = require("./risk-exposure");
const explanation_layer_1 = require("./explanation-layer");
const index_1 = require("../engine-v2/index");
const okx_swap_sizing_1 = require("../engine-v2/okx-swap-sizing");
Object.defineProperty(exports, "normalizeOkxSwapContractsFromNotional", { enumerable: true, get: function () { return okx_swap_sizing_1.normalizeOkxSwapContractsFromNotional; } });
const env_1 = require("../config/env");
const reconciler_1 = require("../engine-v2/reconciler");
/** RANGE 1m edge reversal candle ?덇굅??寃뚯씠????V2 ?ㅽ뻾 遊됲닾媛 ENTER쨌臾댄븯?쒕툝濡앹씪 ??理쒖쥌 ?ㅽ뻾 寃쎈줈瑜?留됱? ?딅룄濡?遺꾨━?쒕떎. */
const LEGACY_RANGE_EDGE_NO_REVERSAL_REJECTS = new Set([
    "RANGE_LOWER_LONG_NO_REVERSAL_CONFIRMATION",
    "RANGE_UPPER_SHORT_NO_REVERSAL_CONFIRMATION"
]);
const balance_authority_1 = require("../engine-v2/live-account/balance-authority");
const range_engine_1 = require("./range-engine");
...
function computeEntryEvidenceScore(input) {
    ...
    if (input.candidateStrength === "strong")
        score += 20;
    else if (input.candidateStrength === "weak")
        score += 8;
    if (input.activeEngine === "RANGE" && typeof input.boxPos === "number") {
        const zone = (0, types_1.classifyRangeZone)(input.boxPos);
        if ((input.side === "short" && zone === "upper") || (input.side === "long" && zone === "lower"))
            score += 20;
    }
    if (input.activeEngine === "TREND") {
        if (input.trendOk)
            score += 10;
        if (Math.abs(input.emaGap ?? 0) >= ENTRY_EVIDENCE_TREND_EMA_GAP_MIN)
            score += 6;
        if ((input.trendWeaknessScore ?? 1) <= ENTRY_EVIDENCE_TREND_WEAKNESS_MAX)
            score += 4;
    }
    return Math.max(0, Math.min(100, score));
}
function computeAvgBoxCohesion01(snapshots) {
    let sum = 0;
    let n = 0;
    for (const s of snapshots) {
        if (s.boxHigh == null || s.boxLow == null || s.lastPrice <= 0)
            continue;
...
function volumeRatioProxyFromCandles(candles) {
...
function toSymbolDiagnostic(symbol, endpoint, d) {
    return {
        symbol,
        endpoint,
...
function buildSignalSummary(snapshots) {
    const totalSymbols = snapshots.length;
    const longV1 = snapshots.filter((s) => s.signal === "paper_long_candidate").length;
    const longV2 = snapshots.filter((s) => s.signal === "paper_long_candidate_v2").length;
    const shortV1 = snapshots.filter((s) => s.signal === "paper_short_candidate").length;
    const shortV2 = snapshots.filter((s) => s.signal === "paper_short_candidate_v2").length;
    return {
        totalSymbols,
...
function deriveRunIdentity(snapshots) {
    const v2Sigs = snapshots.filter(s => s.signal.includes("_v2"));
...
function buildRequestDiagnosticsBySymbol(diags) {
    const bySymbol = {};
    for (const d of diags) {
        const sym = String(d.symbol);
        const cur = bySymbol[sym] ?? {};
        const slice = diagToSlice(d);
        if (d.endpoint === EP.ticker)
            bySymbol[sym] = { ...cur, ticker: slice };
        else if (d.endpoint === EP.kline)
            bySymbol[sym] = { ...cur, kline: slice };
        else if (d.endpoint === EP.funding)
            bySymbol[sym] = { ...cur, funding: slice };
    }
    return bySymbol;
}
/** Latest close per symbol: time + side + 泥?궛 ?ъ쑀쨌?④퀎 (?ъ쭊??荑⑤떎?는룹셿???먮떒??. */
function latestCloseMetaBySymbol(history) {
    const m = new Map();
    for (const row of history) {
        ...
        const side = o.side;
        if (typeof sym !== "string" || typeof closed !== "number" || !Number.isFinite(closed))
            continue;
        ...
        if (!prev || closed >= prev.closedAt) {
            ...
            const closeReason = cr === "candidate_lost" ||
                cr === "take_profit" ||
                cr === "stop_loss" ||
                cr === "trailing_stop" ||
                cr === "time_based_exit" ||
...
function buildFailureSummary(errors) {
    const out = {};
    for (const e of errors) {
        const sym = String(e.symbol);
        const fe = e.failedEndpoint;
        const diagnosticRef = fe === "unknown" ? `requestDiagnostics.bySymbol.${sym}` : `requestDiagnostics.bySymbol.${sym}.${fe}`;
        out[sym] = {
            failed: true,
            failedEndpoint: fe,
...
function orderBuildFailureStructuredPayload(first, res, entryStage, regime) {
    const d = res.decision;
    const ex = res.executorDecision;
    const af = res.adaptiveFailure;
    const adaptiveMergedDetail = (res.adaptiveDetail ?? af?.detail ?? null);
    const entryPolicyProof = adaptiveMergedDetail && typeof adaptiveMergedDetail.entry_policy_proof === "object"
        ? adaptiveMergedDetail.entry_policy_proof
        : null;
    const trendVolumeRelaxProof = adaptiveMergedDetail && typeof adaptiveMergedDetail.trend_volume_relax_proof === "object"
        ? adaptiveMergedDetail.trend_volume_relax_proof
        : (d.trend_volume_relax_proof ?? null);
    const side = res.intentSide ??
...
function parseOkxSubmitErrorMessage(msg) {
    const mApi = /^okx_api_([^:]+):([\s\S]*)$/.exec(msg);
    if (mApi)
        return { code: mApi[1].trim(), message: (mApi[2] ?? "").trim() || "request_failed" };
    const mHttp = /^okx_http_([^:]+):([\s\S]*)$/.exec(msg);
    if (mHttp)
        return { code: `http_${mHttp[1].trim()}`, message: (mHttp[2] ?? "").trim() || "request_failed" };
    return { code: null, message: msg };
}
/** OKX `clOrdId`: letters/digits only, max length 32 (hyphens and underscores rejected by API). */
function okxClOrdIdSideCode(side) {
    switch (side) {
        case "buy":
            return "b";
        case "sell":
...
function buildOkxClOrdId(rawSymbol, sideForCode) {
    ...
    const suffix = (0, node_crypto_1.randomUUID)().replace(/-/g, "").slice(0, 8);
 ...
 */
function isOrderEngineOwned(ord) {
    if (!ord.clOrdId)
        return false;
    return ord.clOrdId.startsWith("p");
}
function parseOkxSwapInstrumentSizing(inst) {
    ...
    if (!Number.isFinite(lotSz) || lotSz <= 0)
        return null;
...
function isBtcEthSampleSymbol(symbol) {
    const s = String(symbol).toUpperCase();
    return s === "BTCUSDT" || s === "ETHUSDT";
}
function normalizeEntryStageFromSizeEvidence(rec) {
    const entryStage = rec.entryStage ?? 1;
    const scaled = typeof rec.initialSizeUsd === "number" &&
...
function normalizeRangeManagementState(rec) {
    if (rec.regimeAtEntry !== "RANGE") {
        return { normalized: rec, changed: false };
    }
    let normalized = rec;
    let changed = false;
    const raw = rec.rangeManagementState;
    let currentState = raw === "REATTACK_ELIGIBLE"
        ? "REATTACK_READY"
        : raw === "REATTACK_READY" || raw === "REATTACK_USED" || raw === "PROFIT_LOCKED"
            ? raw
            : "INIT";
    const isScaled = (normalized.entryStage ?? 1) >= 2 ||
...
function isCommittedEntryStopPrice(v) {
    return typeof v === "number" && Number.isFinite(v) && v !== 0;
}
function extractV2StrategyStopPriceForEntry(authority, decisionStopLoss) {
    const n = (u) => typeof u === "number" && Number.isFinite(u) && u !== 0 ? u : null;
    const inv = n(authority.invalidationPx);
    if (inv != null)
        return { price: inv, source: "invalidation_px" };
    const asp = n(authority.stopPrice);
    if (asp != null)
        return { price: asp, source: "authority_stop_price" };
    const nsp = n(authority.newStopPrice);
    if (nsp != null)
        return { price: nsp, source: "new_stop_price" };
    const dsl = n(decisionStopLoss);
    if (dsl != null)
        return { price: dsl, source: "decision_stop_loss" };
    return null;
}
function buildV2PreEntryRiskPlanCommitted(authority, decision, side, referenceEntryPx) {
    if (!(referenceEntryPx > 0) || !Number.isFinite(referenceEntryPx))
        return { ok: false };
    const sp = extractV2StrategyStopPriceForEntry(authority, decision.stopLoss);
    if (sp == null)
        return { ok: false };
    const tp = typeof decision.takeProfit === "number" && Number.isFinite(decision.takeProfit) && decision.takeProfit !== 0
        ? decision.takeProfit
...
function isPaperCloseAttestationComplete(row) {
    const cr = row.closeReason;
    if (cr == null || cr === "" || cr === "none")
        return false;
    const et = row.exitType;
    const cs = row.closeSource;
...
class PaperEngine {
    ...
    positions;
    risk;
    lastAdaptiveMode = { mode: "sideways", detail: {} };
    lastRegime = market_regime_detector_1.INITIAL_ENGINE_REGIME;
    lastHealthyBtcRegime = null;
    lastRisk = null;
    lastEffectiveLane = "IDLE";
    lastModeChangeAt = 0;
    lastEntryDecision = null;
    rangeCooldownUntilByKey = new Map();
    rangeFailCountByKey = new Map();
    trendCooldownUntilBySymbol = new Map();
    /** 鍮꾩쁺?? 理쒓렐 ?깅퀎 `decision_funnel_tick` ?ㅻ깄??(理쒕? DECISION_FUNNEL_RING_MAX). */
    decisionFunnelTickRing = [];
    /** 鍮꾩쁺?? Stage 1 吏꾩엯 寃??SKIP) 以묒씤 ?щ낵??泥대쪟 ?쒓컙 諛??덉쭏 異붿쟻 */
    /** 鍮꾩쁺?? Stage 1 吏꾩엯 寃€??SKIP) 以묒씤 ?щ낵??泥대쪟 ?쒓컙 諛??덉쭏 異붿쟻 */
    reviewingState = new Map();
    lastMarketMode = null;
    lastRiskExposure = null;
    lastExplanation = null;
    rangeRuntimeBySymbol = new Map();
    trendBreakoutBySymbol = new Map();
    trendHoldMemoryBySymbol = new Map();
    trendPyramidLevelBySymbol = new Map();
    rangeRecheckPromotionByKey = new Map();
    v2RecoveryActiveBySymbol = new Map();
    /** RANGE ?듭젅 ???ъ쭊??荑⑤떎???고쉶 ?먮떒??留뚮즺 ?쒓컖). */
    rangeReopenArmedUntilBySymbol = new Map();
    /** 吏곸쟾 ??`evaluateRangeEngineForSymbol` 寃곌낵(吏꾩엯 ?ш린쨌?섎뜑 ?곕룞). */
    lastTickRangeEvalBySymbol = new Map();
    lastTickSymbolSnapshotBySymbol = new Map();
    rangeReversalImmediateSwitchForSymbol = new Map();
    rangeReversalSwitchPendingBySymbol = new Map();
    /** TREND ?ㅼ쐞移??쒓컖(1h 移댁슫????selector). */
    trendSwitchTimestampsMs = [];
    /** RANGE ?ъ쭊???깃났 ?쒓컖(?덈룄 ???잛닔 ?쒗븳). */
    rangeReopenTimestampsBySymbol = new Map();
    trendFollowScoreBySymbol = new Map();
    trendBreakoutConfidenceBySymbol = new Map();
    rangeRoundTripStreakBySymbol = new Map();
    /** Consecutive close-eval ticks with raw box break (for EXIT_RANGE_REBALANCE debounce). */
    rangeBoxBreakConsecutiveBySymbol = new Map();
    /** RANGE ?섏씡沅?異붿쥌: ?щ낵:openedAt ???쇳겕쨌?좉툑 (諛뺤뒪 ?댄깉 由щ갭?곗뒪 吏?곗슜). */
    rangeProfitTrailByKey = new Map();
    /** RANGE upper short add-on: per-position one-shot guard (symbol:openedAt -> count). */
    rangeUpperShortAddOnCountByKey = new Map();
    rangeRecentOutcomeScoresBySymbol = new Map();
    /** ?μ꽭 遺?곹빀 醫낅즺(EXIT_REGIME) ?뚮え ?대젰. ?숈씪 ?먮쫫 ??諛섎났 吏꾩엯/醫낅즺 諛⑹?. */
    regimeExitConsumedBySymbol = new Map();
    /** RANGE edge stop_loss ?ъ쭊??李⑤떒??硫붾え由?留? */
    rangeStopReentryBlockedBySymbol = new Map();
    lastExitReasonLabel = "";
    lastSwitchReasonLabel = "";
    /** 吏곸쟾 ??援ш컙 諛섏쟾 泥?궛 ?곸슜(PEL proof?? */
    rangeReversalExitThisTickBySymbol = new Map();
    /**
     * Flow-based one-shot deduplication for terminal exits (EXIT_REGIME, EXIT_SL, EXIT_TIME_STOP).
     * Key: `${symbol}:${side}:${openedAt}`
     */
    terminalExitConsumedByFlow = new Set();
    /** symbol:side -> true if OKX actual position exists but not in paper ledger. */
    symbolExternalManualBlocked = new Set();
    /** symbol -> true if protective stop order failed to register; block further entries. */
    symbolProtectionFailedBlocked = new Set();
    /** Deduplication for V2 exit authority proof logs. Key: `${symbol}_${side}` */
    lastV2ExitAuthorityProofBySymbol = new Map();
    /** Manual close cooldown to prevent immediate re-entry. Key: symbol */
    manualCloseCooldownBySymbol = new Map();
    /** Cached active algo orders from exchange for auto-repair logic. */
    cachedOpsAlgos = [];
    instrumentCache = new Map();
    lastInstrumentCacheUpdateAt = 0;
    instrumentCacheTTL = 3600_000; // 1 hour
    okxDemo;
    okxAccountConfigLoaded = false;
    okxDemoKeysLoaded = false;
    okxSignedRestReady = false;
    okxAccountConfigOk = false;
    okxBalanceOk = false;
    okxPositionsOk = false;
    okxOrderSubmitOk = false;
    okxSmokeTestPerformed = false;
    okxWalletBalanceUsdt = null;
    okxAvailableBalanceUsdt = null;
    liveBalanceReady = false;
    liveBalanceBlockReason = null;
    liveBalanceFetchError = null;
    lastLiveBalancePayload = null;
    lastLivePositionsPayload = null;
    lastSignedRestError = null;
    lastSignedRestSuccessAt = null;
    lastSignedRestFailAt = null;
    runCycleId = 0;
    paperExecutionReady = false;
    paperExecutionReadyChangedAt = null;
    signedExecutionReady = false;
    signedExecutionReadyChangedAt = null;
    freshTickRequiredAfterReadiness = false;
    readinessTransitionCycleId = null;
    staleEntryDroppedCount = 0;
    /** barrier hit 시 V2 ENTER 후보를 다음 사이클 재검증까지 보존 (심볼별 1개, Map dedupe) */
    v2DeferredEntryQueue = new Map();
    lastEntryEvaluatedAt = null;
    lastEntrySignalFetchedAt = null;
    readinessFreshTickCompletedCycles = 0;
    readinessFreshTickRequiredCycles = 2;
    readinessFreshTickLastFetchedAt = null;
    readinessFreshTickLastCandleTs = null;
    contaminatedEntrySamples = [];
    lastEntryQualitySamples = {
        profit: [],
        loss: [],
        contaminated: []
    };
    lastEntryQualitySampleSourceBreakdown = {
    ...
    engineLastTickAt = null;
    marketDataLastUpdateAt = null;
    v2LastDecisionAt = null;
    bundleLastWrittenAt = null;
    /** Main loop scheduling (mirrors `src/loop.ts` + ORBITALPHA_PAPER_LOOP_INTERVAL_MS). */
    loopIntervalTargetMs = 10_000;
    lastLoopDurationMs = null;
    lastLoopStartedAt = null;
    lastLoopFinishedAt = null;
    loopDelayReason = null;
    publicMarketDataReady = false;
    v2JudgmentReady = false;
    positionTrackingAlive = false;
    bundleWriterReady = false;
    entryPipelineReady = false;
    exitPipelineReady = false;
    serverTradeControlState = {
    ...
    executionKeysConsumed = new Set();
    historyDirty = true;
    bundleDirty = true;
    lastHistoryRefreshAt = 0;
    lastBundleWriteAt = 0;
    cachedHistory = [];
    executionKeysLoaded = false;
    lastServerTradeControlSignature = null;
    /** When server trade flips disabled?뭪rue; stale-gate for authority/snapshot timestamps. */
    serverTradeEnabledTrueAt = null;
    lastObservedServerTradeEnabled = false;
    reconcileSafetyCloseOnly = false;
    reconcileLastCheckedAt = null;
    reconcileLastMismatchReason = null;
    reconcileCheckIntervalMs = 30_000;
    startupRecoveryBarrierApplied = false;
    /** OKX swap pending orders cache ??diagnostics only; never auto-submits protective orders (stage 1). */
    lastOpsOrdersScanAtMs = 0;
    opsOrdersScanMinIntervalMs = 10_000;
    opsOrdersScanEverDone = false;
    cachedOpsPending = [];
    cachedOpsFetchErrors = [];
    lastPositionOpsSurface = null;
    positionExitProofThrottleByFlow = new Map();
    tradeControlPath() {
    ...
    async loadServerTradeControlState(nowTs) {
        ...
        try {
            ...
            this.serverTradeControlState = {
                server_trade_enabled: typeof camel.serverTradeEnabled === "boolean"
                    ? camel.serverTradeEnabled
                    : parsed.server_trade_enabled === true,
                close_only_mode: typeof camel.closeOnlyMode === "boolean"
                    ? camel.closeOnlyMode
                    : parsed.close_only_mode === true,
                kill_switch_active: typeof camel.killSwitch === "boolean"
                    ? camel.killSwitch
                    : parsed.kill_switch_active === true,
                authority_source: "server_state",
                updated_at: typeof camel.updatedAt === "number" && Number.isFinite(camel.updatedAt)
                    ? camel.updatedAt
    ...
    async ensureExecutionKeysLoaded() {
        if (this.executionKeysLoaded)
            return;
        ...
        try {
            ...
            for (const k of parsed.consumed ?? []) {
                if (typeof k === "string" && k.trim() !== "")
                    this.executionKeysConsumed.add(k);
            }
        }
        catch {
            // empty bootstrap
    ...
    currentServerTradeControlSignature() {
        return [
    ...
    clearPendingDecisionState(reason, changedAt, authoritySource) {
        const dropped = {
        ...
        const droppedCount = dropped.reviewing_state +
    ...
    evaluateServerTradeControlTransition(nowTs) {
        ...
        const transitionReason = !enabledBefore && enabledNow ? "server_trade_enabled_true" : "server_trade_control_transition";
    ...
    applyStartupRecoveryBarrier(nowTs) {
        if (this.startupRecoveryBarrierApplied)
            return;
    ...
    async updateInstrumentCache() {
        const now = Date.now();
        if (now - this.lastInstrumentCacheUpdateAt < this.instrumentCacheTTL && this.instrumentCache.size > 0)
            return;
        if (!this.okxPublic)
            return;
        try {
     ...
     */
    async runPositionOperationsWatch(nowTs, paperOpens) {
        await this.updateInstrumentCache();
        const syncSnap = (0, okx_position_sync_1.buildLedgerOkxPositionSyncSnapshot)(paperOpens, this.lastLivePositionsPayload, this.instrumentCache);
        this.symbolExternalManualBlocked.clear();
        const currentRunBlockedSymbols = new Set();
        ...
        for (const sym of this.symbolProtectionFailedBlocked) {
            ...
            const inExchange = (this.lastLivePositionsPayload ?? []).some(r => {
                const hit = (0, okx_position_sync_1.okxSwapRowToLedgerKey)(r);
        ...
        if (this.lastLivePositionsPayload && Array.isArray(this.lastLivePositionsPayload)) {
            for (const row of this.lastLivePositionsPayload) {
                const hit = (0, okx_position_sync_1.okxSwapRowToLedgerKey)(row);
                if (!hit)
                    continue;
                const key = hit.key; // symbol:side
                // Block if not in ledger OR if in ledger as EXTERNAL_MANUAL_POSITION
                const isNormalInLedger = paperOpens.some(p => `${p.symbol}:${p.side}` === key &&
        ...
        const criticalMismatch = hasUnblockedMismatch &&
        ...
        const canScan = Boolean(this.okxDemo && this.signedExecutionReady) &&
        ...
        if (canScan && this.okxDemo) {
            ...
            if (this.lastLivePositionsPayload && Array.isArray(this.lastLivePositionsPayload)) {
                for (const row of this.lastLivePositionsPayload) {
                    ...
                    const avgPx = typeof avgPxRaw === "number"
                        ? avgPxRaw
            ...
            const surfaceScan = (0, position_ops_monitor_1.buildPositionOpsSurface)({
                now: nowTs,
                paperOpens,
            ...
            for (const r of surfaceScan.rows) {
                ...
                const algoForSymbol = this.cachedOpsAlgos?.filter((a) => a.instId === r.inst_id) ?? [];
                const swapForSymbol = this.cachedOpsPending?.filter((a) => a.instId === r.inst_id) ?? [];
                ...
                if (allOpenOrders.length > 0) {
                    for (const ord of allOpenOrders) {
                        ...
                        else if (isReduceOnly && hasEngineClOrdId)
                        ...
                        if (!isReduceOnly && (ord.ordId || ord.algoId) && liveExposure) {
                            ...
                            try {
                                if (ord.algoId) {
                                    await this.okxDemo?.cancelAlgoOrder([{ instId: (0, okx_demo_1.toOkxSwapInstId)(r.symbol), algoId: ord.algoId }]);
                                }
                                else if (ord.ordId) {
                                    await this.okxDemo?.cancelOrder((0, okx_demo_1.toOkxSwapInstId)(r.symbol), ord.ordId);
                                }
                ...
                if (r.symbol === "BTCUSDT" && this.isBtcSuppressionTarget()) {
                    ...
                    const pricingLast = r.okx_avg_px != null && r.okx_avg_px > 0 ? r.okx_avg_px : ledgerPos.entryPrice > 0 ? ledgerPos.entryPrice : undefined;
        ...
        this.lastPositionOpsSurface = (0, position_ops_monitor_1.buildPositionOpsSurface)({
            now: nowTs,
            paperOpens,
            okxPayload: this.lastLivePositionsPayload,
            pendingOrders: this.cachedOpsPending,
    ...
    async logAndSuppressBtcUsdtAction(action, paperSide = "none", extraSuppressedActions = []) {
        let okxActualSide = "none";
        if (this.lastLivePositionsPayload && Array.isArray(this.lastLivePositionsPayload)) {
            for (const p of this.lastLivePositionsPayload) {
                const hit = (0, okx_position_sync_1.okxSwapRowToLedgerKey)(p);
        ...
        if (resolvedPaperSide === "none") {
            try {
                ...
                const paperBtcPos = opens.find((p) => p && p.symbol === "BTCUSDT");
                if (paperBtcPos) {
        ...
        const v2InferredSide = this.lastRisk?.v2InferredSide ?? okxActualSide;
    ...
    isBtcSuppressionTarget() {
        return true;
    }
    async runPositionStateReconciliation(nowTs) {
        if (this.reconcileLastCheckedAt != null && nowTs - this.reconcileLastCheckedAt < this.reconcileCheckIntervalMs)
            return;
        this.reconcileLastCheckedAt = nowTs;
        if (!this.okxDemo || !this.signedExecutionReady)
            return;
        ...
        for (const row of okxPositions) {
            ...
            if (!hit)
                continue;
            const posSideRaw = String(row.posSide ?? "").toLowerCase();
        ...
        const untrackedFills = [];
        const RECONCILE_GRACE_PERIOD_MS = 30_000;
        const reconcileManualFullClose = async (open, source) => {
            ...
            const metrics = (0, paper_close_finalize_1.computePaperCloseLegMetrics)({
                open,
                closePrice,
                closedAt,
            ...
            const closedRow = (0, paper_close_finalize_1.finalizePaperClosedRecord)({
                open,
                symbol: open.symbol,
                closePrice,
                closedAt,
            ...
            if (this.okxDemo) {
                try {
                    const instId = (0, okx_demo_1.toOkxSwapInstId)(open.symbol);
                    ...
                    if (pendRes.ok && pendRes.value) {
                        for (const ord of pendRes.value) {
                            ...
                            if (!isReduceOnly && ord.ordId) {
                                ...
                                await this.okxDemo?.cancelOrder((0, okx_demo_1.toOkxSwapInstId)(open.symbol), String(ord.ordId));
                            }
                        }
                    }
                }
        ...
        for (let open of rawOpens) {
            ...
            const isAdoptedFamily = open.reconcileState === "ADOPTED" ||
            ...
            if (isAdoptedFamily && remotePos && remotePos.notionalUsd > 0) {
                ...
                const needsCurrentNotionalSync = Math.abs(open.sizeUsd - remoteNotional) > 0.5 ||
                    !open.notionalUsd ||
            ...
            if (isPending || open.lifecycleState === "INITIAL" || !open.lifecycleState) {
                if (remotePos) {
                    const instIdR = (0, okx_demo_1.toOkxSwapInstId)(open.symbol);
            ...
            if (isClosePending || isPartialPending) {
                ...
                if (!(ordId || clOrdId)) {
                    ...
                    if (isPartialPending) {
                        ...
                        const slReg = (0, position_ops_monitor_1.regimeForSl)(open.regimeAtEntry);
                        const newStop = (0, position_ops_monitor_1.engineMirrorStopPrice)(remotePos.avgPx, open.side, slReg);
                ...
                const ordRes = await this.okxDemo.getOrder((0, okx_demo_1.toOkxSwapInstId)(open.symbol), ordId, clOrdId);
                if (ordRes.ok && ordRes.value.length > 0) {
                    const ord = ordRes.value[0];
                        ...
                        if (isClosePending) {
                            if (orderState === "filled") {
                                ...
                                const closedRow = (0, paper_close_finalize_1.finalizePaperClosedRecord)({
                                    ...
                                    closePrice: fillPx,
                                    closedAt: fillTime,
                                    closeReason: open.closePendingReason || "v2_exit_authority",
                                    legMarginUsd: open.sizeUsd,
                                    metrics,
                                    feeRate: this.config.paperTakerFeeRate,
                                    fundingIntervalHours: this.config.paperFundingIntervalHours,
                            ...
                            if (unitConversionOk && deltaFillSz > 0) {
                                ...
                                if (open.pos != null)
                                    open.pos = open.sizeUsd / (open.entryPrice || 1);
                                if (open.baseQty != null)
                                    open.baseQty = Math.max(0, open.baseQty - (deltaFillSz * ctVal));
                                if (open.okxContracts != null)
                                    open.okxContracts = Math.max(0, open.okxContracts - deltaFillSz);
                                if (open.notionalUsd != null)
                                    open.notionalUsd = Math.max(0, open.notionalUsd - deltaFilledUsd);
            ...
            if (open.lifecycleState === "OPEN" || open.lifecycleState === "CLOSE_ONLY_MANAGED") {
                ...
                const paperBaseForCompare = typeof open.baseQty === "number" && Number.isFinite(open.baseQty) && open.baseQty > 0
                    ? open.baseQty
                    : paperNotional / Math.max(1e-12, open.entryPrice || 1);
                const sizeDiff = typeof open.baseQty === "number" && Number.isFinite(open.baseQty) && open.baseQty > 0
                ...
                else if (typeof open.baseQty === "number" &&
                ...
                const instIdHydrate = (0, okx_demo_1.toOkxSwapInstId)(open.symbol);
        ...
        for (const [key, remoteVal] of remoteMap.entries()) {
            if (!rawOpens.some(p => `${p.symbol}:${p.side}` === key)) {
                ...
                const side = sideToken === "short" ? "short" : "long";
                ...
                const okxRow = (this.lastLivePositionsPayload || []).find(r => String(r.instId) === remoteVal.instId &&
        ...
        const hasNonBtcSeriousAdopted = next.some(p => p.symbol !== "BTCUSDT" &&
            p.sourceSignal === "okx_reconcile_adopted" &&
            p.lifecycleState !== "EXTERNAL_MANUAL_POSITION" &&
            p.lifecycleState !== "CLOSE_ONLY_MANAGED");
        const hasNonBtcUntrackedFills = next.some(p => p.symbol !== "BTCUSDT" &&
        ...
        if (nonBtcMismatchCount > 0 || hasNonBtcSeriousAdopted || hasNonBtcUntrackedFills || nonBtcUntrackedFillsList.length > 0) {
            ...
            if (hasNonBtcUntrackedFills || nonBtcUntrackedFillsList.length > 0)
                reason = `untracked_fills_detected_gate_blocked:${nonBtcUntrackedFillsList.join(",")}`;
            else if (hasNonBtcSeriousAdopted)
                reason = "unquarantined_adopted_position_block";
        ...
        for (const symbol of this.config.symbols) {
            const instId = (0, okx_demo_1.toOkxSwapInstId)(symbol);
    ...
    computePaperExecutionReadiness() {
        ...
        if (!currentPaper) {
            const reasons = [];
            if (!serverTradeEnabled)
                reasons.push("SERVER_TRADE_DISABLED");
            if (closeOnlyMode)
                reasons.push("CLOSE_ONLY_MODE");
            if (killSwitch)
                reasons.push("KILL_SWITCH");
            if (reconcileSafe)
                reasons.push("RECONCILE_SAFE_MODE");
            if (!engineLoopHealthy)
                reasons.push("ENGINE_LOOP_NOT_HEALTHY");
            if (!marketReady)
                reasons.push("MARKET_DATA_NOT_READY");
            if (!positionStateReady)
                reasons.push("POSITION_TRACKING_NOT_ALIVE");
            if (!writerReady)
                reasons.push("BUNDLE_WRITER_NOT_READY");
            if (!pipelineReady)
                reasons.push("PIPELINE_NOT_READY");
    ...
    computeSignedExecutionReadiness() {
        if (!this.config.okxAuthReady)
            return false;
    ...
    applyLiveBalanceAuthorityResult(result) {
    ...
    async refreshLiveBalanceSnapshot(nowTs) {
        ...
        if (this.config.okxAuthMode !== "disabled" && this.okxDemo) {
            ...
            if (needsSmokeTest) {
                ...
                try {
                    ...
                    if (this.okxSignedRestReady) {
                        ...
                        const errs = [];
                        if (!ready.configOk)
                            errs.push(`cfg:${ready.diagnostics.config?.retMsg || "fail"}`);
                        if (!ready.balanceOk)
                            errs.push(`bal:${ready.diagnostics.balance?.retMsg || "fail"}`);
                        if (!ready.positionsOk)
                            errs.push(`pos:${ready.diagnostics.positions?.retMsg || "fail"}`);
                        this.lastSignedRestError = errs.join("|");
                    }
                    // Mode-aware logs
                    if (this.okxAccountConfigOk)
                        this.logger.info("OKX_SIGNED_ACCOUNT_CONFIG_OK", { okx_auth_mode: this.config.okxAuthMode });
                    else
                        this.logger.error("OKX_SIGNED_ACCOUNT_CONFIG_FAIL", { ...ready.diagnostics.config, okx_auth_mode: this.config.okxAuthMode });
                    if (this.okxBalanceOk)
                        this.logger.info("OKX_SIGNED_BALANCE_OK", { okx_auth_mode: this.config.okxAuthMode });
                    else
                        this.logger.error("OKX_SIGNED_BALANCE_FAIL", { ...ready.diagnostics.balance, okx_auth_mode: this.config.okxAuthMode });
                    if (this.okxPositionsOk)
                        this.logger.info("OKX_SIGNED_POSITIONS_OK", { okx_auth_mode: this.config.okxAuthMode });
                    else
                        this.logger.error("OKX_SIGNED_POSITIONS_FAIL", { ...ready.diagnostics.positions, okx_auth_mode: this.config.okxAuthMode });
                    if (this.okxSignedRestReady) {
            ...
            try {
                ...
                if (bal.ok) {
                    this.lastLiveBalancePayload = bal.value?.[0] ?? null;
    ...
    buildBalanceDisplayContext(opens) {
        ...
        const authority = (0, balance_authority_1.deriveLiveBalanceAuthority)({
            okxAuthMode: mode,
    ...
    dropReadinessStaleState(reason, changedAt) {
    ...
    maxCandleTsFromSnapshots(snapshots) {
        let maxTs = null;
        for (const s of snapshots) {
    ...
    updateReadinessFreshTickBarrierProgress(fetchedAt, snapshots) {
    ...
    evaluateReadinessTransition(nowTs) {
    ...
    async processOperatorInstructions(nowTs) {
        if (!this.serverTradeControlState.instructions || this.serverTradeControlState.instructions.length === 0)
            return;
        let ledgerModified = false;
        const opens = await this.positions.loadOpenAll();
        ...
        for (const inst of this.serverTradeControlState.instructions) {
            if (inst.type === "ADOPT_EXCHANGE_STATE") {
                ...
                const okxPos = this.lastLivePositionsPayload?.find(p => {
                    const k = (0, okx_position_sync_1.okxSwapRowToLedgerKey)(p)?.key;
                    return k === key;
                });
                if (!okxPos) {
    ...
    recordRangeRoundTripOutcome(symbolKey, win) {
    ...
    recentRangeWinRate01(symbolKey) {
        const arr = this.rangeRecentOutcomeScoresBySymbol.get(symbolKey) ?? [];
        if (arr.length === 0)
            return 0.5;
        return arr.filter((x) => x > 0).length / arr.length;
    }
    constructor(config, logger, okxClientOverride, storeOverride) {
        this.config = config;
        this.logger = logger;
        this.store = storeOverride ?? new json_store_1.JsonStore(path.resolve(config.dataDir));
        this.okxPublic = new okx_demo_1.OkxDemoClient({
        ...
        this.positions = new position_manager_1.PositionManager(this.store);
        this.risk = new risk_manager_1.RiskManager(config);
        if (okxClientOverride != null) {
            ...
            if (!this.okxDemoKeysLoaded) {
                ...
                this.okxDemo = new okx_demo_1.OkxDemoClient({
        ...
        if ((config.okxDemoEnvRequested || config.okxLiveEnabled) && !config.okxExchangeAuthOptIn) {
            this.logger.info("okx_exchange_auth_disabled", {
                ...this.okxAuthProofContext(),
                detail: "OKX auth env is on but ORBITALPHA_OKX_EXCHANGE_ENABLED is not true ??no signed OKX calls; paper execution + public market data only"
            });
        }
    ...
    async runOnce() {
        ...
        for (let i = 0; i < allOpensForNormalization.length; i++) {
            ...
            if (norm1.changed || norm2.changed) {
                ...
                if (norm1.changed)
                    normalizedFieldsSet.add("entryStage");
                if (norm2.changed)
                    normalizedFieldsSet.add("rangeManagementState");
            }
        }
        if (ledgeChanged) {
        ...
        let btcFeedFallbackAgeMs = null;
        let regimeFallbackSource = "direct_btc_feed";
        let regimeDetected;
        if (btc5r.ok) {
            regimeDetected = (0, market_regime_detector_1.detectMarketRegime)({ btcCandles5m: btc5 });
            ...
            if (fallback && age <= fallbackMaxAgeMs) {
                ...
                regimeDetected = (0, market_regime_detector_1.regimeWhenBtcFeedFailed)(btc5r.error ?? "btc_candles_unavailable");
            }
        }
        ...
        const btc1m_atr = btc1m.length > 20 ? (0, entry_gate_1.atrWilderLast)(btc1m, 14) : null;
        const nowStateTs = Date.now();
        this.lastRisk = (0, risk_control_layer_1.evaluateRiskControls)({
            config: this.config,
            now: nowStateTs,
            history,
        ...
        const routingOverride = (0, directional_routing_1.deriveDirectionalRoutingOverride)({
        ...
        const adaptiveMode = routingOverride.effectiveExecutionLane === "TREND" ? "trend" :
        ...
        const failedReasons = {};
        for (const e of errors)
            failedReasons[e.symbol] = e.error;
        ...
        const readinessBarrierActive = this.freshTickRequiredAfterReadiness &&
        ...
        const hasAnyCandidate = snapshots.some((s) => s.signal.includes("_candidate"));
        let candidateRunPath;
        if (hasAnyCandidate && latestPath && metaPath) {
            try {
                const candidateSymbols = snapshots
                    .filter((s) => s.signal.includes("_candidate"))
        ...
        const rawMarketModeOut = (0, mode_selector_1.evaluateMarketModeSelector)({
        ...
        const riskExposureOut = (0, risk_exposure_1.evaluateRiskExposure)({
            config: this.config,
        ...
        const explanationOut = (0, explanation_layer_1.buildPaperExplanation)({
            marketMode: marketModeOut,
        ...
        const lastCloseMetaBySymbolForDecision = this.config.paperReentryCooldownMs > 0 ? latestCloseMetaBySymbol(await this.store.readPositionsHistory()) : null;
        const regimeUnknown = btc5.length < market_regime_detector_1.MIN_BTC_5M_BARS_REGIME;
        ...
        const effectiveRegimeForDecision = (effectiveLane === "IDLE" ? "NO_TRADE" : effectiveLane);
        ...
        for (const sym of polledSymbols) {
            ...
            if (consumed) {
                ...
                const currentSignalSide = currentSnap?.signal === "paper_long_candidate"
                    ? "long"
            ...
            const snapForDecision = snap == null
                ? null
                : (0, paper_symbol_decision_1.applyPaperSignalMarketAlignment)({
                    snapshot: snap,
                    risk,
            ...
            if (stopBlkPre && snapForDecision) {
                ...
                const curZone = (0, types_1.classifyRangeZone)(bp);
                let releaseReason = null;
                if (effectiveRegimeForDecision !== "RANGE")
                    releaseReason = "regime_not_range";
                else if (sigSide === "none")
                    releaseReason = "signal_none";
                else if (sigSide !== stopBlkPre.side)
                    releaseReason = "signal_opposite";
                else if (curZone !== stopBlkPre.zone)
                    releaseReason = "zone_left_block_zone";
                if (releaseReason) {
            ...
            const rangeReversalImmediateSwitchEarly = this.getRangeReversalImmediateSwitch(symKeyEarly, snap ?? null, fetchedAt, effectiveRegimeForDecision, marketModeOut.routing.activeEngine);
            ...
            const blockRes = symbolBlocked ? {
                decision: {
                    ...
                    signal_state: "NONE",
                    regime_state: effectiveRegimeForDecision,
                    edge_state: "PASS",
            ...
            if (!snap) {
                const resNull = blockRes || (0, paper_symbol_decision_1.evaluatePaperSymbolEntry)({
                    config: this.config,
                    snapshot: null,
                ...
                this.logger.info("PAPER_TRADE_BLOCK_DECOMPOSITION", this.paperTradeBlockDecompositionPayload(sym, null, resNull, {
                    nowTick: fetchedAt,
                    regime: effectiveRegimeForDecision,
                    effectiveLane,
                    regimeUnknown,
            ...
            // 2. Decision Logic
            let res = blockRes || (0, paper_symbol_decision_1.evaluatePaperSymbolEntry)({
                config: this.config,
            ...
            const isAdoptedOrCloseOnly = existingPos?.reconcileState === "ADOPTED" ||
            ...
            if (isAdoptedOrCloseOnly && res.decision.final_signal_state !== "NONE") {
                ...
                res.decision.final_signal_state = "NONE";
            }
            /** Engine-V2 Execution Path (Standard 2: Selector Bridge) */
            // Entry authority is fully transferred to engine-v2.
            const v2Mode = "engine_v2";
            const envelope = (0, reconciler_1.resolveSymbolDecisionEnvelope)({
                symbol: sym,
                fetchedAt,
                ...
                state: (() => {
                    ...
                    return buildV2StateBridge(opensAfterClose, this.lastRisk, this.config, this.paperExecutionReady, this.signedExecutionReady, readinessBarrierActive, this.freshTickRequiredAfterReadiness, this.readinessFreshTickCompletedCycles, this.readinessFreshTickRequiredCycles, this.buildEntryQualityProfilesForV2(), this.serverTradeControlState, this.reconcileSafetyCloseOnly, this.lastLivePositionsPayload, this.liveBalanceReady, this.okxWalletBalanceUsdt, this.okxAvailableBalanceUsdt, this.okxPositionsOk, this.okxOrderSubmitOk, 0, 0, this.lastSignedRestSuccessAt ?? fetchedAt, this.lastSignedRestSuccessAt ?? fetchedAt, this.lastSignedRestSuccessAt ?? fetchedAt);
                })(),
                v2Mode
            });
            ...
            const paperRejectReason = res.decision.reject_reason;
            const paperRiskCooldownSubreason = res.decision.risk_cooldown_subreason ?? null;
            const paperCooldownAction = (() => {
                if (!paperRejectReason)
                    return "none";
                ...
                if (paperRejectReason === "RISK_HALT")
                    return "halt";
            ...
            const cooldown_superseded_by_server_authority = !this.serverTradeControlState.server_trade_enabled ||
            ...
            if (v2CooldownAuthority) {
                ...
                if ((0, index_1.shouldEmitV2Proof)("V2_COOLDOWN_AUTHORITY_PROOF", String(sym), proofKey, highPriority)) {
                    this.logger.info("V2_COOLDOWN_AUTHORITY_PROOF", {
                        ...
                        directional_shock_state: selectorResult?.v2_result.rawMetrics?.directionalShockState ?? "UNKNOWN",
            ...
            if (v2PositionStateAuthority) {
                ...
                const trueInconsistencyReasons = [];
                const proofReasons = [];
                const knownShadowGaps = [...v2PositionStateAuthority.knownShadowGaps];
                ...
                if ((0, index_1.shouldEmitV2Proof)("V2_POSITION_STATE_AUTHORITY_PROOF", String(sym), positionStateProofKey, positionStateHighPriority)) {
            ...
            if ((envelope.runtime_authority_owner === "V2" || authority.source === "v2") &&
                ...
                if (v2Env?.marketSubtype === "WHIPSAW_SHOCK_RECHECK") {
                    ...
                    if (v2Env?.marketSubtype === "SHOCK_REACTION_UP" && v2Env?.trend_side_candidate === "long") {
                        if (zone === "mid")
                            side_veto_detail = "SHOCK_UP_MID_RETEST_REQUIRED";
                        else if (trendOk === false)
                            side_veto_detail = "SHOCK_UP_TREND_CONFIRMATION_WEAK";
                        else if (v2Env?.reversal_confirmed === false)
                            side_veto_detail = "SHOCK_UP_RECLAIM_NOT_CONFIRMED";
                    }
                    else if (v2Env?.marketSubtype === "SHOCK_REACTION_DOWN" && v2Env?.trend_side_candidate === "short") {
                        if (zone === "mid")
                            side_veto_detail = "SHOCK_DOWN_MID_RETEST_REQUIRED";
                        else if (trendOk === false)
                            side_veto_detail = "SHOCK_DOWN_TREND_CONFIRMATION_WEAK";
                        else if (v2Env?.reversal_confirmed === false)
                            side_veto_detail = "SHOCK_DOWN_BREAKDOWN_RETEST_NOT_CONFIRMED";
                    }
                ...
                let refinedMissingCondition = v2Env?.primary_missing_condition ||
                ...
                const noEntryAuditNextByVetoOrMissing = {
                ...
                const missingKey = refinedMissingCondition != null && typeof refinedMissingCondition === "string" ? refinedMissingCondition : "";
                const nextFromAuditKey = (vetoKey && noEntryAuditNextByVetoOrMissing[vetoKey]) ||
                ...
                let expected_retest_direction = null;
                if (v2Env?.marketSubtype === "WHIPSAW_SHOCK_RECHECK") {
                    chase_blocked = true;
                ...
                this.logger.info("V2_NO_ENTRY_REASON_AUDIT_PROOF", {
                    ...
                    leverage_block_reason: v2Risk?.legacy_block_reason ?? v2Risk?.blockReason ?? null,
                    ...
                    trendWeaknessScore,
                    rangeConfidence,
                    v2_entry_quality_loss_distance: v2Risk?.diagnostics?.["entry_quality_distance_loss"] ?? null,
            ...
            if (selectorResult) {
                await this.store.appendJsonlLine("reports/v2_shadow_parity.jsonl", {
                    ...(0, reconciler_1.buildV2ShadowParityPayload)(String(sym), fetchedAt, selectorResult),
        ...
        if (decisionBySymbol.size > 0)
            this.v2LastDecisionAt = fetchedAt;
        ...
        if (decisionBySymbol.size > 0) {
            ...
            const funnel_tick = (0, decision_funnel_1.computeFunnelTick)(legacyMap);
            this.decisionFunnelTickRing.push(funnel_tick);
            if (this.decisionFunnelTickRing.length > decision_funnel_1.DECISION_FUNNEL_RING_MAX) {
                this.decisionFunnelTickRing.shift();
            }
            const decision_funnel_50 = (0, decision_funnel_1.sumDecisionFunnelTicks)(this.decisionFunnelTickRing);
            const decision_funnel_50_size = this.decisionFunnelTickRing.length;
            const reject_reason_counts_tick = (0, decision_funnel_1.aggregateRejectReasonCountsTick)(legacyMap);
            try {
                const risk = this.lastRisk;
                ...
                const statusRelaxBypass = effectiveRegimeForDecision === "RANGE" &&
                    ...
                    decisionBySymbolValues.some((v) => v.decision.range_risk_limit_relax_active === true &&
                ...
                await this.store.writeJson("reports/engine-state.json", {
                    ...
                    current_regime: (routingOverride.effectiveExecutionLane === "IDLE" ? "NO_TRADE" : routingOverride.effectiveExecutionLane),
                    ...
                    entryAllowedLong: Array.from(decisionBySymbol.values()).some(env => (env.v2_side ?? env.selector?.v2_result.side) === "long" &&
                        (env.v2_decision ?? env.selector?.v2_result.decision) === "ENTER"),
                    entryAllowedShort: Array.from(decisionBySymbol.values()).some(env => (env.v2_side ?? env.selector?.v2_result.side) === "short" &&
                        (env.v2_decision ?? env.selector?.v2_result.decision) === "ENTER"),
                    blocked_reason: routingOverride.effectiveExecutionLane === "IDLE"
                    ...
                    symbol_decisions: mergeEngineSymbolDecisionsWithOpenLedgerExposure(decisionBySymbol, opensForBalanceDisplay, 500_000, this.logger),
                    exchange: "okx",
                    okx_demo_effective_enabled: this.config.okxDemoEnabled,
                    okx_demo_keys_loaded: this.okxDemoKeysLoaded,
                    paper_execution_ready: this.paperExecutionReady,
                    ...
                    fresh_tick_age_ms: this.readinessFreshTickLastFetchedAt != null ? Math.max(0, stateNow - this.readinessFreshTickLastFetchedAt) : null,
                    snapshot_age_ms: this.marketDataLastUpdateAt != null ? Math.max(0, stateNow - this.marketDataLastUpdateAt) : null,
                    okx_signed_rest_ready: this.okxSignedRestReady,
                    okx_account_config_ok: this.okxAccountConfigOk,
                    okx_balance_ok: this.okxBalanceOk,
                    okx_positions_ok: this.okxPositionsOk,
                    okx_order_submit_ok: this.okxOrderSubmitOk,
                    ledger_okx_position_sync: (0, okx_position_sync_1.buildLedgerOkxPositionSyncSnapshot)(opensForBalanceDisplay, this.lastLivePositionsPayload, this.instrumentCache),
        ...
        try {
            ...
            this.evaluateReadinessTransition(Date.now());
            const nextLoopTiming = (0, env_1.getPaperLoopIntervalMs)(process.env);
            ...
            this.logger.info("ENGINE_24H_RUNTIME_STATUS", {
                ...
                reconcileSafeMode: this.reconcileSafetyCloseOnly,
                reconcile_last_mismatch_reason: this.reconcileLastMismatchReason,
                fresh_tick_age_ms: this.readinessFreshTickLastFetchedAt != null ? Math.max(0, Date.now() - this.readinessFreshTickLastFetchedAt) : null,
                snapshot_age_ms: this.marketDataLastUpdateAt != null ? Math.max(0, Date.now() - this.marketDataLastUpdateAt) : null,
                public_market_data_ready: this.publicMarketDataReady,
                v2_judgment_ready: this.v2JudgmentReady,
                position_state_ready: this.positionTrackingAlive,
                bundle_writer_ready: this.bundleWriterReady,
            ...
            const longestPhase = phaseRows.reduce((best, cur) => (cur[1] > best[1] ? cur : best), ["none", 0]);
    ...
    async emitPipelineEventsFromDecision(first, envelope, nowTs, entryStage = 0, 
    /**
     ...
     */
    suppressLegacyEntryAllowedReason = undefined) {
        const res = envelope.legacy;
        const authority = envelope.authority;
        const sym = String(first.symbol);
        const d = res.decision;
        const ex = res.executorDecision;
        const suppressLegacyEntryAllowed = suppressLegacyEntryAllowedReason != null && suppressLegacyEntryAllowedReason !== "";
        ...
        if (d.reject_reason === "ORDER_BUILD_FAIL") {
            const structured = orderBuildFailureStructuredPayload(first, res, entryStage, effectiveRegime);
            await this.store.appendJsonlLine("reports/events.jsonl", {
                ts: nowTs,
        ...
        if (authority.decision === "REJECT" || authority.decision === "DISABLED") {
            if (ex?.entry_allowed && (d.reject_reason === "AI_REJECT" || d.reject_reason === "AI_DIRECTION_MISMATCH")) {
                ...
                const lossStreak = this.lastRisk?.recentLossStreakByMode?.[effectiveRegime] ?? 0;
                const last10Net = typeof this.lastRisk?.detail?.last10_net_usd === "number" && Number.isFinite(this.lastRisk.detail.last10_net_usd)
                    ? this.lastRisk.detail.last10_net_usd
                    : 0;
                const aiIn = ex
                    ? (0, entry_approval_1.aiInputFromDecision)({
                        decision: ex,
                    ...
                    : null;
                if (aiIn) {
                    const aiOut = (0, entry_approval_1.aiApproveEntry)(aiIn);
    ...
    pruneRangeReversalSwitchPending(symKey, snap, nowMs) {
        ...
        if (snap != null && typeof snap.boxPos === "number" && (0, types_1.classifyRangeZone)(snap.boxPos) !== p.zone) {
            this.rangeReversalSwitchPendingBySymbol.delete(symKey);
        }
    }
    getRangeReversalImmediateSwitch(symKey, snap, nowMs, regime, activeEngine) {
        if (regime !== "RANGE" || activeEngine !== "RANGE")
            return undefined;
        if (snap == null || typeof snap.boxPos !== "number")
            return undefined;
        ...
        if (!pend || nowMs > pend.untilMs)
            return undefined;
        const z = (0, types_1.classifyRangeZone)(snap.boxPos);
        if (z !== pend.zone)
            return undefined;
     ...
     */
    logHighwayCandlePipelineProof(stage, payload) {
    ...
    logHighwayCoreStiffnessProofIfNeeded(sym, res) {
        const br = res.executorDecision?.blocked_reason;
        const det = res.executorDecision?.detail;
    ...
    paperTradeBlockDecompositionPayload(sym, snap, res, ctx) {
        ...
        const blockedRegimesCompact = risk?.blockedRegimes != null
            ? Object.entries(risk.blockedRegimes)
                .filter(([, v]) => v != null)
                .map(([reg, v]) => v
                ? {
                    regime: reg,
    ...
    resolveEntryIdentity(authority, decision, liveRegime) {
        ...
        let effectiveExecutorAtEntry;
        let effectiveRegimeAtEntryCandidate;
        if (authReg === "RANGE") {
        ...
        const normalizedRegimeAtEntry = effectiveRegimeAtEntryCandidate === "UNKNOWN" ? "NO_TRADE" : effectiveRegimeAtEntryCandidate;
        return {
    ...
    isTrendManagedPosition(pos) {
        if (pos.executorAtEntry === "RANGE")
            return false;
        if (pos.regimeAtEntry === "RANGE" && pos.executorAtEntry !== "TREND")
            return false;
        const sourceSignal = typeof pos.sourceSignal === "string" ? pos.sourceSignal : "";
        if (pos.executorAtEntry === "TREND" || pos.regimeAtEntry === "TREND")
            return true;
        return (pos.strategyVersion === "paper-v2" ||
            sourceSignal.includes("_v2") ||
            (pos.executorAtEntry === undefined && pos.strategyVersion === "paper-v2"));
    }
    isEntryPostOpenRegimeLaneProtectActive(openedAt, evalAtMs, record) {
    ...
    shouldDeferRegimeLaneTransitionClose(open, evalAtMs, closeReason, postPartialProtectActive) {
     ...
     */
    evaluateRegimeExitConfirmation(open, triggerOwner, invalidationReason, requiredTicks = 2, evalAtMs) {
        ...
        if (o.regime_exit_trigger_owner !== triggerOwner ||
     ...
     */
    classifyUpperExitAuthorityTrendBreakTrigger(input) {
        const { marketMode, trendOkNow, positionSide, snapSignal, v2Decision, v2Side } = input;
        const opposingSignal = (positionSide === "long" && snapSignal === "paper_short_candidate") ||
        ...
        if (marketMode === "TREND" && !trendOkNow) {
            const v2WantsOpposite = v2Decision === "ENTER" &&
                v2Side != null &&
     ...
     */
    rangeZoneEvalProofPayload(sym, snap, res, rangeReversalImmediateSwitchIn, openPositionSide = null) {
        const d = res.decision;
        const boxPos = snap?.boxPos ?? null;
        const zone = typeof boxPos === "number" && Number.isFinite(boxPos) ? (0, types_1.classifyRangeZone)(boxPos) : null;
        ...
        const rangeSigReason = typeof exDetail?.range_signal_reason === "string" ? exDetail.range_signal_reason : null;
        const rangeSigState = typeof exDetail?.range_signal === "string" ? exDetail.range_signal : null;
        const revIn = rangeReversalImmediateSwitchIn ?? null;
        const stalled = revIn != null &&
        ...
        return {
            ...
            long_intent_upper_without_range_stage0: d.range_stage0_engine_taken !== true &&
                zone === "upper" &&
                res.intentSide === "long",
            /** ?뺤콉 ?꾨컲 ?섏떖: stage0?몃뜲 ?곷떒쨌濡??섎룄 */
            range_stage0_upper_long_intent_anomaly: d.range_stage0_engine_taken === true &&
                zone === "upper" &&
                res.intentSide === "long",
     ...
     */
    normalizeOkxReduceOrderSize(args) {
        ...
        normalized_contracts = Number(normalized_contracts.toFixed(Math.min(12, (0, okx_swap_sizing_1.okxInstrumentSzDecimals)(lot))));
        const normalized_sz = (0, okx_swap_sizing_1.formatOkxSwapContractSzString)(normalized_contracts, lot);
    ...
    async dispatchOkxClose(input) {
        ...
        const instId = (0, okx_demo_1.toOkxSwapInstId)(input.symbol);
        const inst = this.instrumentCache.get(instId);
        let finalQty = 0;
        let finalSzStr = "0";
        if (inst) {
    ...
    buildLiveLimitOrderPrice(input) {
    ...
    async cancelProtectiveStopOrder(symbol, algoId, flowId) {
        if (!this.okxDemo)
            return false;
        this.logger.info("OKX_PROTECTIVE_STOP_ORDER_CANCEL_SUBMIT", { symbol, algoId, flowId });
        const res = await this.okxDemo.cancelAlgoOrder([{ instId: (0, okx_demo_1.toOkxSwapInstId)(symbol), algoId }]);
        if (res.ok) {
     ...
     */
    async cancelProtectionOrdersForClosedPosition(symbol, posSide, openedAt, reason) {
        if (!this.okxDemo)
            return;
        const instId = (0, okx_demo_1.toOkxSwapInstId)(symbol);
    ...
    extractOkxOrderAlgoRowCodes(diagnostics) {
    ...
    validateProtectiveOkxTriggerLayout(input) {
        ...
        if (positionSide === "long") {
            if (!(slTriggerPx < lastPx))
                violations.push("LONG_SL_MUST_BE_BELOW_LAST");
            ...
            if (!(slTriggerPx > lastPx))
                violations.push("SHORT_SL_MUST_BE_ABOVE_LAST");
            if (tpTriggerPx != null && Number.isFinite(tpTriggerPx) && tpTriggerPx > 0 && !(tpTriggerPx < lastPx)) {
    ...
    async ensureProtectiveStopOrder(open, flowId, pricingLastInput, protectionSource) {
        if (!this.okxDemo)
        ...
        const syncSnap = (0, okx_position_sync_1.buildLedgerOkxPositionSyncSnapshot)(paperOpensForSync, this.lastLivePositionsPayload, this.instrumentCache);
        ...
        const accountConfig = cfgTry.value[0];
        ...
        const actualMgnMode = String(actualPos.mgnMode || "cross").toLowerCase();
        ...
        let activeStopPrice = open.stopPrice ?? open.ledger_stop_px;
        ...
        const regime = (0, position_ops_monitor_1.regimeForSl)(open.regimeAtEntry);
        ...
        if (!activeStopPrice || !Number.isFinite(activeStopPrice)) {
            if (open.isV2Authority !== true) {
                const mirroredSl = (0, position_ops_monitor_1.engineMirrorStopPrice)(refPx, open.side, regime);
        ...
        if (!activeTpPrice || !Number.isFinite(activeTpPrice)) {
            if (open.isV2Authority !== true) {
                const mirroredTp = (0, position_ops_monitor_1.engineMirrorTpPrice)(refPx, open.side, regime);
        ...
        if (!pricingLast || pricingLast <= 0)
            pricingLast = refPx;
        if (!pricingLast || pricingLast <= 0)
            pricingLast = this.lastTickSymbolSnapshotBySymbol.get(open.symbol)?.lastPrice;
        if (!pricingLast || pricingLast <= 0)
            pricingLast = 0;
        ...
        let engineOwnedSl = null;
        let engineOwnedTp = null;
        ...
        for (const algo of pendingAlgos) {
            ...
            let isWrong = false;
            if (algoTdMode !== tdModeUsed) {
                wrongTdModeCount++;
                isWrong = true;
            }
            if (Math.abs(algoSz - contractsToProtect) > 1e-8) {
                wrongSizeCount++;
                isWrong = true;
            }
            if (algoSide !== expectedSide) {
                wrongSideCount++;
                isWrong = true;
            }
            if (hasSlTrigger && Math.abs(Number(algo.slTriggerPx) - activeStopPrice) > 1e-8) {
                wrongPriceCount++;
                isWrong = true;
            }
            if (hasTpTrigger && wantsTp && Math.abs(Number(algo.tpTriggerPx) - activeTpPrice) > 1e-8) {
                wrongPriceCount++;
                isWrong = true;
            }
            if (hasSlTrigger || (isOco && wantsTp)) {
                if (!engineOwnedSl && !isWrong && isMyPosition) {
                    ...
                    duplicateSlCount++;
                    cancelTargets.push({ instId, algoId: algo.algoId });
                }
            }
        ...
        const updatedRecord = {
            ...
            protectiveSlAlgoId: engineOwnedSl?.algoId,
            protectiveTpAlgoId: engineOwnedTp?.algoId,
    ...
    async submitOkxOrder(input) {
        ...
        const logCtx = {
            ...
            signed_execution_ready: this.signedExecutionReady,
            live_order_notional_usdt: null,
            live_cap_passed: null,
            live_cap_block_reason: null,
        ...
        if (!this.signedExecutionReady) {
            ...
            const tag = input.reason.indexOf("close") !== -1
        ...
        const sizingMeta = parseOkxSwapInstrumentSizing(instTryAll.value);
        if (!sizingMeta) {
            this.logger.info("ORDER_BUILD_FAIL", {
                order_build_fail_reason: "OKX_INSTRUMENT_SIZING_METADATA_INVALID",
                symbol: input.symbol,
                instId,
                instrument_keys: Object.keys(instTryAll.value)
            });
            return {
                ok: false,
                ordId: null,
        ...
        const orderTdMode = okxAcctLv === "3" || okxAcctLv === "4" ? "cross" : "isolated";
        const payloadPosSide = okxPosMode === "long_short_mode" ? input.posSide : undefined;
        let refPrice = null;
        let final_submitted_notional_usdt = null;
        let limitPrice = null;
        if (this.config.okxAuthMode === "live") {
            ...
            let v2_intended_notional_usdt = null;
            if ((input.orderNotionalUsdt ?? 0) > 0)
                v2_intended_notional_usdt = input.orderNotionalUsdt;
            else if ((input.desiredNotionalUsdt ?? 0) > 0)
                v2_intended_notional_usdt = input.desiredNotionalUsdt;
                ...
                if (marginUsdt > 0 && lev > 0)
                    v2_intended_notional_usdt = marginUsdt * lev;
            ...
            const okx_dynamic_notional_cap_usdt = okx_available_balance_usdt != null && okx_confirmed_leverage != null
            ...
            let estimatedLiquidationPrice = appliedLeverage != null && entryPriceVal != null ? this.estimateLiquidationPrice({ side: input.side, entryPrice: entryPriceVal, leverage: appliedLeverage }) : null;
            ...
            if (input.isNewEntry && estimatedLiquidationPrice !== null && entryPriceVal !== null && (this.okxAvailableBalanceUsdt ?? 0) > 0) {
                const marginUsdt = (input.stageMarginKrw ?? 0) / PAPER_LEDGER_KRW_NOTIONAL_PER_USD;
                const entryPriceSafe = entryPriceVal;
            ...
            let stopDistancePct = canEvaluateLiquidation && stopPrice !== null && entryPriceVal !== null
                ? Math.abs(entryPriceVal - stopPrice) / Math.max(1e-9, entryPriceVal)
                : null;
            let liquidationDistancePct = canEvaluateLiquidation && estimatedLiquidationPrice !== null && entryPriceVal !== null
                ? Math.abs(entryPriceVal - estimatedLiquidationPrice) / Math.max(1e-9, entryPriceVal)
                : null;
            let stopBeforeLiquidation = canEvaluateLiquidation && estimatedLiquidationPrice !== null && stopPrice !== null && entryPriceVal !== null
                ? (input.side === "buy"
                    ? (entryPriceVal > stopPrice && stopPrice > estimatedLiquidationPrice)
                    : (entryPriceVal < stopPrice && stopPrice < estimatedLiquidationPrice))
                : false;
            let liquidationBufferRatio = stopDistancePct != null && stopDistancePct > 0 && liquidationDistancePct != null
                ? liquidationDistancePct / stopDistancePct
                : null;
            const requiredBufferRatio = 1.5;
            const liveLeverageAllowed = stopBeforeLiquidation === true &&
            ...
            if (!liveLeverageAllowed) {
                ...
                if (isV2NewEntry && isLiqDataMissing) {
                    ...
                    const missingFields = [];
                    if (stopPrice === null)
                        missingFields.push("stopPrice");
                    if (entryPriceVal === null)
                        missingFields.push("entryPrice");
                    if (estimatedLiquidationPrice === null)
                        missingFields.push("estimatedLiquidationPrice");
        ...
        let pricingLast = refPrice;
        let effectiveNotionalUsdt = this.config.okxAuthMode === "live" ? final_submitted_notional_usdt : null;
        if (this.config.okxAuthMode !== "live") {
            ...
            if ((input.orderNotionalUsdt ?? 0) > 0)
                effectiveNotionalUsdt = input.orderNotionalUsdt;
            else if ((input.desiredNotionalUsdt ?? 0) > 0)
                effectiveNotionalUsdt = input.desiredNotionalUsdt;
            else if ((input.exposureNotionalKrw ?? 0) > 0)
        ...
        if (input.reduceOnly) {
            ...
            norm = (0, okx_swap_sizing_1.normalizeOkxSwapContractsFromNotional)({
        ...
        if (instId.startsWith("BTC-")) {
            if (this.isBtcSuppressionTarget()) {
                ...
                return { ok: false, error: "BTCUSDT_PROTECTED_BYPASS" };
            }
        }
        try {
            ...
            if (!status.ok) {
                const pollError = status.error || "poll_failed";
    ...
    buildInvariantProofPayload(input) {
        return {
            symbol: input.symbol,
            side: input.side,
    ...
    async isHistoryDuplicate(record) {
        try {
    ...
    async appendClosedWithStandardRouting(input) {
        const authoritySource = input.envelope?.authority.source ??
            input.open.authoritySourceAtEntry ??
            input.open.authority ??
            null;
        const adoptedEngine = input.envelope?.selector?.adopted_result.engine ??
        ...
        await this.cancelProtectionOrdersForClosedPosition(String(input.open.symbol), input.open.side, input.open.openedAt, input.exitReason);
        ...
        const blockReason = invalidPrice ? "INVALID_CLOSE_PRICE" :
        ...
        if (blockReason) {
            this.logger.warn("HISTORY_APPEND_BLOCKED_EXTERNAL_MANUAL_POSITION", {
                ...
                okx_avg_px: okxPos ? okxPos.avgPx : null,
                okx_base_qty: okxPos ? okxPos.baseQty : null,
                okx_contracts: okxPos ? okxPos.pos : null,
                reason: blockReason,
    ...
    async tryPaperPositionClose(input) {
        ...
        const crashForceClosedKeys = new Set();
        const crashReducedThisTickKeys = new Set();
        /** Issued only immediately after a successful `appendClosed` row with full attestation (never from terminal-dedup shortcut). */
        const openLedgerPruneAuthorizedFlowIds = new Set();
        const authorizeOpenLedgerPruneAfterAttestedClose = (fid, closedRow) => {
            ...
            for (const op of opens) {
                ...
                if (isLong) {
                    ...
                    if (forceExit || forceReduce) {
                        ...
                        const m = (0, paper_close_finalize_1.computePaperCloseLegMetrics)({
                            open: op,
                        ...
                        if (forceExit) {
                            const routedClosed = await this.appendClosedWithStandardRouting({
                                ...
                                currentRegime: input.marketMode.marketMode ?? "NO_TRADE"
                            });
        ...
        // ------------------------------------
        let remaining = [];
        const stopBackfillSaveExpectations = [];
        ...
        const exitEventJsonlType = (r) => {
            if (r === "time_based_exit")
                return "EXIT_TIME_STOP";
            if (r === "stop_loss")
                return "EXIT_SL";
            const t = (0, paper_close_finalize_1.paperExitDisplayMeta)(r).exitType;
            if (t === "EXIT_PARTIAL_SPLIT_1" || t === "EXIT_PARTIAL_SPLIT_2")
                return "EXIT_PARTIAL_SPLIT";
            if (t === "EXIT_PARTIAL_TP" || t === "EXIT_TP_1" || t === "EXIT_TP_2")
                return "EXIT_TP";
            if (t === "EXIT_TIME_STOP")
                return "EXIT_TIME_STOP";
            if (t === "EXIT_SL")
                return "EXIT_SL";
            if (r === "v2_exit_authority")
                return "EXIT_V2_AUTHORITY";
            if (t === "EXIT_TRAILING" || t === "EXIT_REGIME")
                return "EXIT_REGIME";
            if (t === "EXIT_REGIME_BREAK")
                return "EXIT_TREND_BREAK";
            return t;
        };
        const exitDetailBase = (open, m) => ({
        ...
        for (const openRaw of opens) {
            ...
            if (open.status === "open" && (open.stopPrice === undefined || open.stopPrice === null || !Number.isFinite(open.stopPrice))) {
                ...
                const slReg = (0, position_ops_monitor_1.regimeForSl)(open.regimeAtEntry);
                const newStop = ep > 0 ? (0, position_ops_monitor_1.engineMirrorStopPrice)(ep, open.side, slReg) : null;
                if (newStop != null && Number.isFinite(newStop)) {
            ...
            const exitLane = trendManagedPosition ? "TREND" : "RANGE";
            const slRegime = exitLane === "RANGE" ? "RANGE" : "TREND";
            ...
            if (snap) {
                closePrice = snap.lastPrice;
                m = (0, paper_close_finalize_1.computePaperCloseLegMetrics)({
                    open,
                    closePrice,
                    closedAt: snap.fetchedAt,
                    snapFundingRate: snap.fundingRate,
            ...
            const isManaged = open.lifecycleState === "OPEN" ||
            ...
            const leg = (marginUsd) => (0, paper_close_finalize_1.computePaperCloseLegMetrics)({
                open,
                closePrice,
                closedAt,
                snapFundingRate: snap.fundingRate,
            ...
            const rangeManagedPosition = open.regimeAtEntry === "RANGE" &&
                open.executorAtEntry !== "TREND" &&
                !trendManagedPosition;
            const blockedByExecutorMismatch = open.regimeAtEntry === "RANGE" &&
                trendManagedPosition;
            let finalCloseReason = "none";
            let confirmedExitType = null;
            let confirmedCloseSource = null;
            let posTrail = { ...open };
            ...
            const regimeNow = input.marketMode.marketMode;
            const snapPaths = {
            ...
            if (open.lifecycleState === "CLOSE_PENDING" && open.closePendingClOrdId) {
                ...
                this.logger.info("OKX_ORDER_FILL_STATUS_PROOF", {
                    ...
                    ok: status.ok,
                    state: status.value?.[0]?.state,
                    fillSz: status.value?.[0]?.fillSz,
                    flowId
                });
                if (status.ok && status.value?.[0]) {
                    ...
                    if (isActuallyClosedOnOkx) {
                        ...
                        const closedRow = (0, paper_close_finalize_1.finalizePaperClosedRecord)({
                            ...
                            closePrice,
                            closedAt: snap.fetchedAt,
                            closeReason: open.closePendingReason || "stop_loss",
                            legMarginUsd: open.sizeUsd,
                            metrics,
                            feeRate: this.config.paperTakerFeeRate,
                            fundingIntervalHours: this.config.paperFundingIntervalHours,
                        ...
                        const routedClosed = await this.appendClosedWithStandardRouting({
                            ...
                            flowId,
                            envelope: null,
                            exitReason: open.closePendingReason || "stop_loss",
                            closeSource: "priority_stop_loss_gate_fill_confirmed",
                            currentRegime: input.marketMode.marketMode
                        });
                    ...
                    this.logger.warn("STOP_LOSS_CLOSE_PENDING_RETRY", { symbol: open.symbol, flowId, error: status.error });
                    remaining.push(posTrail);
                    continue;
                }
            }
            ...
            if (snap && m && closePrice != null) {
                if (typeof open.stopPrice === "number" && Number.isFinite(open.stopPrice)) {
                    ...
                    isSlTriggered = m.pnlPctNet <= (0, regime_exit_1.stopLossPctForRegime)(slRegime);
                }
                if (isSlTriggered) {
                    // EXCLUDE: symbolExternalManualBlocked, EXTERNAL_MANUAL_POSITION, OPERATOR_MANAGED
                    const isOperatorManaged = this.symbolExternalManualBlocked.has(symSideKey) ||
                        open.lifecycleState === "EXTERNAL_MANUAL_POSITION" ||
                        open.lifecycleState === "OPERATOR_MANAGED" ||
                        open.lifecycleState === "CLOSE_ONLY_MANAGED" ||
                        open.isOperatorManaged === true;
            ...
            if (open.isV2Authority && open.takeProfit1Px != null) {
                ...
                if (stage < 2 && !open.v2RangeTp2Triggered && tp2Hit) {
                    ...
                    const closedRowF = (0, paper_close_finalize_1.finalizePaperClosedRecord)({
                        open,
                        symbol: open.symbol,
                        closePrice,
                        closedAt,
                ...
                if (stage < 1 && !open.v2RangeTp1Triggered && tp1Hit) {
                    ...
                    const closedRowP = (0, paper_close_finalize_1.finalizePaperClosedRecord)({
                        open,
                        symbol: open.symbol,
                        closePrice,
                        closedAt,
            ...
            if (isSlTriggered || isTpTriggered || isTrailingTriggered) {
                ...
                const cr = isSlTriggered ? "stop_loss" : isTpTriggered ? "take_profit" : "trailing_stop";
                finalCloseReason = cr;
                ...
                const feeRate = this.config.paperTakerFeeRate;
                const intervalH = this.config.paperFundingIntervalHours;
                const toClosedLocal = (crLoc, metricsLoc, legMarginUsdLoc) => (0, paper_close_finalize_1.finalizePaperClosedRecord)({
                    open,
                    symbol: open.symbol,
                    closePrice,
                    closedAt,
            ...
            let v2TakeoverAction = "none";
            let v2TakeoverReason = null;
            let v2TakeoverDetail = null;
            ...
            const handleV2ExitAuthorityProof = (paperExitAction, paperExitReason) => {
                if (!v2ExitAuthority || paperExitProofHandled)
                    return;
                paperExitProofHandled = true;
                const v2PaperExitAgreement = (v2ExitAuthority.shouldExit === true && paperExitAction === "exit") ||
                    (v2ExitAuthority.shouldExit !== true && paperExitAction === "none");
                const highPriority = v2PaperExitAgreement === false ||
                ...
                if ((0, index_1.shouldEmitV2Proof)("V2_EXIT_AUTHORITY_PROOF", sk, proofKey, highPriority)) {
                ...
                if ((0, index_1.shouldEmitV2Proof)("V2_EXIT_EXECUTION_AUTHORITY_PROOF", sk, execProofKey, highPriority)) {
            ...
            const handleV2PartialAuthorityProof = (paperPartialAction, paperPartialReason) => {
                if (!v2PartialAuthority || paperPartialProofHandled)
                    return;
                paperPartialProofHandled = true;
                const v2PaperPartialAgreement = paperPartialAction === "superseded_by_exit" ||
                ...
                const highPriority = v2PaperPartialAgreement === false ||
                    (Array.isArray(v2PartialAuthority.trueInconsistencyReasons) && v2PartialAuthority.trueInconsistencyReasons.length > 0);
                if (!(0, index_1.shouldEmitV2Proof)("V2_PARTIAL_AUTHORITY_PROOF", sk, proofKey, highPriority))
                    return;
            ...
            const toClosed = (cr, metrics, legMarginUsd) => (0, paper_close_finalize_1.finalizePaperClosedRecord)({
                open,
                symbol: open.symbol,
                closePrice,
                closedAt,
            ...
            const { longUsd, shortUsd } = (0, range_engine_1.marginsForSymbol)(opens, symKey);
            ...
            const postPartialProtectActive = ((open.partialExitStage ?? 0) >= 1 &&
            ...
            if (rangeManagedPosition && !exitManagedByV2) {
                rangeState = (0, range_engine_1.evaluateRangeEngineForSymbol)({
                    symbol: symKey,
                    lastPrice: closePrice,
                ...
                if (rangeState) {
                    if (open.side === "long" && typeof snap.boxPos === "number" && Number.isFinite(snap.boxPos)) {
                        const raZone = (0, types_1.classifyRangeZone)(snap.boxPos);
                        if (raZone === "upper") {
                            this.logger.info("REGIME_EXIT_GUARD_PROOF", {
                                symbol: open.symbol,
                                side: open.side,
            ...
            if ((exitLane === "TREND" || regimeAtEntry === "TREND") && !exitManagedByV2) {
                trendState = (0, trend_engine_1.evaluateTrendEngineForSymbol)({
                    mark: closePrice,
                    entryPrice: open.entryPrice,
                    atr: snap.atr,
                    marketMode: input.marketMode,
            ...
            if (rangeManagedPosition && rangeState) {
                const liveZone = typeof snap.boxPos === "number" && Number.isFinite(snap.boxPos)
                    ? (0, types_1.classifyRangeZone)(snap.boxPos)
                    : "mid";
                const rangeReattackEligibleNow = open.rangeFirstProfitLocked !== true &&
                ...
                let st = (0, range_engine_1.evaluateRangeStructuralExit)({
                    lastPrice: closePrice,
                ...
                const trailStep = (0, range_engine_1.computeRangeProfitTrailStep)({
                    side: open.side,
                    closePrice,
                    boxUpper: rangeState.boxUpper,
                    boxLower: rangeState.boxLower,
                ...
                if (trailStep.trailExit) {
                    ...
                    const crTrail = "range_profit_trail";
                    const closedRowTrail = (0, paper_close_finalize_1.finalizePaperClosedRecord)({
                        open,
                        symbol: open.symbol,
                        closePrice,
                        closedAt,
                ...
                if (trailStep.deferBoxBreak && st.shouldExit && st.reason === "range_box_break") {
                    ...
                    const boxZoneNow = typeof snap.boxPos === "number" && Number.isFinite(snap.boxPos) ? (0, types_1.classifyRangeZone)(snap.boxPos) : "mid";
                    ...
                    const addOnAvailableForStateLoop = open.regimeAtEntry === "RANGE" &&
                        ...
                        !addOnUsed;
                    const profitLockWindowActive = open.rangeFirstProfitLocked !== true &&
                        (open.partialExitStage ?? 0) === 0 &&
                        m.pnlPctNet >= -0.0002;
                    const holdExtraMs = 120_000;
                    const isReattackUsed = open.rangeManagementState === "REATTACK_USED";
                    const preferHoldOverRebalance = (open.rangeManagementState ?? "INIT") !== "PROFIT_LOCKED" &&
                ...
                if (st.shouldExit && st.reason) {
                    ...
                    let cr = "range_box_break";
                    if (st.reason === "structural_regime_shift")
                        cr = "structural_regime_shift";
                    if (st.reason === "risk_exposure_breach")
                        cr = "regime_exit";
                    if (st.reason !== "risk_exposure_breach" &&
                    ...
                    const closedRow = st.reason === "risk_exposure_breach"
                        ? (0, paper_close_finalize_1.finalizePaperClosedRecord)({
                            open,
                            symbol: open.symbol,
                            closePrice,
                            closedAt,
            ...
            if (regimeAtEntry === "TREND" && trendState) {
                const plan = (0, trend_engine_1.planTrendSwitch)(trendState, open.side);
                if (plan.execute && plan.openSide && plan.closeSide) {
                    ...
                    let newSz = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(open.sizeUsd * input.riskExposure.switchSizeMultiplier * 100) / 100);
                    if (this.okxDemo) {
                    ...
                    newSz = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(open.sizeUsd * input.riskExposure.switchSizeMultiplier * 100) / 100);
                    const rev = {
                        openedAt: closedAt,
                        symbol: open.symbol,
                        side: plan.openSide,
            ...
            if (regimeAtEntry === "TREND" && !exitManagedByV2) {
                ...
                if (regimeNow !== "TREND" || !trendOkNow) {
                    ...
                    const cr = marketMode === "RANGE" || marketMode === "NO_TRADE" ? "regime_exit" : "trend_break_exit";
                    ...
                    await this.store.appendJsonlLine("reports/events.jsonl", {
                        ...
                        side: open.side,
                        regime: open.regimeAtEntry ?? null,
                        executor: (0, executor_normalize_1.executorForExitEventPayload)(open.executorAtEntry, open.regimeAtEntry),
                        reason: cr,
            ...
            let exitEval = { action: "hold" };
            if (!exitManagedByV2 && !partialManagedByV2) {
                exitEval =
                    exitLane === "RANGE"
                        ? (0, range_executor_1.rangeExecutorEvaluateExit)({
                            side: open.side,
                            pnlPctNet: m.pnlPctNet,
                            mark: closePrice,
                            boxPos: snap.boxPos,
                        ...
                        : (0, highway_exit_engine_1.highwayExitEngine)({
                            position: open,
                            aiScores: (0, ai_highway_filter_1.evaluateAiHighwayQuality)(snap.candles ?? [], open.symbol),
                            lastPrice: closePrice,
                            ema20: snap.ema20,
                            ema60: snap.ema60
                        });
            }
            const regimeExitSnap = (0, regime_exit_1.evaluateRegimeExitPolicy)({
                regime: slRegime,
                side: open.side,
                pnlPctNet: m.pnlPctNet,
                holdingMs: m.holdingMs,
            ...
            const partialPol = (0, live_partial_exit_policy_1.evaluatePartialExitPolicy)({
                mode: open.adaptiveModeAtEntry ?? this.lastAdaptiveMode.mode,
                direction: open.side,
                pnlPctNet: m.pnlPctNet,
            ...
            if (diagTs - lastExitPf >= 15_000) {
                ...
                this.logger.info("POSITION_EXIT_POLICY_PROOF", {
                    ...
                    stop_loss_pct_gate: (0, regime_exit_1.stopLossPctForRegime)(slRegime),
                    regime_exit_snapshot: regimeExitSnap,
                    executor_exit_eval_action: exitEval.action,
                    executor_exit_eval_reason: exitEval && typeof exitEval.reason === "string"
                        ? exitEval.reason
                        : null
                });
            ...
            if (v2TakeoverAction !== "none" && v2TakeoverAction !== "partial_close") {
            ...
            if (v2TakeoverAction === "partial_close" && v2PartialAuthority?.shouldPartial === true) {
            ...
            if (exitLane === "RANGE" &&
                ...
                const firstProfitLockEligible = m.pnlPctNet >= firstProfitLockThreshold &&
                    m.pnlUsdNet >= Math.max(0.01, open.sizeUsd * 0.00002);
                if (firstProfitLockEligible) {
                    const detail = (exitEval.detail ?? {});
                    exitEval = {
                        ...exitEval,
                        action: "partial_close",
            ...
            if (exitEval.action === "close") {
                ...
                if (cr === "regime_exit" || cr === "trend_break_exit" || cr === "candidate_lost" || cr === "trend_switch") {
                    const exDetail = (exitEval.detail ?? {});
            ...
            if (exitEval.action === "partial_close") {
                ...
                const partialDetail = (partial.detail ?? {});
                const adaptiveMode = open.adaptiveModeAtEntry ?? this.lastAdaptiveMode.mode;
                const rawRatio = partial.partialExitRatio;
                let ratio = typeof rawRatio === "number" && Number.isFinite(rawRatio) && rawRatio > 0
                    ? rawRatio
                    : (0, live_partial_exit_policy_1.defaultPartialExitRatioForStage)(adaptiveMode, open.partialExitStage ?? 0);
                ...
                if (newMargin < live_position_sizing_1.MIN_POSITION_SIZE_USD) {
                    this.logger.info("partial_exit_skipped", {
                        ...exitDetailBase(open, m),
                        reason: "remaining_below_min",
                        partial_ratio: ratio,
                        remaining_after: newMargin,
                        min_usd: live_position_sizing_1.MIN_POSITION_SIZE_USD
                    });
                }
                else {
                    const stage = (open.partialExitStage ?? 0) + 1;
                    const pReason = stage === 1 ? "partial_exit_1" : "partial_exit_2";
                    ...
                    open = {
                        ...
                        trailingExtremePrice: partial.trailingExtreme,
            ...
            posTrail = { ...posTrail, trailingExtremePrice: exitEval.trailingExtreme };
            if (rangeManagedPosition) {
                const raZone = typeof snap.boxPos === "number" && Number.isFinite(snap.boxPos)
                    ? (0, types_1.classifyRangeZone)(snap.boxPos)
                    : "mid";
                const aligned = (open.side === "long" && raZone === "lower") || (open.side === "short" && raZone === "upper");
                this.logger.info("RANGE_CLOSE_ALIGNMENT_PROOF", {
                    symbol: open.symbol,
                    side: open.side,
                    range_zone_detected: raZone,
                ...
                if ((open.side === "long" && raZone === "upper") || (open.side === "short" && raZone === "lower")) {
                    ...
                    const closedRow = (0, paper_close_finalize_1.finalizePaperClosedRecord)({
                        ...
                        fundingIntervalHours: intervalH,
                        strategyVersion: inheritedStrategyVersion,
                        closeReasonLabelOverride: open.side === "long"
                    ...
                    const mappedType = exitEventJsonlType(cr);
                    this.terminalExitConsumedByFlow.add(flowId);
                    const isRegimeRelatedCode = mappedType === "EXIT_REGIME" ||
                ...
                const zk = typeof snap.boxPos === "number" && Number.isFinite(snap.boxPos) ? (0, types_1.classifyRangeZone)(snap.boxPos) : "mid";
                const keep = (open.side === "long" &&
            ...
            const stagedOrScaled = (open.entryStage ?? 1) >= 2 ||
                (typeof open.initialSizeUsd === "number" &&
                    open.initialSizeUsd > 0 &&
                    open.sizeUsd > open.initialSizeUsd * 1.05);
            const opposingSignal = (open.side === "long" && snap.signal === "paper_short_candidate") ||
                (open.side === "short" && snap.signal === "paper_long_candidate");
            const zoneMismatch = typeof snap.boxPos === "number" &&
                ((open.side === "long" && (0, types_1.classifyRangeZone)(snap.boxPos) !== "lower") ||
                    (open.side === "short" && (0, types_1.classifyRangeZone)(snap.boxPos) !== "upper"));
            ...
            const trendStructureStillValid = open.regimeAtEntry === "TREND"
                ? (snap.trendOk === true &&
                    Math.abs(snap.emaGap ?? 0) >= ENTRY_EVIDENCE_TREND_EMA_GAP_MIN &&
                    (snap.trendWeaknessScore ?? 1) <= ENTRY_EVIDENCE_TREND_WEAKNESS_MAX)
                : false;
            const rangeStructureStillValid = open.regimeAtEntry === "RANGE"
                ? (typeof snap.boxPos === "number" &&
                    ((open.side === "short" && (0, types_1.classifyRangeZone)(snap.boxPos) === "upper") ||
                        (open.side === "long" && (0, types_1.classifyRangeZone)(snap.boxPos) === "lower")))
                : false;
            const entryEvidenceStillValid = entryEvidence == null
            ...
            this.logger.info("FINAL_CLOSE_CONFIRMATION_PROOF", {
                ...
                confirmedExitType,
                confirmedCloseSource,
                will_be_removed_from_ledger: finalCloseReason !== "none"
            });
            if (finalCloseReason === "none") {
        ...
        const rescuedForUnauthorizedLedgerPrune = [];
        for (const o of opens) {
            const fid = `${o.symbol}:${o.side}:${o.openedAt}`;
            if (remainingIdsBeforeFinalGate.has(fid))
                continue;
            const ledgerPruneAllowed = openLedgerPruneAuthorizedFlowIds.has(fid) || this.terminalExitConsumedByFlow.has(fid);
            if (ledgerPruneAllowed)
                continue;
            remaining.push(o);
        ...
        const openPositionsChanged = remaining.length !== opens.length || remaining.some((r, i) => r !== opens[i]);
        const shouldSaveOpenLedger = crashPositionsModified || openLedgerPruned || openPositionsChanged;
        ...
        if (shouldSaveOpenLedger) {
            ...
            const stopBackfillVerifiedPresentFlows = [];
            const stopBackfillSkippedRemovedFlows = [];
            const stopBackfillStillMissing = [];
     ...
     */
    buildAuthorityAdaptiveBridge(authority, legacyAdaptive, forceEnter = false) {
        if (!forceEnter && authority.decision !== "ENTER")
            return null;
        const side = authority.side;
        if (side !== "long" && side !== "short")
            return null;
        const stageMarginKrw = (authority.stageMarginKrw ?? 0) > 0
            ? (authority.stageMarginKrw ?? 0)
            : forceEnter
                ? Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD * PAPER_LEDGER_KRW_NOTIONAL_PER_USD, Math.round((0, live_position_sizing_1.computePaperSizingAnchorUsd)(this.config) * 0.35 * PAPER_LEDGER_KRW_NOTIONAL_PER_USD * 100) / 100)
                : 0;
        ...
        const leverageMultiplier = this.config.leverage > 0 ? Math.max(0.5, appliedLeverage / this.config.leverage) : 1.0;
    ...
    evaluateRangeRecheckPromotion(input) {
        ...
        const waitRecheckCandidate = reasonRaw === "WAIT_RECHECK" ||
        ...
        const serverControlOpen = input.serverTradeEnabled &&
            !input.closeOnlyMode &&
            !input.killSwitch &&
        ...
        const commonEligible = isV2Owner &&
            v2EngineOwner &&
            decisionHold &&
        ...
        const regimeAtDecision = authorityRegimeUpper === "RANGE" || input.activeEngine === "RANGE" ? "RANGE" : "TREND";
        ...
        const zone = typeof input.boxPos === "number" && Number.isFinite(input.boxPos)
            ? (0, types_1.classifyRangeZone)(input.boxPos)
            : "mid";
        ...
        if (regimeAtDecision === "RANGE") {
            if (zone === "mid")
                return { promote: false, reason: "range_mid_wait_not_promotable", ticks };
            const zoneAligned = (input.side === "short" && zone === "upper") ||
                (input.side === "long" && zone === "lower");
            if (!zoneAligned)
                return { promote: false, reason: "range_side_zone_mismatch", ticks };
            const rangeConfidence = Math.max(0, input.rangeConfidence ?? 0);
            const rangeStructureQualified = rangeConfidence >= 0.65 &&
        ...
        const trendDirectionalAligned = shock === "NONE" ||
    ...
    toEntryQualityVectorFromSnapshot(snapshot, side) {
    ...
    async buildEntryQualitySamplesFromHistory(history) {
        const profit = [];
        const loss = [];
        for (const row of history.slice(-120)) {
            ...
            try {
                ...
                const snap = snaps.find((s) => String(s.symbol ?? "") === symbol);
                if (!snap)
                    continue;
                const vector = this.toEntryQualityVectorFromSnapshot({
                    qualityScore: Number(snap.qualityScore ?? 0),
                    atr: Number(snap.atr ?? 0),
                    lastPrice: Number(snap.lastPrice ?? 0),
                    emaGap: Number(snap.emaGap ?? 0),
                    volumeRatioProxy: Number(snap.volumeRatioProxy ?? 0)
                }, side);
                const sample = {
    ...
    toEntryQualityVectorFromEvent(ev, side) {
        const qualityScore = this.toNumberOrNull(ev.entry_quality_score) ??
            this.toNumberOrNull(ev.quality_score) ??
            this.toNumberOrNull(ev.qualityScore) ??
            50;
        const atrPct = this.toNumberOrNull(ev.atr_pct) ??
            this.toNumberOrNull(ev.atrPct) ??
            0;
        const emaGapAbs = Math.abs(this.toNumberOrNull(ev.ema_gap) ??
            this.toNumberOrNull(ev.emaGap) ??
            this.toNumberOrNull(ev.entry_ema_gap) ??
            0);
        const volumeRatioProxy = this.toNumberOrNull(ev.range_confidence) ??
    ...
    eventTypeFromEvent(ev) {
        if (ev.type != null)
            return { value: String(ev.type), source: "type" };
        if (ev.event != null)
            return { value: String(ev.event), source: "event" };
        if (ev.name != null)
            return { value: String(ev.name), source: "name" };
        const reasonLike = ev.eventType ?? ev.exitType ?? ev.closeType ?? ev.reason ?? ev.closeReason ?? ev.exitReason;
        if (reasonLike != null)
            return { value: String(reasonLike), source: "reason" };
    ...
    symbolFromEvent(ev) {
        const direct = (typeof ev.symbol === "string" && ev.symbol.trim().length > 0
            ? ev.symbol
    ...
    async buildEntryQualitySamplesFromEvents() {
        const profit = [];
        const loss = [];
        const contaminated = [];
        const diagnostics = {
        ...
        for (const line of lines) {
            ...
            if (typeSource === "type")
                diagnostics.event_type_field_hits_type += 1;
            else if (typeSource === "event")
                diagnostics.event_type_field_hits_event += 1;
            else if (typeSource === "name")
                diagnostics.event_type_field_hits_name += 1;
            else if (typeSource === "reason")
                diagnostics.event_type_field_hits_reason += 1;
            if (type === "ENTRY_OPENED" || type.includes("ENTRY_OPENED"))
                diagnostics.entry_opened_events += 1;
            const hasExitReason = ev.closeReason != null || ev.exitReason != null || ev.exitType != null || ev.closeType != null;
            const isExitCandidate = type.startsWith("EXIT_") || type.includes("EXIT_") || hasExitReason;
            if (!isExitCandidate)
                continue;
            diagnostics.exit_candidate_events += 1;
            if (type.startsWith("EXIT_") || type.includes("EXIT_"))
            ...
            const pnlUsd = this.toNumberOrNull(ev.realizedPnlUsd) ??
            ...
            const pnlPct = this.toNumberOrNull(ev.realizedPnlPct) ??
    ...
    async refreshEntryQualitySamples(history) {
        ...
        this.lastEntryQualitySampleSourceBreakdown = {
            ...
            total_sample_count: this.lastEntryQualitySamples.profit.length +
        ...
        const regimeExitRate = (rows) => {
            if (rows.length === 0)
                return 0;
            let regimeExits = 0;
            for (const r of rows) {
                const reason = String(r.closeReason ?? "").toLowerCase();
                if (reason.includes("regime"))
                    regimeExits += 1;
            }
    ...
    evaluateEntryQualityGate(snapshot, side) {
        ...
        const nearest = (samples) => {
            ...
            for (const s of samples)
                best = Math.min(best, this.qualityVectorDistance(current, s.vector));
            return best;
        };
    ...
    buildEntryQualityProfilesForV2() {
        const avg = (rows) => {
            if (rows.length === 0)
                return { qualityScoreAvg: 0, emaGapAvg: 0, atrPctAvg: 0, volumeRatioAvg: 0, count: 0 };
            let q = 0;
            let e = 0;
            let a = 0;
            let v = 0;
     ...
     * ?ъ???蹂댁쑀 ?쒖뿉留??됯?쨌?ㅽ뻾?쒕떎.
     */
    async processPaperSymbolEntries(input) {
        ...
        if (this.v2DeferredEntryQueue.size > 0) {
            ...
            for (const [symKey, deferred] of this.v2DeferredEntryQueue) {
                ...
                if (!freshTickOk)
                    invalidReason = "FRESH_TICK_NOT_OK";
                else if (!readinessOk)
                    invalidReason = "READINESS_NOT_OK";
                else if (positionConflict)
                    invalidReason = positionConflictReason ?? "SYMBOL_POSITION_CONFLICT";
                else if (!stopPriceValid)
                    invalidReason = lastPrice > 0 && !stopDirectionValid ? "STOP_PRICE_DIRECTION_INVALID" : "STOP_PRICE_INVALID";
                else if (!serverTradeEnabledOk)
                    invalidReason = "SERVER_TRADE_DISABLED";
                else if (!signedExecutionReadyOk)
                    invalidReason = "SIGNED_EXECUTION_NOT_READY";
                else if (!paperExecutionReadyOk)
                    invalidReason = "PAPER_EXECUTION_NOT_READY";
                else if (!closeOnlyOk)
                    invalidReason = "CLOSE_ONLY_MODE";
                else if (!killSwitchOk)
                    invalidReason = "KILL_SWITCH_ACTIVE";
                else if (!reconcileSafeOk)
                    invalidReason = "RECONCILE_SAFE_MODE";
                else if (!dailyLossGuardOk)
                    invalidReason = "DAILY_LOSS_GUARD";
                else if (!riskModeOk)
                    invalidReason = "RISK_MODE_HALT";
        ...
        input.decisionBySymbol.forEach((envelope, symKey) => {
            ...
            if (authority.source === "v2") {
                const v2QueueAllowed = envelope.v2_execution_envelope?.authoritySource === "v2_execution_envelope" &&
                ...
                if (!v2QueueAllowed) {
                    if (authority.decision === "ENTER" || authority.originalDecision === "ENTER") {
                        let blockReason = "v2_hard_safety_block";
                        if (authority.decision !== "ENTER")
                            blockReason = "authority_decision_not_enter";
                        else if (authority.hardBlockPresent === true)
                            blockReason = "hard_block_present";
                        else if (authority.nonBypassableHardBlockPresent === true)
                            blockReason = "non_bypassable_hard_block";
                        else if (!executionSnapshot.signedReady)
                            blockReason = "signed_not_ready";
                        else if (!executionSnapshot.tradeEnabled)
                            blockReason = "trade_disabled";
                        else if (executionSnapshot.dailyLossGuard)
                            blockReason = "daily_loss_guard";
                        this.logger.info("V2_ENTER_CANDIDATE_ENQUEUE_PROOF", {
                            symbol: symKey,
                            authority_decision: authority.decision,
                            original_decision: authority.originalDecision ?? null,
                            authority_side: authority.side,
                            original_side: authority.originalSide ?? null,
                            stage_margin_krw: authority.stageMarginKrw ?? 0,
                            original_stage_margin_krw: authority.originalStageMarginKrw ?? 0,
                            ...
                            decision_id: authority.decision_id ?? null
                        });
                    }
                    return;
                }
                ...
                const sig = authority.side === "long" ? "paper_long_candidate" : "paper_short_candidate";
            ...
            if (authority.source === "v1") {
                ...
                if (effectiveAdaptiveResult == null)
                    return;
                if (!base)
                    return;
                const sig = authority.side === "long" ? "paper_long_candidate" : "paper_short_candidate";
        ...
        const shouldBypassPathCheckForV2 = hasV2Enqueued === true &&
        ...
        this.logger.info("ENTRY_QUEUE_BUILD_COMPLETE_PROOF", {
            ...
            readiness_barrier_active: input.readinessBarrierActive,
            candidate_run_path: input.candidateRunPath,
            will_enter_consume_loop: entryQueue.length > 0 &&
        ...
        const freshTickHardBlock = input.readinessBarrierActive || this.freshTickRequiredAfterReadiness;
            ...
            if (v2BypassReady) {
                ...
                for (const q of entryQueue) {
                    if (q.authoritySource === "v2") {
                        ...
                        const authSide = (authority?.side ??
                            authority?.runtime_authority_side ??
                            authority?.v2_side ??
                            authority?.final_side ??
                            null);
                        const resolvedSide = authSide === "long" || authSide === "short"
                            ? authSide
                            : q.signal === "paper_short_candidate" ? "short" : "long";
                        const stopPx = authority?.stopPrice ?? null;
                        ...
                        this.v2DeferredEntryQueue.set(symStr, {
                            ...
                            decisionId: authority?.decision_id ?? null,
                        ...
                        // V1 후보는 오염 샘플로만 기록
                        const v1Side = q.signal === "paper_short_candidate" ? "short" : "long";
        ...
        const opens = opensRaw.map((r) => {
            ...
            if (stageN.changed) {
                current = stageN.normalized;
                changed = true;
            }
            const rangeN = normalizeRangeManagementState(current);
            if (rangeN.changed) {
                current = rangeN.normalized;
                changed = true;
            }
            if (changed)
                openPositionsChanged = true;
        ...
        const activePendingEntryOrders = [];
        ...
        for (const pending of pendingEntryOrders) {
            ...
            if (liveOrder) {
                ...
                if (orderState === "filled") {
                    ...
                    let actualPos = null;
                    if (this.lastLivePositionsPayload && Array.isArray(this.lastLivePositionsPayload)) {
                        actualPos = this.lastLivePositionsPayload.find((p) => p.instId === pending.instId && String(p.posSide).toLowerCase() === pending.side);
                        if (!actualPos) {
                            actualPos = this.lastLivePositionsPayload.find((p) => {
                                if (p.instId !== pending.instId)
                                    return false;
                    ...
                    if (!isCommittedEntryStopPrice(record.stopPrice) && isCommittedEntryStopPrice(pending.stopPrice)) {
                        record.stopPrice = pending.stopPrice;
                    }
                    ...
                    try {
                        await this.store.appendJsonlLine("reports/events.jsonl", buildEntryOpenedEventPayload(pending.symbol, pending.authoritySnapshot, record));
                    }
                    catch (e) { }
                    pendingRegistryModified = true;
                }
        ...
        for (const first of entryQueue) {
            ...
            const gateTimestamps = [readinessChangedAt, executionSnapshot.serverTradeEnabledTrueAt].filter((x) => typeof x === "number" && Number.isFinite(x));
            ...
            if (effectiveReadinessGateTs != null) {
                ...
                if ((envelope.signalFetchedAt ?? first.fetchedAt) < g)
                    staleReasons.push("signal_fetched_before_readiness_or_trade_enable_gate");
                if (envelope.authorityEvaluatedAt < g)
                    staleReasons.push("authority_evaluated_before_readiness_or_trade_enable_gate");
                if (envelope.executorDecisionEvaluatedAt < g)
                    staleReasons.push("executor_decision_before_readiness_or_trade_enable_gate");
                if (first.fetchedAt < g)
                    staleReasons.push("snapshot_fetched_before_readiness_or_trade_enable_gate");
                if (envelope.decisionCycleId < executionSnapshot.runCycleId)
                    staleReasons.push("order_basis_previous_cycle");
            }
            const v2BypassConditionsMet = envelope.selector?.adopted_result.engine === "V2" &&
                authority.decision === "ENTER" &&
                (authority.side === "long" || authority.side === "short") &&
                (authority.stageMarginKrw ?? 0) > 0 &&
                executionSnapshot.paperReady === true &&
            ...
            const exDetailForRecheck = (res.executorDecision?.detail ?? {});
            ...
            const recheckPromotion = authority.side === "long" || authority.side === "short"
                ? this.evaluateRangeRecheckPromotion({
                    ...
                    v2BlockReason,
                    v2Signal,
                    relaxedRangeEntry: exDetailForRecheck.relaxedRangeEntry === true ||
            ...
            const authorityDecisionForExecution = (authority.source === "v2" && adoptedEngine === "V2")
                ? authority.decision
            ...
            if (recheckPromotion.promote) {
                ...
                this.logger.info("V2_WAIT_RECHECK_PROMOTED_TO_ENTER", {
                    ...
                    zone: typeof first.boxPos === "number" && Number.isFinite(first.boxPos)
                        ? (0, types_1.classifyRangeZone)(first.boxPos)
                        : "mid",
            ...
            const NON_BYPASSABLE_HARD_BLOCKS = new Set([
            ...
            const hasOriginalEnterIntent = (authority.originalDecision === "ENTER" || authority.decision === "ENTER") &&
                (authority.originalSide === "long" || authority.originalSide === "short" ||
                    authority.side === "long" || authority.side === "short") &&
                (authority.originalStageMarginKrw ?? authority.stageMarginKrw ?? 0) > 0;
            const nonBypassableActive = authority.nonBypassableHardBlockPresent === true ||
                (authority.hardBlockReason != null && NON_BYPASSABLE_HARD_BLOCKS.has(authority.hardBlockReason));
            const safetyGatesOk = input.serverTradeEnabled === true &&
            ...
            if (isV2Source && hasOriginalEnterIntent && !nonBypassableActive && safetyGatesOk &&
                ...
                rehydratedSide = (authority.originalSide ?? authority.side);
            ...
            const effectiveSideForExecution = rehydrationApplied ? rehydratedSide : authority.side;
            const effectiveAdaptiveResult = this.buildAuthorityAdaptiveBridge({ ...authority, decision: effectiveAuthorityDecision, side: effectiveSideForExecution }, res.adaptiveResult, effectiveAuthorityDecision === "ENTER" && authority.decision !== "ENTER");
            if (effectiveAdaptiveResult == null)
                continue;
            ...
            if (this.lastEffectiveLane === "RANGE") {
                ...
                const qz = typeof first.boxPos === "number" ? (0, types_1.classifyRangeZone)(first.boxPos) : null;
            ...
            const intentSide = authority.side;
            let v2CommittedRiskPlan = null;
            ...
            const absHardBlockPresent = authority.hardBlockPresent === true || envelope.hard_block_present === true;
            /** ?좉퇋 吏꾩엯(existingIdx<0) ?꾩슜: ?ㅼ??쇱씤쨌?좊뱶??寃쎈줈?먯꽌???덇굅??RANGE no-reversal 臾댁떆瑜??곸슜?섏? ?딅뒗?? */
            const v2EnterSignedPaperReadyHandoff = authority.source === "v2" &&
                adoptedEngine === "V2" &&
                authorityDecisionForExecution === "ENTER" &&
                !absHardBlockPresent &&
                executionSnapshot.paperReady === true &&
                executionSnapshot.signedReady === true;
            const legacyRangeRejectIgnoredForV2 = v2EnterSignedPaperReadyHandoff &&
                existingIdx < 0 &&
            ...
            if (res.decision.reject_reason === "ORDER_BUILD_FAIL" && res.executorDecision?.entry_allowed) {
                const ob = orderBuildFailureStructuredPayload(first, res, entryStage, (this.lastEffectiveLane === "IDLE" ? "NO_TRADE" : this.lastEffectiveLane));
            ...
            const effectiveMarketSubtype = authority.marketSubtype != null
            ...
            const stage1ExecutionEngine = stage1ExecutorLabel === "RANGE" || stage1ExecutorLabel === "TREND"
                ? stage1ExecutorLabel
                : activeEngine;
            ...
            const effectiveRegimeForAi = (this.lastEffectiveLane === "IDLE" ? "NO_TRADE" : this.lastEffectiveLane);
            const aiIn = (0, entry_approval_1.aiInputFromDecision)({
                decision,
                executorDirection: authority.side,
            ...
            let aiOutput = null;
            if (aiIn) {
                aiOutput = (0, entry_approval_1.aiApproveEntry)(aiIn);
            ...
            const sideAllowedByGuard = authority.side === "long" ? allowLongGuard :
            ...
            const crashLockExpiredOrSoon = crashLockUntil === 0 ||
                nowForCrash >= crashLockUntil ||
                (nowForCrash + CRASH_GUARD_REGIME_AWARE_RELEASE_MS) >= crashLockUntil;
            const crashGuardRegimeAwareReleased = (crashState === "CRASH_EXIT" || crashState === "CRASH_LOCK") &&
                activeEngine === "RANGE" &&
                crashLockExpiredOrSoon;
            // CRASH_REDUCE ?????댁긽 濡?吏꾩엯 ?꾨㈃ 李⑤떒?섏? ?딆쓬 (size 異뺤냼 ?덉씠?대줈 ?€泥?
            const crashEntryGuardApplies = authority.side === "long" &&
                (crashState === "CRASH_EXIT" || crashState === "CRASH_LOCK") &&
                !crashGuardRegimeAwareReleased;
            const zone = typeof first.boxPos === "number" && Number.isFinite(first.boxPos) ? (0, types_1.classifyRangeZone)(first.boxPos) : null;
            const exDetail = (res.executorDecision?.detail ?? null);
            const bypassReason = typeof exDetail?.crash_lock_bypass_reason === "string" && exDetail.crash_lock_bypass_reason.length > 0
                ...
                : typeof exDetail?.bypass_reason === "string" && exDetail.bypass_reason.length > 0
                    ...
                    : null;
            const overrideReason = typeof exDetail?.override_reason === "string" && exDetail.override_reason.length > 0
                ? exDetail.override_reason
                : bypassReason;
            const preRiskCrashState = typeof exDetail?.pre_risk_crash_state === "string" && exDetail.pre_risk_crash_state.length > 0
                ? exDetail.pre_risk_crash_state
                : crashState;
            const reversalConfirmedFromLegacyExecutor = res.executorDecision?.entry_allowed === true &&
                (res.executorDecision?.blocked_reason == null ||
                    !String(res.executorDecision?.blocked_reason).toLowerCase().includes("reversal_confirm"));
            const reversalConfirmed = legacyRangeRejectIgnoredForV2 || reversalConfirmedFromLegacyExecutor;
            const relaxedRangeEntryForCrashBypass = exDetail?.relaxedRangeEntry === true ||
                first.rangeSignalKeptByRelax === true;
            const rangeZoneAlignedForCrashBypass = (authority.side === "long" && zone === "lower") ||
                (authority.side === "short" && zone === "upper");
            const rangeContextForCrashBypass = activeEngine === "RANGE" || String(authority.regime ?? "").toUpperCase() === "RANGE";
            const serverControlAllowsEntry = executionSnapshot.tradeEnabled &&
            ...
            const ngeStage0UpperLongBlock = authority.side === "long" &&
                zone === "upper" &&
            ...
            let oppositeLegBlockedReason = null;
            if (otherLeg) {
            ...
            if (authority.source !== "v2") {
            ...
            else if (first.candidateStrength === "weak" &&
                ...
                if (typeof first.boxPos !== "number" || !Number.isFinite(first.boxPos)) {
                    ...
                    const rz = (0, types_1.classifyRangeZone)(first.boxPos);
                ...
                const trendEvidenceOk = first.trendOk === true &&
            ...
            if (finalBlockedReason === "CRASH_ENTRY_GUARD_BLOCK") {
                ...
                const crashBypassEligible = rangeContextForCrashBypass &&
                    authorityDecisionForExecution === "ENTER" &&
            ...
            if (finalBlockedReason === null) {
                const d = adaptive.detail;
                const adaptiveConfTierRaw = typeof d?.confidence_tier === "string" ? String(d.confidence_tier).toLowerCase() : "";
                const adaptiveConfScore = typeof d?.confidence_score === "number" && Number.isFinite(d.confidence_score) ? d.confidence_score : null;
                const tierAggressive = adaptiveConfTierRaw === "top" || adaptiveConfTierRaw === "high";
                const evidenceLow = entryEvidenceScore < 60 ||
            ...
            const softBlockWarnings = [];
            const v2AuthorityCandidate = authority.source === "v2" &&
                adoptedEngine === "V2";
            ...
            if (v2AuthorityCandidate) {
                let systemHardBlock = null;
                if (!localSnapshot.tradeEnabled)
                    systemHardBlock = "SERVER_TRADE_DISABLED";
                else if (localSnapshot.killSwitch)
                    systemHardBlock = "KILL_SWITCH";
                else if (localSnapshot.closeOnly)
                    systemHardBlock = "CLOSE_ONLY_MODE";
                else if (localSnapshot.reconcileSafe)
                    systemHardBlock = "RECONCILE_SAFE_MODE";
                else if (!localSnapshot.paperReady)
                    systemHardBlock = "PAPER_EXECUTION_NOT_READY";
                else if (!localSnapshot.signedReady)
                    systemHardBlock = "SIGNED_EXECUTION_NOT_READY";
                else if (localSnapshot.riskModeHalt)
                    systemHardBlock = "RISK_MODE_HALT";
                else if (localSnapshot.dailyLossGuard)
                    systemHardBlock = "DAILY_LOSS_GUARD";
                else if (localSnapshot.freshTickBlocked)
                    systemHardBlock = "FRESH_TICK_EXECUTION_BLOCKED";
            ...
            const authoritySideLower = (0, reconciler_1.normalizePositionSideLower)(authority.side);
            ...
            const mutexEval = mutexEvaluated
                ? this.positions.evaluateSymbolPositionMutex(sym, authoritySideLower, next, existingIdx >= 0, authority.addOnAllowed === true)
                : { blocked: false, blockReason: null, sameSymbolOpenCount: 0, existingSides: [], existingPositionIds: [] };
            if (v2AuthorityCandidate) {
            ...
            let minReqNotional = live_position_sizing_1.MIN_POSITION_SIZE_USD;
            let minOrderUnderflow = false;
            let minOrderBlockReason = null;
            if (authority.source === "v2") {
                ...
                if (!inst || currentPrice <= 0) {
                    ...
                    if ((authority.stageMarginKrw ?? 0) > 0 && computedNotionalUsdt < minReqNotional)
                        minOrderUnderflow = true;
                }
            }
            else if ((authority.stageMarginKrw ?? 0) > 0 && marginUsdt < live_position_sizing_1.MIN_POSITION_SIZE_USD) {
                minOrderUnderflow = true;
            }
            ...
            const finalEntryAuthorization = authorityDecisionForExecution === "ENTER" &&
            ...
            const levScaled = Math.max(1, Math.round(this.config.leverage * adaptive.leverageMultiplier * 100) / 100);
            const liveRegimeForEntryIdentity = (this.lastEffectiveLane === "IDLE" ? "NO_TRADE" : this.lastEffectiveLane);
            ...
            if (authority.source !== "v2") {
                entrySizeUsd = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(entrySizeUsd * entryQualitySizeMultiplier * 100) / 100);
            }
            else {
                entrySizeUsd = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(entrySizeUsd * 100) / 100);
            }
            ...
            if (authority.source === "v2" && authorityDecisionForExecution === "ENTER") {
                ...
                if (auditFail) {
                    const auditFailReason = !auditSideOk ? "INVALID_SIDE" :
            ...
            if (v2AuthorityCandidate && authorityDecisionForExecution === "ENTER") {
                ...
                const marginUsdt = (authority.stageMarginKrw ?? 0) / PAPER_LEDGER_KRW_NOTIONAL_PER_USD;
                const authorityNotionalUsdt = typeof authority.exposureNotionalKrw === "number" && authority.exposureNotionalKrw > 0
                    ? authority.exposureNotionalKrw / PAPER_LEDGER_KRW_NOTIONAL_PER_USD
                    : marginUsdt * (authority.appliedLeverage ?? 10);
                const liveMaxOrderNotionalUsdt = typeof this.config.okxLiveMaxOrderNotionalUsdt === "number" && this.config.okxLiveMaxOrderNotionalUsdt > 0
                ...
                const mPreV2 = (0, range_engine_1.marginsForSymbol)(next, symS);
                if (riskE) {
                    ...
                    if (currentUsd + v2EntrySizeUsd > cap) {
                        ...
                        if (v2EntrySizeUsd < live_position_sizing_1.MIN_POSITION_SIZE_USD) {
                ...
                // 5. Execution Handoff
                const openTraceId = (0, node_crypto_1.randomUUID)();
                ...
                if (this.okxDemo && signedMode === "enabled") {
                    ...
                    const intendedSide = authority.side;
                    let macroBias = "DAILY_NEUTRAL_RANGE";
                    ...
                    if (macroUpSubtypes.has(currentSubtype)) {
                        ...
                        if (intendedSide === "short") {
                            macroCounterTrend = true;
                            macroSizeMultiplier = 0.6;
                        }
                    }
                    ...
                    if (macroCounterTrend) {
                        ...
                        const isStrongCounterTrendSignal = currentSubtype === "BREAKDOWN_RETEST_FAILED" ||
                        ...
                        v2EntrySizeUsd = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(v2EntrySizeUsd * macroSizeMultiplier * 100) / 100);
                    ...
                    const initialTpForRecord = v2CommittedRiskPlan.initial_tp_price != null ? v2CommittedRiskPlan.initial_tp_price : undefined;
                    ...
                    if (isRecovery && recoveryInfo) {
                        ...
                        const conditionsPassed = !hasPosition &&
                            !!stopPrice &&
                            signedReady &&
                            !protectionBlocked &&
                    ...
                    this.logger.info("V2_ENTER_ORDER_PATH_PROOF", {
                        ...
                        min_order_notional_usdt: live_position_sizing_1.MIN_POSITION_SIZE_USD,
                        notional_ok: v2EntrySizeUsd >= live_position_sizing_1.MIN_POSITION_SIZE_USD,
                    ...
                    if (submit.ok) {
                        const slReg = (0, position_ops_monitor_1.regimeForSl)(authority.regime);
                        const isPending = submit.fillConfirmed !== true;
                        const fillPxNum = submit.fillPx != null && String(submit.fillPx).length > 0 ? Number(submit.fillPx) : NaN;
                        const entryPxOpen = Number.isFinite(fillPxNum) && fillPxNum > 0 ? fillPxNum : first.lastPrice;
                        const baseQtyOpen = typeof submit.baseQty === "number" && Number.isFinite(submit.baseQty) && submit.baseQty > 0
                            ? submit.baseQty
                        ...
                        const record = {
                            ...
                            notionalUsd: typeof submit.notionalUsd === "number" && Number.isFinite(submit.notionalUsd)
                                ? submit.notionalUsd
                                : v2EntrySizeUsd,
                            avgPx: typeof submit.avgPx === "number" && Number.isFinite(submit.avgPx)
                                ? submit.avgPx
                                : entryPxOpen,
                        ...
                        if (isPending) {
                            ...
                            const pendingReg = {
                                ...
                                instId: (0, okx_demo_1.toOkxSwapInstId)(sym),
                                authority_source: authority.source,
                        ...
                        const instIdTarget = (0, okx_demo_1.toOkxSwapInstId)(sym);
                        let actualPosReconcile = null;
                        let reconcileMatchMethod = "none";
                        if (this.lastLivePositionsPayload && Array.isArray(this.lastLivePositionsPayload)) {
                            // 1李? posSide 吏곸젒 留ㅼ묶
                            actualPosReconcile = this.lastLivePositionsPayload.find((p) => p.instId === instIdTarget &&
                            ...
                            if (actualPosReconcile) {
                                ...
                                const samePosInstId = this.lastLivePositionsPayload.filter((p) => p.instId === instIdTarget);
            ...
            if (existingIdx >= 0) {
                ...
                const pnlPctPreScale = intentSide === "long"
            ...
            if (!entryKeyOk)
                continue;
            await this.store.appendJsonlLine("reports/events.jsonl", buildEntryAllowedEventPayload(sym, (this.lastEffectiveLane === "IDLE" ? "NO_TRADE" : this.lastEffectiveLane), decision, authority));
            const sourceSignal = first.signal;
            let confScore = typeof adaptive.detail.confidence_score === "number" && Number.isFinite(adaptive.detail.confidence_score)
                ? adaptive.detail.confidence_score
                : undefined;
            let confTier = typeof adaptive.detail.confidence_tier === "string" ? adaptive.detail.confidence_tier : undefined;
            let sizeMult = typeof adaptive.detail.size_multiplier === "number" && Number.isFinite(adaptive.detail.size_multiplier)
                ? adaptive.detail.size_multiplier
                : undefined;
            const evidenceClamdown = authority.entryQualityGrade === "B" ||
                first.candidateStrength === "weak" ||
            ...
            const isRangeCampaignNewEntry = entryIdentity.effectiveExecutorAtEntry === "RANGE" && entryIdentity.effectiveRegimeAtEntry === "RANGE";
            ...
            if (authority.source !== "v2") {
                if (!isRangeCampaignNewEntry && riskE) {
                    entrySizeUsd = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(adaptive.sizeUsd * riskE.sizeMultiplier * 100) / 100);
                }
                const symS = String(first.symbol);
                if (!isRangeCampaignNewEntry && this.lastMarketMode?.routing.activeEngine === "TREND") {
                    ...
                    entrySizeUsd = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(entrySizeUsd * (1 + Math.min(4, pyr) * 0.07) * 100) / 100);
                }
                if (!isRangeCampaignNewEntry && this.lastMarketMode?.routing.activeEngine === "RANGE") {
                    ...
                    if (rSt) {
                        const cycleM = (0, range_engine_1.rangeCycleSizePolicy)(rSt.rangeCycleCount, rSt.hedgeBalance);
                        const legM = (0, range_engine_1.rangeLadderLegMultiplier)(rSt.rangeLadderLevel, rSt.hedgeBalance);
                        const recM = (0, range_engine_1.rangeAccumulationRecoveryMultiplier)(rSt.hedgeBalance, adaptive.direction, rSt.rangeCycleCount);
                        entrySizeUsd = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(entrySizeUsd * cycleM * legM * recM * 100) / 100);
                    }
                }
                if (isRangeCampaignNewEntry) {
                    const paperBase = (0, live_position_sizing_1.computePaperSizingAnchorUsd)(this.config);
                    ...
                    entrySizeUsd = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(riskScaledInitialUsd * 100) / 100);
            ...
            try {
                ...
                const prePersistMutexEval = authoritySideLower == null
                    ...
                    : this.positions.evaluateSymbolPositionMutex(sym, authoritySideLower, latestOpenPositions, existingIdx >= 0, authority.addOnAllowed === true);
                if (prePersistMutexEval.blocked) {
                    ...
                    await this.emitPipelineEventsFromDecision(first, {
                        ...envelope,
                        legacy: {
                            ...res,
                            decision: {
                ...
                const symS = String(first.symbol);
                const mPre = (0, range_engine_1.marginsForSymbol)(next, symS);
                if (riskE &&
                    ...
                    const reducedSizeUsd = capRemainingUsd >= live_position_sizing_1.MIN_POSITION_SIZE_USD
                        ? Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(Math.min(entrySizeUsd, capRemainingUsd) * 100) / 100)
                        : 0;
                    const riskModeNow = this.lastRiskExposure?.riskMode ?? null;
                    const serverControlsBlocked = !this.serverTradeControlState.server_trade_enabled ||
                        this.serverTradeControlState.close_only_mode ||
                        this.serverTradeControlState.kill_switch_active ||
                        this.reconcileSafetyCloseOnly;
                    const canReduceSize = reducedSizeUsd >= live_position_sizing_1.MIN_POSITION_SIZE_USD &&
                        riskModeNow !== "HALT" &&
                    ...
                    if (canReduceSize) {
                        ...
                        }, nowTs, entryStage, "risk_exposure_cap_for_leg");
                        continue;
                    }
                }
                const lowExpectedMoveRelaxSizeLimited = first.rangeSignalKeptByRelax === true &&
                    first.rangeSignalDowngradeReason === "low_expected_move_relaxed_by_range_structure";
                if (lowExpectedMoveRelaxSizeLimited) {
                    const capped = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round((0, live_position_sizing_1.computePaperSizingAnchorUsd)(this.config) * 0.4 * 100) / 100);
                    ...
                    this.logger.info("RANGE_LOW_EXPECTED_MOVE_RELAX_PROOF", {
                        ...
                        expectedMoveRatio: first.gateExpectedMove != null && first.gateRequiredMove != null && first.gateRequiredMove > 0
                ...
                if (this.okxDemo && signedModeForEntry === "enabled") {
                    const instId = (0, okx_demo_1.toOkxSwapInstId)(first.symbol);
                    ...
                    const committedStopForSubmit = authority.source === "v2" && v2CommittedRiskPlan
                        ...
                        : typeof res.decision.stopLoss === "number" && Number.isFinite(res.decision.stopLoss) && res.decision.stopLoss !== 0
                            ...
                            : null;
                    const committedTpForSubmit = authority.source === "v2" && v2CommittedRiskPlan?.initial_tp_price != null
                        ...
                        : typeof res.decision.takeProfit === "number" && Number.isFinite(res.decision.takeProfit) && res.decision.takeProfit !== 0
                            ...
                            : null;
                    if (authority.source === "v2") {
                        const v2DecisionObj = res.v2Decision ?? res.decision;
                        ...
                        const planValid = rawPlan.symbol === String(first.symbol) &&
                ...
                if (isPendingConfirm) {
                    ...
                    const pendingCommittedStop = authority.source === "v2" && v2CommittedRiskPlan
                        ? v2CommittedRiskPlan.stop_price
                        : authority.newStopPrice != null && isCommittedEntryStopPrice(authority.newStopPrice)
                            ...
                            : authority.invalidationPx != null && isCommittedEntryStopPrice(authority.invalidationPx)
                                ...
                                : typeof res.decision.stopLoss === "number" && isCommittedEntryStopPrice(res.decision.stopLoss)
                                    ? res.decision.stopLoss
                                    : undefined;
                    const pendingCommittedTp = authority.source === "v2" && v2CommittedRiskPlan?.initial_tp_price != null
                        ? v2CommittedRiskPlan.initial_tp_price
                        : typeof res.decision.takeProfit === "number"
                            ? res.decision.takeProfit
                            : undefined;
                    const legacyPendingReg = {
                        symbol: symStr,
                        side: authority.side,
                        ordId: String(submit.ordId),
                        clOrdId: trace.exchange_client_order_id ?? "",
                        instId: (0, okx_demo_1.toOkxSwapInstId)(symStr),
                        authority_source: authority.source,
                ...
                const lifecycleState = isExchangeEnabled ? "OPEN" : "INITIAL";
                const record = {
                    openedAt: Date.now(),
                    symbol: first.symbol,
                    side: authority.side,
                    ...
                    realizedPnl: 0,
                    stopPrice: (() => {
                        if (authority.source === "v2" && v2CommittedRiskPlan)
                            return v2CommittedRiskPlan.stop_price;
                        ...
                        if (val !== undefined && isCommittedEntryStopPrice(val))
                            return val;
                        return undefined;
                    })(),
                    targetPrice1: (() => {
                    ...
                    regimeAtEntry: entryIdentity.effectiveRegimeAtEntry,
                    ...
                    ...(entryIdentity.attachRangeMetadata
                        ? {
                            ...(typeof first.boxPos === "number" ? { rangeEntryBoxPos: first.boxPos, rangeEntryZone: (0, types_1.classifyRangeZone)(first.boxPos) } : {}),
                            rangeManagementState: "INIT",
                    ...
                    entryEvidence: {
                        capturedAt: nowTs,
                        regime_at_entry: (0, position_ops_monitor_1.regimeForSl)(entryIdentity.effectiveRegimeAtEntry === "UNKNOWN" ? "NO_TRADE" : entryIdentity.effectiveRegimeAtEntry),
                        active_engine_at_entry: activeEngine,
                        entry_signal: first.signal,
                        entry_quality_grade: authority.entryQualityGrade ?? null,
                        entry_quality_score: first.qualityScore ?? null,
                    ...
                    takeProfit2Px: authority.takeProfit2Px,
                    partialExitRatio: authority.partialExitRatio,
                    invalidationPx: authority.source === "v2" && v2CommittedRiskPlan
                        ? v2CommittedRiskPlan.stop_price
                ...
                this.logger.info("STOP_STATE_PROOF", {
                    ...
                    source: authority.source === "v2" && v2CommittedRiskPlan
                ...
                if (this.lastEffectiveLane === "RANGE") {
                    const fillZone = typeof first.boxPos === "number" ? (0, types_1.classifyRangeZone)(first.boxPos) : null;
                    ...
                    const fillProof = {
                        ...
                        adaptive_soft_explore: adaptive.detail?.stage1_adaptive_soft_explore ?? null
                    };
                    ...
                    if (entryZoneForAnomaly === "upper" && record.side === "long" && isRangeOriginPosition) {
                        this.logger.warn("RANGE_ANOMALY_UPPER_LONG_OPEN_CODE_PATH", {
                            ...
                            anomaly_note: "RANGE stage0 ?곷떒?먯꽌??濡?吏꾩엯???섏삤硫????????덇굅???섏씠?⑥씠쨌?뚯뒪??諛붿씠?⑥뒪쨌?덉뒪?좊━ zone ?쇰꺼 遺덉씪移??깆쓣 ?섏떖"
                        });
                        try {
                            await this.store.appendJsonlLine("reports/events.jsonl", {
                                ts: Date.now(),
                ...
                if (reopenArmActive && this.lastMarketMode?.routing.activeEngine === "RANGE") {
                    ...
                    arr.push(Date.now());
                    this.rangeReopenTimestampsBySymbol.set(symS, arr.filter((t) => t > Date.now() - range_engine_1.RANGE_REOPEN_WINDOW_MS));
                }
                ...
                this.logger.info("STAGE1_POSITION_OPEN_SUCCESS", {
                    ...
                    low_expected_move_relax_size_limited: first.rangeSignalKeptByRelax === true &&
                        first.rangeSignalDowngradeReason === "low_expected_move_relaxed_by_range_structure",
    ...
    evaluateAddOnStructureReinforced(open, snap, intentSide) {
        ...
        if (reg === "RANGE" || exec === "RANGE") {
            ...
            const rz = (0, types_1.classifyRangeZone)(snap.boxPos);
            const ok = (intentSide === "long" && rz === "lower") ||
        ...
        const trendStructureOk = snap.trendOk === true &&
            Math.abs(snap.emaGap ?? 0) >= ENTRY_EVIDENCE_TREND_EMA_GAP_MIN &&
            (snap.trendWeaknessScore ?? 1) <= ENTRY_EVIDENCE_TREND_WEAKNESS_MAX;
        const dirAligned = (intentSide === "long" && (snap.emaGap ?? 0) >= ENTRY_EVIDENCE_TREND_EMA_GAP_MIN) ||
    ...
    async tryPaperPositionScaleIn(existing, envelope, first, nowTs, entryQualitySizeMultiplier = 1) {
        const { legacy: res, authority } = envelope;
        const emitScaleInvariantBroken = (reason) => {
        ...
        const pnlPctNow = existing.side === "long"
        ...
        let incrementalSizeUsd;
        let targetStage;
        let scalingWeights;
        let rangeAddOnSizeMultApplied;
        ...
        if (authority.source === "v2") {
            ...
            incrementalSizeUsd = authority.source === "v2"
                ...
                : Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(adaptive.sizeUsd * 100) / 100);
            ...
            incrementalSizeUsd = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(riskScaledAddOnUsd * 100) / 100);
            ...
            if (existing.regimeAtEntry === "RANGE" && typeof first.boxPos === "number") {
                const zz = (0, types_1.classifyRangeZone)(first.boxPos);
            ...
            if (!sw) {
                if (existing.regimeAtEntry === "RANGE")
                    sw = [0.30, 0.30, 0.40];
                else if (existing.regimeAtEntry === "TREND")
                    sw = [0.30, 0.30, 0.40];
                else
                    sw = [1.0];
            }
            ...
            rangeAddOnSizeMultApplied = 1;
            if (incrementalSizeUsd < live_position_sizing_1.MIN_POSITION_SIZE_USD) {
                incrementalSizeUsd = live_position_sizing_1.MIN_POSITION_SIZE_USD;
        ...
        if (entryQualitySizeMultiplier < 1) {
            incrementalSizeUsd = Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, Math.round(incrementalSizeUsd * entryQualitySizeMultiplier * 100) / 100);
        ...
        if (existing.regimeAtEntry === "TREND") {
            ...
            if (!(0, trend_engine_1.trendPyramidAllowsScaleIn)(tfs, pyr)) {
        ...
        const { longUsd, shortUsd } = (0, range_engine_1.marginsForSymbol)(opensList, symEx);
        if (re) {
        ...
        if (existing.regimeAtEntry === "RANGE" && typeof first.boxPos === "number") {
            this.logger.info("RANGE_SCALE_IN_SUCCESS_PROOF", {
                symbol: existing.symbol,
                side: existing.side,
                box_zone: (0, types_1.classifyRangeZone)(first.boxPos),
        ...
        if (rangeCampaignScaleInPath || rangeAddOnCandidate) {
            ...
            const nextState = existing.rangeFirstProfitLocked === true ? "PROFIT_LOCKED" : "REATTACK_USED";
        ...
        if (this.okxDemo) {
            ...
            await this.submitOkxOrder({
                ...
                posSide: sPosSide,
                qty: sQtyLegacy,
                desiredNotionalUsdt: existing.isV2Authority === true ? incrementalSizeUsd : (incrementalSizeUsd * sLev),
    ...
    async pollSymbol(symbol, fetchedAt, klineLimit, regimeDetected) {
        ...
        if (!rT.ok || !rC.ok || !rF.ok) {
            ...
            if (!rC.ok)
                parts.push(rC.error);
            if (!rF.ok)
                parts.push(rF.error ?? "unknown");
            const failedEndpoint = !rT.ok ? "ticker" : (!rC.ok ? "kline" : (!rF.ok ? "funding" : "unknown"));
            return { ok: false, error: parts.join("; "), symbolDiagnostics, failedEndpoint, basePollMs, htfFetchMs: 0 };
        }
        const htf_candles = {};
        const htfTimeframes = ["5m", "15m", "1h", "4h", "1d"];
        const htf_diagnostics = {};
        ...
        const atr = (0, entry_gate_1.atrWilderLast)(rC.value, 14);
        const trend = (0, trend_filter_1.trendFilterOneMinuteCloses)(closes);
        ...
        const boxPos = boxHigh !== null && boxLow !== null && boxHigh > boxLow
            ? Math.min(1, Math.max(0, (lastPrice - boxLow) / (boxHigh - boxLow)))
            : null;
        let entry = (0, entry_signal_1.evaluatePaperEntryV1)({
            symbol,
            ema20: trend.ema20,
            ema60: trend.ema60,
            latestCandleClose,
        ...
        const sideForQuality = entry.signal === "paper_long_candidate"
            ? "long"
            : entry.signal === "paper_short_candidate"
                ? "short"
                : null;
        const qualityScore = sideForQuality === null || entry.candidateStrength === null
            ? 0
            : (0, paper_entry_quality_1.computePaperEntryQualityScore)({
                ema20: trend.ema20,
                ema60: trend.ema60,
                lastPrice,
                latestCandleClose,
        ...
        const signalStrength = (0, paper_entry_quality_1.paperSignalStrengthLabel)(qualityScore, this.config.paperEntryRelaxed);
        let signal = entry.signal;
        let entryCandidate = entry.entryCandidate;
        let gateEval = null;
        let gateBlockedReason = null;
        const entrySide = entry.signal === "paper_long_candidate"
            ? "long"
            : entry.signal === "paper_short_candidate"
                ? "short"
                : null;
        const qualityMinEffective = entry.candidateStrength === "weak"
        ...
        if (entrySide !== null) {
            ...
            const sideEdgeAligned = hasRangeEdge &&
                ((entrySide === "long" && (boxPos ?? 0.5) <= 0.38) ||
                    (entrySide === "short" && (boxPos ?? 0.5) >= 0.62));
            let rangeSideZoneMismatchReason = null;
            ...
            const rangeStructureStrong = isRangeRegime &&
                ...
                sideEdgeAligned;
            const rangeSignalKeepByRelaxCandidate = symbol === "BTCUSDT" &&
                isRangeRegime &&
            ...
            if (rangeSideZoneMismatchReason !== null) {
                const zone = (0, types_1.classifyRangeZone)(boxPos ?? 0.5);
                let mismatchDiagnosticSource = "null_no_extra_fetch";
                let proofGateExpectedMove = null;
                let proofGateRequiredMove = null;
                let proofExpectedMoveRatio = null;
                const reusedGateSnapshot = gateEval;
                if (reusedGateSnapshot !== null) {
                    ...
                    const localOnlyGate = (0, entry_gate_1.evaluateEntryCostAndHigherTfGate)({
                ...
                const tfHi = (0, entry_gate_1.entryGateHigherTimeframe)();
                const limHi = (0, entry_gate_1.entryGateHigherTfKlineLimit)();
                ...
                const gate = (0, entry_gate_1.evaluateEntryCostAndHigherTfGate)({
                    entryTimeframeCandles: rC.value,
                ...
                gateEval = gate;
                if (!gate.allowed) {
                    const expectedMoveRatio = gate.requiredMove > 0 && Number.isFinite(gate.expectedMove) && Number.isFinite(gate.requiredMove)
                        ? gate.expectedMove / gate.requiredMove
                        : null;
                    const lowExpectedMoveRelaxCandidate = gate.blockReason === "low_expected_move" &&
                        isRangeRegime &&
                        rangeStructureStrong &&
                        expectedMoveRatio != null &&
                        expectedMoveRatio >= 0.25;
                    const lowExpectedMoveRelaxHardBlocked = gate.blockReason === "low_expected_move" &&
                        expectedMoveRatio != null &&
                        expectedMoveRatio < 0.25;
        ...
        this.logHighwayCandlePipelineProof("snapshot_before_return", {
            ...
            classify: (snapshot.candles?.length ?? 0) === 0
    ...
    hydrateRiskPlan(record) {
        let modified = false;
        const regime = (0, position_ops_monitor_1.regimeForSl)(record.regimeAtEntry);
        if (record.stopPrice == null || !Number.isFinite(record.stopPrice)) {
            const mirrored = (0, position_ops_monitor_1.engineMirrorStopPrice)(record.entryPrice, record.side, regime);
        ...
        if (record.targetPrice1 == null || !Number.isFinite(record.targetPrice1)) {
            const mirroredTp = (0, position_ops_monitor_1.engineMirrorTpPrice)(record.entryPrice, record.side, regime);
            if (mirroredTp != null && Number.isFinite(mirroredTp)) {
    ...
    async writeOpenPositions(positions) {
        await this.positions.saveOpenAll(positions);
    }
    async writeClosedPositions(positions) {
        for (const p of positions) {
            await this.positions.appendClosed(p);
        }
    }
    computeOkxFilledNotionalUsdt(fillSize, fillPx, ctVal = 0.001) {
    ...
    async cancelOrder(symbol, ordId, algoId) {
        if (!this.okxDemo)
            return false;
        if (algoId) {
            const res = await this.okxDemo.cancelAlgoOrder([{ instId: (0, okx_demo_1.toOkxSwapInstId)(symbol), algoId }]);
        ...
        const res = await this.okxDemo.cancelOrder((0, okx_demo_1.toOkxSwapInstId)(symbol), ordId);
        return res.ok;
    }
    async prunePositions(symbol) {
        await this.removeClosedPositionFromLedger(symbol);
    }
    async removeClosedPositionFromLedger(symbol) {
        const opens = await this.positions.loadOpenAll();
        const filtered = opens.filter((p) => p.symbol !== symbol);
        await this.writeOpenPositions(filtered);
    }
    async helperUpsertPendingOrder(existingPendingList, newPendingItem) {
        const list = [...(existingPendingList || [])];
        const idx = list.findIndex((p) => p && ((newPendingItem.ordId && p.ordId === newPendingItem.ordId) || (newPendingItem.clOrdId && p.clOrdId === newPendingItem.clOrdId)));
        if (idx >= 0) {
        ...
        await this.store?.writePendingEntryOrders?.(list);
    }
    async executeAuthorizedV2Action(args) {
        ...
        const pendingList = (await this.store?.readPendingEntryOrders?.()) ?? [];
        const activePendingForSymbol = pendingList.find((p) => p && p.symbol === symbol && p.status !== "filled" && p.status !== "cancelled");
        ...
        const existingProtectionPending = existingPositions.find((p) => p.symbol === symbol && p.status === "PROTECTION_PENDING");
    ...
    async executeAuthorizedV2Entry(params) {
        ...
        if (!stopCreated) {
            openRecord.status = "PROTECTION_PENDING";
    ...
    async executeAuthorizedV2Addon(params) {
        ...
        if (!stopCreated) {
            updatedRecord.status = "PROTECTION_PENDING";
...
function computeLedgerSymbolExposureNotionalKrw(opens, symbol) {
    let sum = 0;
    for (const o of opens) {
        if (String(o.symbol) !== symbol || (o.status ?? "open") !== "open")
            continue;
...
function mergeEngineSymbolDecisionsWithOpenLedgerExposure(decisionBySymbol, opens, accountEquityKrw, proofLogger) {
    const out = {};
 ...
 */
function buildEngineStateSymbolDecision(envelope) {
    ...
    return {
        ...
        v2_partial_action: selector?.v2_result.v2PartialAuthority?.partialAction ??
        ...
        v2_exit_action: selector?.v2_result.v2ExitAuthority?.exitAction ??
 ...
 */
function buildAuthorityEventMeta(authority, executedEntrySizeUsd) {
    const useExecuted = authority.decision === "ENTER" &&
 ...
 */
function buildEntryAllowedEventPayload(symbol, regime, ex, authority) {
    return {
        ts: Date.now(),
        type: "ENTRY_ALLOWED",
        symbol,
 ...
 */
function buildAiEventPayload(type, symbol, regime, aiIn, aiOut, authority) {
    return {
        ts: Date.now(),
        type,
        symbol,
 ...
 */
function buildEntryBlockedEventPayload(symbol, regime, res, authority, riskStatus) {
    const d = res.decision;
    const ex = res.executorDecision;
    return {
        ts: Date.now(),
 ...
 */
function buildEntryOpenedEventPayload(symbol, authority, pos) {
    return {
        ts: Date.now(),
        type: "ENTRY_OPENED",
        symbol,
...
function buildV2SnapshotBridge(snap) {
    return {
        lastPrice: snap.lastPrice,
...
function buildV2LegacyBridge(res) {
    return {
...
function buildV2ConfigBridge(config) {
    return {
        baseSizeUsd: (0, live_position_sizing_1.computePaperSizingAnchorUsd)(config),
...
function buildV2StateBridge(opensAfterClose, lastRisk, config, paperExecutionReady, signedExecutionReady, freshTickBarrierActive, freshTickExecutionBlocked, freshTickCompletedCycles, freshTickRequiredCycles, entryQualityProfiles, serverTradeControlState, reconcileSafeModeActive, lastLivePositionsPayload, liveBalanceReady, okxWalletBalanceUsdt, okxAvailableBalanceUsdt, okxPositionsOk, okxPendingOrdersReady, pendingOrdersNotionalUsdt, pendingSymbolNotionalUsdt, balanceFetchedAt, positionsFetchedAt, pendingOrdersFetchedAt) {
    let okxActualSide = "none";
    ...
    const okxActualPositions = Array.isArray(lastLivePositionsPayload)
        ? lastLivePositionsPayload.map((p) => {
            const hit = (0, okx_position_sync_1.okxSwapRowToLedgerKey)(p);
            const symbol = hit?.symbol ?? p.symbol ?? String(p.instId ?? "").replace("-SWAP", "").replace("-USDT", "USDT");
            const rawSide = hit?.side ?? p.side ?? p.posSide ?? "";
            const sideStr = String(rawSide).toLowerCase();
            const normSideStr = sideStr === "buy" ? "long" : sideStr === "sell" ? "short" : sideStr;
            const rawNotional = Number(p.notionalUsd ?? p.notionalUSDT ?? p.sizeUsd ?? 0);
            return {
                symbol,
    ...
    return {
        currentPositions: opensAfterClose
            .map((p) => {
            const side = (0, reconciler_1.normalizePositionSideUpper)(p.side);
            if (side === "NONE")
                return null;
            return {
                symbol: p.symbol,
        ...
        directionalShockState: (lastRisk?.directionalShockState ?? "UNKNOWN"),
...
function buildPositionIdentityMeta(pos) {
    const p = pos;
    return {
...
function computeOkxFilledNotionalUsdt(fillSize, fillPx, ctVal = 0.001) {
    const sz = Number(fillSize) || 0;
    const px = Number(fillPx) || 0;
    if (sz <= 0 || px <= 0)
        return 0;
...
function getOkxInstrumentMeta(symbol) {
    const norm = String(symbol).trim().toUpperCase();
    if (exports.OKX_INSTRUMENT_SPECS[norm])
        return exports.OKX_INSTRUMENT_SPECS[norm];
    const swapNorm = norm.includes("-SWAP") ? norm : `${norm.replace("USDT", "")}-USDT-SWAP`;
    if (exports.OKX_INSTRUMENT_SPECS[swapNorm])
        return exports.OKX_INSTRUMENT_SPECS[swapNorm];
    return null;
}
