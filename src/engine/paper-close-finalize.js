...
exports.coerceCanonicalPaperCloseReason = coerceCanonicalPaperCloseReason;
exports.defaultLabelForExitType = defaultLabelForExitType;
exports.inferPaperCloseSourceFromExitType = inferPaperCloseSourceFromExitType;
exports.derivePaperCloseSource = derivePaperCloseSource;
exports.outcomeStatusFromNetPnl = outcomeStatusFromNetPnl;
exports.finiteUsd = finiteUsd;
exports.computePaperCloseLegMetrics = computePaperCloseLegMetrics;
exports.paperExitDisplayMeta = paperExitDisplayMeta;
exports.finalizePaperClosedRecord = finalizePaperClosedRecord;
/** 디스크·레거시에 남은 Highway 60 EMA 전량 종료 원문 → 표준 closeReason 코드. */
function coerceCanonicalPaperCloseReason(closeReason) {
    if (typeof closeReason !== "string")
        return closeReason;
    const t = closeReason.trim();
    if (t === "Highway 60 EMA Breakout (Short)" ||
...
function defaultLabelForExitType(t) {
    switch (t) {
        case "EXIT_SL":
            return "손절";
        case "EXIT_TP":
...
function inferPaperCloseSourceFromExitType(et) {
    switch (et) {
        case "EXIT_SL":
            return "SL";
        case "EXIT_TP":
...
function derivePaperCloseSource(closeReason, exitType) {
    if (exitType === "EXIT_RISK")
        return "RISK";
    if (exitType === "EXIT_LONG_CRASH_FORCE" || exitType === "EXIT_LONG_CRASH_REDUCE")
        return "CRASH_LONG_DEFENSE";
    if (exitType === "EXIT_SHORT_MOMENTUM_TRAIL")
        return "CRASH_SHORT_MOMENTUM";
    if (exitType === "EXIT_CRASH_FORCE" || exitType === "EXIT_CRASH_REDUCE")
        return "CRASH";
    if (exitType === "EXIT_V2_AUTHORITY")
        return "V2_AUTHORITY";
    switch (closeReason) {
        case "EXIT_LONG_CRASH_FORCE":
        case "EXIT_LONG_CRASH_REDUCE":
        ...
        case "partial_exit_1":
        case "partial_exit_2":
            if (exitType === "EXIT_PARTIAL_SPLIT_1" || exitType === "EXIT_PARTIAL_SPLIT_2")
                return "PARTIAL_SPLIT";
            return "TP_PARTIAL";
        case "trailing_stop":
            return "TRAIL";
        ...
        case "v2_exit_authority":
            return "V2_AUTHORITY";
        default:
            if (exitType !== "EXIT_UNKNOWN")
                return inferPaperCloseSourceFromExitType(exitType);
            return "UNKNOWN";
    }
}
function outcomeStatusFromNetPnl(netUsd) {
    const eps = 1e-9;
    if (netUsd > eps)
        return "win";
    if (netUsd < -eps)
...
function finiteUsd(n) {
    return typeof n === "number" && Number.isFinite(n) ? n : 0;
}
function computePaperCloseLegMetrics(input) {
    const feeRate = input.paperTakerFeeRate;
    const lev = input.open.leverage;
    const marginRaw = input.marginUsd;
    const margin = typeof marginRaw === "number" && Number.isFinite(marginRaw) && marginRaw > 0 ? marginRaw : 0;
    ...
    const fundingRateAppliedClose = typeof rawCloseFr === "number" && Number.isFinite(rawCloseFr)
        ? rawCloseFr
...
function paperExitDisplayMeta(closeReason) {
    switch (closeReason) {
        case "EXIT_LONG_CRASH_FORCE":
        ...
        default: {
            ...
            if (coerced !== closeReason) {
                return paperExitDisplayMeta(coerced);
            }
...
function closedHistoryContextAliases(open) {
    const authRaw = open.authoritySourceAtEntry ??
        (typeof open.authority === "string" && open.authority.trim().length > 0 ? open.authority : undefined);
    const sideRaw = open.authoritySideAtEntry ??
...
function finalizePaperClosedRecord(input) {
    ...
    const isPartialStage = closeReasonNorm === "partial_exit_1" || closeReasonNorm === "partial_exit_2";
    let exitType = input.exitTypeOverride ?? fromReason.exitType;
    let neutralPartialLabel = null;
    ...
    const closeReasonLabel = input.closeReasonLabelOverride ??
        neutralPartialLabel ??
        (input.exitTypeOverride != null ? defaultLabelForExitType(input.exitTypeOverride) : fromReason.closeReasonLabel);
    const closeSource = input.closeSourceOverride ??
        (neutralPartialLabel != null ? "PARTIAL_SPLIT" : derivePaperCloseSource(closeReasonNorm, exitType));
