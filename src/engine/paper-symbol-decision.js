...
exports.paperTradeBlockDecompositionPayload = paperTradeBlockDecompositionPayload;
exports.evaluateDirectionalTrendAddOnGuard = evaluateDirectionalTrendAddOnGuard;
exports.applyPaperSignalMarketAlignment = applyPaperSignalMarketAlignment;
exports.evaluatePaperSymbolEntry = evaluatePaperSymbolEntry;
const regime_exit_1 = require("../strategy/regime-exit");
const live_market_mode_1 = require("../strategy/live-market-mode");
const live_entry_pipeline_1 = require("../strategy/live-entry-pipeline");
const live_position_sizing_1 = require("../strategy/live-position-sizing");
const live_entry_policy_1 = require("../strategy/live-entry-policy");
const highway_entry_executor_1 = require("./highway-entry-executor");
const reconciler_1 = require("../engine-v2/reconciler");
const op_mode_1 = require("../engine-v2/op-mode");
const entry_approval_1 = require("../ai/entry-approval");
const types_1 = require("../models/types");
const ai_highway_filter_1 = require("../engine/ai-highway-filter");
...
const stage0SignalAbsenceProofLoggedAtBySymbol = new Map();
...
function isRangeStage0ReentryRelaxCloseReason(cr) {
    if (cr == null || cr === "stop_loss")
        return false;
    return (cr === "partial_exit_1" ||
        cr === "partial_exit_2" ||
        cr === "take_profit" ||
        cr === "trailing_stop" ||
        cr === "time_based_exit" ||
...
function getLastTwoCompletedOneMinuteBars(candles) {
...
function computeRangeEdgeReversalConfirmation(args) {
    const bars = getLastTwoCompletedOneMinuteBars(args.candles);
    if (!bars) {
        const reject = args.zone === "upper" && args.entrySide === "short"
...
function paperTradeBlockDecompositionPayload(symbol, resNull, fields) {
...
function regimeToState(regime, regimeUnknown) {
    if (regimeUnknown)
        return "UNKNOWN";
    return regime;
}
function rrFromRegime(regime) {
    if (regime === "RANGE")
        return 0.0038 / 0.0026;
    if (regime === "TREND")
        return 0.0105 / 0.0105;
    return 1;
}
function evaluateDirectionalTrendAddOnGuard(args) {
...
const STAGE1_RANGE_EDGE_SOFT_TAGS = {
...
const RANGE_STAGE0_EDGE_THRESHOLDS = { conf: 0.38, cohesion: 0.32, oscillation: 0.3 };
function rangeStage0EdgeStructureGate(sn) {
    ...
    const failed_checks = [];
    if (conf < RANGE_STAGE0_EDGE_THRESHOLDS.conf)
        failed_checks.push("conf_below_min");
    if (cohesion < RANGE_STAGE0_EDGE_THRESHOLDS.cohesion)
        failed_checks.push("cohesion_below_min");
    if (oscillation < RANGE_STAGE0_EDGE_THRESHOLDS.oscillation)
        failed_checks.push("oscillation_below_min");
    return {
...
function rangeStage0EdgeSubconditions(edge) {
...
function buildRangeUpperLongSuppressBranchProof(args) {
    const { edge, rangeSignal, rawSnapshotSignal, sn, boxPos, zone, reversalImmediate, gateResult, gateReason, entryResult, range_upper_short_priority_applied, lowConfidence, rangeReversalSwitchMatches } = args;
    ...
    const relaxedEdgeStructureOk = !edge.ok &&
    ...
    const reversalShortUpperArmed = reversalImmediate != null &&
    ...
    const return_order_matches_code = (rangeSignal.reason === "range_reversal_immediate_switch_upper_short" && firedStepIndex === 0) ||
    ...
    const raw_long_suppress_implies_edge_false = rawSnapshotSignal === "paper_long_candidate" && rangeSignal.reason === "range_upper_suppress_long_candidate_no_inertia"
        ? !edge.ok
        : null;
    let range_upper_short_priority_false_because;
    ...
    const one_line_summary = (!edge.ok && !relaxedEdgeStructureOk && rangeSignal.reason === "range_upper_suppress_long_candidate_no_inertia")
    ...
    return {
        ...
        legacy_order_bug_note: "Previously paper_long_candidate was checked before edgeStructureOk, so short structure never ran on long-raw ticks.",
        evaluate_range_stage0_signal_out: {
            signal: rangeSignal.signal,
            reason: rangeSignal.reason,
            side: rangeSignal.side
        },
        prior_order_would_skip_short_eval: rawSnapshotSignal === "paper_long_candidate"
            ? {
        ...
        range_upper_short_priority_false_because,
        one_line_summary,
        formula: "range_upper_short_priority_applied := (zone === 'upper') && (entryResult === 'RANGE_SHORT_ENTRY')"
    };
}
function calculateRangeReversalConfidenceTier(sn, boxPos, zone, edgeOk, relaxedEdgeOk) {
    let score = 0;
    if (edgeOk)
        score += 40;
    else if (relaxedEdgeOk)
        score += 20;
...
function evaluateRangeStage0Signal(sn, reversalImmediate) {
    const boxPos = typeof sn.boxPos === "number" ? sn.boxPos : 0.5;
    const zone = (0, types_2.classifyRangeZone)(boxPos);
    ...
    const relaxedEdgeStructureOk = !edgeStructureOk &&
...
function evaluateRangeStage0Scores(sn) {
    const conf = clamp01(sn.rangeConfidence ?? 0);
    const cohesion = clamp01(sn.boxCohesion01 ?? 0);
...
function mapExecutorBlockToReject(blocked) {
    switch (blocked) {
...
function isTrendUpShortSuppressMarketGuard(risk, ctx) {
    if (risk == null)
        return false;
    if (risk.directionalShockState !== "UP")
        return false;
    if (ctx.marketMode !== "TREND" && ctx.activeEngine !== "TREND")
        return false;
    if (!ctx.allowNewLong || ctx.allowNewShort)
        return false;
    return true;
}
/**
 ...
 */
function applyPaperSignalMarketAlignment(input) {
    ...
    const rangeStructureContext = snapshot.regimeStateDiag === "RANGE" &&
        (raw === "paper_long_candidate" || raw === "paper_short_candidate");
    const alignedDirectionCandidate = raw === "paper_long_candidate" ? "long" : raw === "paper_short_candidate" ? "short" : null;
    const isDirectionalShockUp = risk?.directionalShockState === "UP";
    const isDirectionalShockDown = risk?.directionalShockState === "DOWN";
    const pumpState = risk?.pumpState ?? null;
    const pump_state = risk?.pump_state ?? null;
    ...
    const suppressByDirectionalShock = (treatUpBias && raw === "paper_short_candidate") ||
        (treatDownBias && raw === "paper_long_candidate");
    const preserveByRangeStructure = rangeStructureContext &&
        ((isDirectionalShockUp && raw === "paper_long_candidate") ||
            (isDirectionalShockDown && raw === "paper_short_candidate"));
    let alignmentApplied = false;
    let alignmentReason = "NO_ALIGNMENT_APPLIED";
    let out = snapshot;
    if (suppressByDirectionalShock) {
        ...
        const crashOk = risk?.crashState !== "CRASH_LOCK";
        ...
        const breakoutEligible = (wantLongBreakout && trendOk && emaGap > 0 && allowLongNow && qualityOk && crashOk) ||
        ...
        if (breakoutEligible) {
            ...
            if (marketSignalProofLogger) {
                marketSignalProofLogger.info("DIRECTIONAL_SHOCK_BREAKOUT_SIGNAL_PROOF", {
                    ...
                    promoted_signal: out.signal,
                    signalDecisionOrigin: out.signalDecisionOrigin ?? null,
                    signalGateBlockedReason: out.signalGateBlockedReason ?? null,
                    signalMissingReason: out.signalMissingReason ?? null,
                    trendOk,
                    ema_gap: emaGap,
                    qualityScore: snapshot.qualityScore,
                    allowLongNow,
                    ...
                    crash_state: risk?.crashState ?? null,
            ...
            if (marketSignalProofLogger) {
                marketSignalProofLogger.info("RANGE_DIRECTIONAL_SHOCK_CONFLICT_PROOF", {
                    ...
                    rangeConfidence: snapshot.rangeConfidence ?? null,
                    trendOk,
                    ema_gap: emaGap,
                    qualityScore: snapshot.qualityScore,
                    allowLongNow,
    ...
    const shouldEmit = marketSignalProofLogger != null &&
...
function computeTrendVolumeRelaxForAdaptive(input) {
    const notRelaxedReasons = [];
    const gates = {
        ...
        strong_tier_eligible: input.candidateStrength === "strong" && input.qualityScore >= live_entry_policy_1.STRONG_SCORE,
        weak_candidate: input.candidateStrength === "weak",
        weak_tier_quality_ok: input.qualityScore >= live_entry_policy_1.TREND_VOLUME_RELAX_WEAK_MIN_QUALITY_SCORE,
        ...
        weak_tier_required_move_available: input.required_move_pct !== null && Number.isFinite(input.required_move_pct),
        weak_tier_required_move_low_enough: input.required_move_pct !== null &&
            input.required_move_pct <= live_entry_policy_1.TREND_VOLUME_RELAX_WEAK_MAX_REQUIRED_MOVE_PCT,
        weak_tier_uses_strong_volume_min: input.qualityScore >= live_entry_policy_1.TREND_VOLUME_RELAX_WEAK_QUALITY_FOR_STRONG_MIN
    };
    const baseOk = gates.adaptive_mode_trend &&
        gates.stage0 &&
        gates.highway_state_valid &&
        gates.highway_core_final_ok &&
        gates.score_source_trend_core_default;
    if (!gates.adaptive_mode_trend)
        notRelaxedReasons.push("adaptive_mode_not_trend");
    if (!gates.stage0)
        notRelaxedReasons.push("not_stage0");
    if (!gates.highway_state_valid)
        notRelaxedReasons.push("highway_state_not_valid");
    if (!gates.highway_core_final_ok)
        notRelaxedReasons.push("highway_core_or_final_not_valid");
    if (!gates.score_source_trend_core_default)
        notRelaxedReasons.push("score_source_not_trend_core_default");
    let relaxTier = null;
    let override = null;
    if (baseOk) {
        if (gates.strong_tier_eligible) {
            relaxTier = "strong_highway";
            override = live_entry_policy_1.TREND_POLICY_MIN_VOLUME_RATIO_PROXY_HIGHWAY_STRONG_RELAX;
        }
        else if (gates.weak_candidate &&
            ...
            override =
                input.qualityScore >= live_entry_policy_1.TREND_VOLUME_RELAX_WEAK_QUALITY_FOR_STRONG_MIN
                    ? live_entry_policy_1.TREND_POLICY_MIN_VOLUME_RATIO_PROXY_HIGHWAY_STRONG_RELAX
                    : live_entry_policy_1.TREND_POLICY_MIN_VOLUME_RATIO_PROXY_HIGHWAY_WEAK_RELAX;
            ...
            if (input.candidateStrength === "weak") {
                if (!gates.weak_tier_quality_ok) {
                    notRelaxedReasons.push(`weak_candidate_quality_below_${live_entry_policy_1.TREND_VOLUME_RELAX_WEAK_MIN_QUALITY_SCORE}`);
                }
                ...
                if (!gates.weak_tier_required_move_available) {
                    ...
                    notRelaxedReasons.push(`weak_tier_blocked_required_move_pct_above_${live_entry_policy_1.TREND_VOLUME_RELAX_WEAK_MAX_REQUIRED_MOVE_PCT}`);
                }
            }
    ...
    return {
        ...
        proof: {
            ...
            default_min_volume_ratio_proxy: live_entry_policy_1.TREND_POLICY_MIN_VOLUME_RATIO_PROXY,
            strong_relax_min: live_entry_policy_1.TREND_POLICY_MIN_VOLUME_RATIO_PROXY_HIGHWAY_STRONG_RELAX,
            weak_relax_min: live_entry_policy_1.TREND_POLICY_MIN_VOLUME_RATIO_PROXY_HIGHWAY_WEAK_RELAX,
            gates,
            not_relaxed_reasons: relaxTier !== null ? [] : [...new Set(notRelaxedReasons)]
        }
    };
}
function pack(input, sym, em, fields) {
    return {
        ts: input.now,
        timestamp: new Date(input.now).toISOString(),
        symbol: String(sym),
        pipeline_version: decision_funnel_1.PIPELINE_VERSION,
...
function internalDiscoverV2Authority(input) {
    const configuredV2Mode = (0, op_mode_1.parseEngineV2OpModeFromEnv)(process.env.ORBITALPHA_ENGINE_V2_MODE);
    const rangeLaneForcesV2 = input.regime === "RANGE" || input.routingActiveEngine === "RANGE";
    ...
    const v2Env = (0, reconciler_1.resolveSymbolDecisionEnvelope)({
        symbol: sn.symbol,
        fetchedAt: input.now,
        snapshot: {
        ...
        config: {
            baseSizeUsd: (0, live_position_sizing_1.computePaperSizingAnchorUsd)(input.config),
 ...
 */
function evaluatePaperSymbolEntry(input) {
    ...
    const supplemental_reasons = [];
    const sn = input.snapshot;
    if (!sn) {
        return {
            decision: pack(input, "UNKNOWN", null, {
                signal_state: "NONE",
    ...
    const v2AuthorityOwnsExecution = authority.decision === "ENTER" &&
        authority.source === "v2" &&
    ...
    let regimeFallbackReason = null;
    let stage1ResultCodeOverride = null;
    let edge_state = "PASS";
    let risk_state = "PASS";
    let execution_state = "PAPER_READY";
    let final_decision = "SKIP";
    let reject_reason = null;
    let expected_move_pct = null;
    let fee_estimate_pct = null;
    let rr = null;
    let atr_pct = null;
    let strategy_executor = "IDLE";
    let intentSide = null;
    let executorDecision = null;
    let adaptiveDetailOut = null;
    let trendVolumeRelaxProof = null;
    let guidanceOut = null;
    let workingSignal = "none";
    let aiGatePassed = false;
    let adaptiveOk = false;
    let adaptiveResult = null;
    let final_executor_before_priority = "IDLE";
    let final_reject_before_priority = null;
    let final_decision_before_priority = "SKIP";
    let final_executor_after_priority = "IDLE";
    let final_reject_after_priority = null;
    let final_decision_after_priority = "SKIP";
    let br = null;
    let fee_drag_filter_applied = false;
    let fee_drag_size_reduced = false;
    let fee_drag_blocked = false;
    let fee_drag_reason = null;
    let legacy_block_original_reason = null;
    let isRangeFallbackActive = false;
    let fee_drag_proof = null;
    let range_stage0_engine_taken = false;
    let range_stage0_exit_reason = null;
    let legacy_executor_path_taken = false;
    let required_move_pct = null;
    let shortfall_pct = 0;
    let stage1_leniency_applied = false;
    let range_cost_warning_applied = false;
    let range_cost_warning_threshold = null;
    let range_cost_warning_shortfall = null;
    let range_reentry_cooldown_applied = false;
    let range_reentry_wait_ms = null;
    let range_reentry_elapsed_ms = null;
    let range_reentry_remaining_ms = null;
    let range_reentry_source = null;
    let range_reentry_same_direction = false;
    let range_same_direction_reentry_relaxed_applied = false;
    let range_same_direction_reentry_wait_ms = null;
    let range_same_direction_reentry_size_mult = null;
    ...
    let range_risk_limit_relax_reason = null;
    let range_risk_limit_relax_started_at = null;
    let range_risk_limit_relax_expires_at = null;
    let range_risk_limit_relax_active = false;
    let range_risk_limit_relax_expired = false;
    let range_soft_suspend_applied = false;
    let range_soft_suspend_size_mult = null;
    let range_soft_suspend_cooldown_ms = null;
    let range_soft_suspend_same_direction_restricted = false;
    let range_bidirectional_applied = false;
    let range_short_allowed = false;
    let range_short_allowed_reason = null;
    let range_upper_edge_near = false;
    let range_center_wait = false;
    let range_final_selected_side = null;
    let range_reversal_zone = null;
    ...
    let range_reversal_short_entry_block_reason = null;
    let range_reversal_immediate_switch_applied = false;
    let range_reversal_immediate_switch_reason = null;
    let range_fresh_reentry_allowed = false;
    let range_fresh_reentry_blocked_reason = null;
    let range_fresh_reentry_size_mult = null;
    let range_reentry_wait_bypassed_no_open_position = false;
    let range_loss_streak_reduced_entry_applied = false;
    let range_loss_streak_reduced_entry_size_mult = null;
    let range_zone_action_policy = null;
    let range_zone_detected = null;
    let range_upper_short_priority_applied = false;
    let range_lower_long_priority_applied = false;
    let range_mid_wait_applied = false;
    let range_final_trade_side_by_zone = null;
    let range_stage0_branch_proof = null;
    let stage1_block_origin = null;
    let legacy_block_reason = null;
    let legacy_regime_gate = null;
    let legacy_gate_source = null;
    let override_by_legacy = false;
    let reentry_cooldown_applied = false;
    let reentry_cooldown_original_ms = null;
    let reentry_cooldown_effective_ms = null;
    let reentry_cooldown_reason = null;
    let risk_cooldown_subreason = null;
    let cooldown_remaining_ms = null;
    let same_dir_cooldown_applied = false;
    let blocked_regime_reason = null;
    let reentry_wait_ms = null;
    let reentry_elapsed_ms = null;
    ...
    let stage1HigherTfBypassSizeMult = null;
    let stage1RangeLowerEdgeSoftSizeMult = null;
    let stage1RangeEdgeSoftApplied = false;
    let stage1SoftExecOverrideFlag = false;
    let fixedUsd = null;
    let expectedMoveUsd = null;
    let requiredCostUsd = null;
    let shortfallUsd = null;
    let executorBlockReasonOriginal = null;
    let totalCost = null;
    let effectiveTotalCost = null;
    let stage1SizeMultFinal = null;
    let range_executor_priority_applied = false;
    let range_executor_priority_reason = null;
    // New hoists for resolving compilation errors
    let useFixedCost = false;
    let rangeSignal = null;
    let rangeScores = null;
    let boxPos = 0.5;
    let zone = null;
    let crashLockBypassApplied = false;
    let crashLockBypassReason = null;
    ...
    let rangeStopReentryRejectOverride = null;
    let rangeEdgeConfirmationReject = null;
    // [DIAG-V1] 진단 변수 — 진입 기준·주문 로직 변경 없음
    let diag_long_candidate_created = null;
    let diag_short_candidate_created = null;
    let diag_long_rejected_reasons = [];
    let diag_short_rejected_reasons = [];
    let diag_btc_bias = null;
    let diag_preferred_direction = null;
    let diag_range_signal_score = null;
    let diag_range_confidence = null;
    let diag_box_cohesion01 = null;
    let diag_range_oscillation_score = null;
    let diag_box_position = null;
    let diag_breakout_failure_rate = null;
    let diag_regime_exit_risk = null;
    let diag_reversal_confirmed = null;
    let diag_directional_guard_blocked = null;
    let diag_risk_blocked = null;
    let diag_final_block_layer = null;
    let diag_final_block_reason = null;
    const directionalGuardResult = (0, directional_routing_1.evaluateDirectionalTrendEntryGuard)({
    ...
    if (directionalGuardResult.blocked) {
        ...
        if (sn.signal === "paper_long_candidate")
            diag_long_rejected_reasons = [diag_final_block_reason];
        if (sn.signal === "paper_short_candidate")
            diag_short_rejected_reasons = [diag_final_block_reason];
    ...
    const directionalOverride = (0, directional_routing_1.deriveDirectionalRoutingOverride)({
    ...
    const ret = (patch, res = {}) => {
        ...
        return {
            decision: pack(input, sym, em, {
                ...
                reject_reason: (extra.reject_reason ?? reject_reason),
    ...
    if (directionalOverride.directional_routing_override_applied) {
        ...
        if (isMismatched || directionalOverride.reason.includes("BLOCKED")) {
            ...
            return ret({
                ...
                intentSide: (incomingSide === "none" ? null : incomingSide),
                executorDecision: null,
                adaptiveOk: false,
                adaptiveDetail: null,
                adaptiveResult: null,
        ...
        if (directionalOverride.allowTrendOverride && overrideRegime === "RANGE") {
            ...
            regime_state = "TREND";
            sn.directionalRoutingApplied = true;
        }
        if (input.currentStage >= 1 && input.hasOpenPosition && input.openPositionSide) {
            ...
            if (!addOnGuard.allowed) {
                return ret({
                    ...
                    intentSide: (input.openPositionSide === "none" ? null : input.openPositionSide),
                    executorDecision: null,
                    adaptiveOk: false,
                    adaptiveDetail: null,
                    adaptiveResult: null,
    ...
    let reentryBlocked_local_deprecated = false; // Placeholder if needed by other legacy code, but we use top scope
    let lowConfidence_local = false;
    let gateResult_local = "RANGE_GATE_PASS";
    let gateReason_local = "range_gate_pass";
    let entryResult_local = "RANGE_ENTRY_NONE";
    ...
    const refNotionalUsd = (0, live_position_sizing_1.computePaperSizingAnchorUsd)(input.config);
    ...
    signal_state = signalToState(sn.signal);
    intentSide = (sn.signal === "paper_long_candidate") ? "long" : (sn.signal === "paper_short_candidate" ? "short" : "none");
    workingSignal = sn.signal;
    ...
    useRangeStage0Engine =
        !v2AuthorityOwnsExecution &&
            ...
            (sn.directionalRoutingApplied ? false : (overrideRegime === "RANGE" || isRangeFallbackActive)) &&
    ...
    if (useRangeStage0Engine) {
        ...
        zone = (0, types_2.classifyRangeZone)(boxPos);
        ...
        range_zone_action_policy = range_engine_1.RANGE_ZONE_ACTION_POLICY;
        ...
        if (rangeSignal.side === "short") {
            if (range_center_wait)
                range_short_allowed_reason = "range_center_wait";
            else if (rangeLowerEdge)
                range_short_allowed_reason = "range_lower_zone_short_forbidden";
            else if (!range_upper_edge_near)
                range_short_allowed_reason = "range_upper_edge_not_near";
            else if (!rangeConfidenceOk && rangeSignal.reason !== "range_upper_short_from_long_reversal_watch_tier_relaxed")
                range_short_allowed_reason = "range_confidence_low";
            else if (!rangeBreakoutFailureOk)
                range_short_allowed_reason = "range_breakout_failure_low";
            else if (!rangeExitRiskOk && !rangeReversalSwitchMatches)
                range_short_allowed_reason = "range_exit_risk_high";
            else if (rangeReversalSwitchMatches)
                range_short_allowed_reason = "range_reversal_immediate_short_after_upper_flatten";
            else
                range_short_allowed_reason = "range_short_allowed_upper_edge";
        }
        ...
        if (input.rangeStopReentryBlock && input.regime === "RANGE") {
            ...
            const snSigSide = sn.signal === "paper_long_candidate" ? "long" : sn.signal === "paper_short_candidate" ? "short" : null;
            const candidateMatches = rangeSignal.side != null &&
        ...
        if (input.lastCloseMetaBySymbol && rangeSignal.side) {
            ...
            const rangeEdgeOkForSameDir = rangeSignal.side === "long"
                ? boxPos <= 0.38
                : rangeSignal.side === "short"
                    ? boxPos >= 0.62
                    : false;
            range_same_direction_reentry_edge_ok = rangeEdgeOkForSameDir;
            const sameDirRelaxEligible = sameDirection &&
                !range_center_wait &&
        ...
        if ((meta?.closedAt ?? 0) > 0 && elapsedMs < waitMs)
            reentryBlocked = true;
        ...
        if (entryResult === "RANGE_SHORT_ENTRY" && zone === "upper") {
            ...
            if (!conf.reversal_confirmed && conf.reject_reason)
                rangeEdgeConfirmationReject = conf.reject_reason;
            ...
            if (!rangeReversalConfirmed)
                entryResult = "RANGE_ENTRY_NONE";
        }
        ...
        executorDecision = {
            ...
            regime: "RANGE",
            executor: "RANGE",
            box_position: (0, types_2.classifyRangeZone)(boxPos),
            ...
            detail: {
                ...
                ...(range_stage0_branch_proof && zone === "upper" && sn.signal === "paper_long_candidate"
                    ? {
                        range_upper_edge_structure_ok: range_stage0_branch_proof.edge_structure_ok,
                        range_upper_edge_structure_failed_checks: range_stage0_branch_proof
                            .edge_structure_failed_checks,
                        range_upper_edge_structure_one_liner: range_stage0_branch_proof.one_line_summary
                    }
                    : {}),
        ...
        if (gateResult !== "RANGE_GATE_PASS" || rangeEdgeConfirmationReject != null) {
            const rangeFinalBlockReason = rangeEdgeConfirmationReject != null
                ...
                : gateResult === "RANGE_GATE_BLOCK_LOW_CONFIDENCE" && rangeSignal.signal === "RANGE_SIGNAL_NONE"
                    ...
                    : gateResult === "RANGE_GATE_BLOCK_RISK_ENGINE"
                        ...
                        : gateResult === "RANGE_GATE_BLOCK_REENTRY"
                            ...
                            : gateResult;
            const stage1Code = rangeEdgeConfirmationReject != null
                ? "STAGE1_BLOCKED_EDGE"
                : gateResult === "RANGE_GATE_BLOCK_REENTRY" || gateResult === "RANGE_GATE_BLOCK_RISK_ENGINE"
                    ...
                    : gateResult === "RANGE_GATE_BLOCK_WAIT_RECHECK"
                        ...
                        : gateResult === "RANGE_GATE_BLOCK_LOW_CONFIDENCE" && rangeSignal.signal === "RANGE_SIGNAL_NONE"
                            ...
                            : "STAGE1_BLOCKED_EDGE";
            const blockedRegimeIsLossStreak = blockedRegimeReasonText.includes("mode_loss_streak") || blockedRegimeReasonText.includes("highway_range_streak");
            const rangeRiskSubreason = rangeEdgeConfirmationReject != null
                ? String(rangeEdgeConfirmationReject)
                : gateResult === "RANGE_GATE_BLOCK_REENTRY"
                    ...
                    : gateResult === "RANGE_GATE_BLOCK_RISK_ENGINE"
                        ? blockedRegimeActive
                            ...
                            : "range_risk_unknown"
                        : null;
            const rangeCooldownRemainingMs = rangeEdgeConfirmationReject != null
                ? 0
                : gateResult === "RANGE_GATE_BLOCK_REENTRY"
                    ? (range_reentry_remaining_ms ?? 0)
            ...
            const rangeReentrySourceOut = rangeEdgeConfirmationReject != null
            ...
            return ret({
                strategy_executor: "RANGE",
                final_decision: "SKIP",
                reject_reason: rangeEdgeConfirmationReject != null
                ...
                diag_long_rejected_reasons: (() => {
                    const reasons = [];
                    if (rangeSignal.signal === "RANGE_SIGNAL_WAIT_RECHECK")
                        reasons.push(`range_signal_wait_recheck:${rangeSignal.reason}`);
                    if (rangeSignal.signal === "RANGE_SIGNAL_NONE")
                        reasons.push(`range_signal_none:${rangeSignal.reason}`);
                    if (gateResult !== "RANGE_GATE_PASS")
                        reasons.push(`gate_blocked:${gateResult}:${gateReason}`);
                    if (rangeEdgeConfirmationReject)
                        reasons.push(`reversal_confirmation_failed:${rangeEdgeConfirmationReject}`);
                    if (riskEngineBlocked)
                        reasons.push("risk_engine_hard_blocked");
                    if (input.risk?.longAllow === false)
                        reasons.push("risk_long_allow_false");
                ...
                diag_short_rejected_reasons: (() => {
                    ...
                    if (rangeSignal.signal === "RANGE_SIGNAL_NONE")
                        reasons.push(`range_signal_none:${rangeSignal.reason}`);
                    if (!range_short_allowed && rangeSignal.side === "short")
                        reasons.push(`short_not_allowed:${range_short_allowed_reason ?? "unknown"}`);
                    if (gateResult !== "RANGE_GATE_PASS")
                        reasons.push(`gate_blocked:${gateResult}:${gateReason}`);
                    if (rangeEdgeConfirmationReject)
                        reasons.push(`reversal_confirmation_failed:${rangeEdgeConfirmationReject}`);
                    if (riskEngineBlocked)
                        reasons.push("risk_engine_hard_blocked");
                    if (input.risk?.shortAllow === false)
                        reasons.push("risk_short_allow_false");
                    return reasons.length > 0 ? reasons : null;
                })(),
                diag_directional_guard_blocked: false,
    ...
    const _aiResult = (0, ai_highway_filter_1.evaluateAiHighwayQuality)(input.snapshot?.candles ?? [], sym, {
    ...
    if (!useRangeStage0Engine) {
        ...
        trendBoxEdgeBufferEligible =
            input.regime === "TREND" &&
                ...
                _aiResult.state === types_1.HighwayTrendState.INVALID &&
                _aiResult.invalidTier === "hard_invalid";
        if (trendBoxEdgeBufferEligible) {
            z = (0, types_2.classifyRangeZone)(sn.boxPos);
            const atBoxEdge = z === "upper" || z === "lower";
            if (atBoxEdge) {
                const rangeRescore = (0, ai_highway_filter_1.evaluateAiHighwayQuality)(input.snapshot?.candles ?? [], sym, {
                    regime: "RANGE",
                    currentStage: 0,
                    boxPos: sn.boxPos,
                    emaGap: sn.emaGap,
                ...
                const rangeStillHardInvalid = rangeRescore.state === types_1.HighwayTrendState.INVALID && rangeRescore.invalidTier === "hard_invalid";
                ...
                if (rangeEscapesHardInvalid) {
                    ...
                    executorDecision = {
                        ...
                        breakout_state: "none",
                        pullback_state: "none",
                        guidance: "諛뺤뒪 ?겶룻븯??洹쇱쿂 TREND: Highway 肄붿뼱 怨쇨꼍吏???RANGE ?ъ젏?섎룄 臾댄슚. ?쎌텛??愿留?HIGHWAY_BOX_EDGE_WATCH).",
                        detail: {
                            highway_state: "INVALID",
                            highway_invalid_tier: "hard_invalid",
                            highway_invalid_reasons: _aiResult.invalidReasons,
                            highway_stiffness_proof_trend_path: _aiResult.aiScoreRaw
                                ?.highwayStiffnessProof,
                            highway_stiffness_proof_range_rescore: rangeRescore.aiScoreRaw
                                ?.highwayStiffnessProof,
        ...
        if (!skipHighwayExecutorCall) {
            _entryIntent = "trend";
            if (aiForHighway.state === types_1.HighwayTrendState.VALID) {
                _entryIntent = "standard";
            }
            else if (aiForHighway.state === types_1.HighwayTrendState.WEAK) {
                _entryIntent = "probe";
            }
            executorDecision = (0, highway_entry_executor_1.highwayExecutorEvaluateEntry)({
            ...
            if (trendBoxEdgeBufferEligible && aiForHighway !== _aiResult) {
                executorDecision = {
                    ...
                    detail: {
                        ...
                        trend_path_stiffness: _aiResult.aiScoreRaw?.highwayStiffnessProof
                    }
                };
            }
        }
    ...
    if (!intentSide || intentSide === "none") {
        const explicitMissingReason = typeof sn?.signalMissingReason === "string" && sn.signalMissingReason.length > 0
            ? sn.signalMissingReason
            : null;
        const explicitGateReason = typeof sn?.signalGateBlockedReason === "string" && sn.signalGateBlockedReason.length > 0
            ? String(sn.signalGateBlockedReason)
            : null;
        const explicitReason = explicitMissingReason === "RANGE_DIRECTIONAL_SHOCK_CONFLICT_WAIT_PULLBACK" ||
            explicitGateReason === "RANGE_DIRECTIONAL_SHOCK_CONFLICT_WAIT_PULLBACK"
            ? "RANGE_DIRECTIONAL_SHOCK_CONFLICT_WAIT_PULLBACK"
            : null;
        const finalReject = (explicitReason ?? "SIGNAL_NONE");
        const shouldProof = final_decision === "SKIP" &&
        ...
        if (shouldProof && stage0SignalAbsenceProofLoggedAtBySymbol.get(symbolKey) !== nowKey) {
            ...
            const pumpState = input.risk?.pumpState ?? null;
            const pump_state = input.risk?.pump_state ?? null;
            const resolved_pump_lock = pumpState === "PUMP_LOCK" || pump_state === "PUMP_LOCK";
            const boxPos = sn?.boxPos ?? null;
            const zone = typeof boxPos === "number" && Number.isFinite(boxPos) ? (0, types_2.classifyRangeZone)(boxPos) : null;
            const range_edge_extreme = typeof boxPos === "number" && Number.isFinite(boxPos) ? (boxPos <= 0.08 || boxPos >= 0.92) : null;
            const logger = input.logger?.info ? input.logger : null;
            const payload = {
                symbol: symbolKey,
                raw_signal_before_alignment: sn?.signal ?? null,
                signal_after_alignment: workingSignal ?? null,
                signalDecisionOrigin: sn?.signalDecisionOrigin ?? null,
                signalMissingReason: sn?.signalMissingReason ?? null,
                signalGateBlockedReason: sn?.signalGateBlockedReason ?? null,
                ...
                candidateStrength: sn?.candidateStrength ?? null,
                entryCandidate: sn?.entryCandidate ?? null,
                range_side_candidate: null,
                trend_side_candidate: null,
                side_zone_valid: null,
                range_edge_extreme,
                relaxedRangeEntry: sn?.rangeSignalKeptByRelax === true || sn?.relaxedRangeEntry === true,
                reversal_confirmed: sn?.reversal_confirmed ?? null,
            ...
            if (logger)
                logger.info("STAGE0_SIGNAL_ABSENCE_PROOF", payload);
            else
                console.log("STAGE0_SIGNAL_ABSENCE_PROOF", payload);
        }
        return ret({
            signal_state: "NONE",
            final_decision: "SKIP",
    ...
    if (regime_state === "UNKNOWN" && input.currentStage === 0 && stage1SignalRelaxed) {
        ...
        boxPos = sn?.boxPos ?? 0.5;
        const zoneFb = (0, types_2.classifyRangeZone)(boxPos);
     ...
     */
    const unknownBlocksEntry = regime_state === "UNKNOWN" &&
        (input.currentStage >= 1 || !(input.currentStage === 0 && input.isAmbiguous));
    /** 荑⑤떎??泥댄겕(RISK_FAIL_REENTRY) ?꾪솕 (RANGE Stage 1 ?쒖젙) */
    if (risk_state === "COOLDOWN" && input.currentStage === 0 && input.regime === "RANGE") {
        ...
        const isRelaxReason = closeReason === "partial_exit_1" ||
    ...
    if (input.regime === "NO_TRADE") {
        ...
        return ret({
            ...
            ai_decision: "N/A",
            adaptive_decision: "N/A",
            guidance: typeof input.regimeDetail?.reason === "string"
    ...
    const costGateComparable = typeof em === "number" &&
    ...
    if (costGateComparable && em !== null && effectiveTotalCost !== null) {
        ...
        if (feeWouldBlock) {
            if (input.currentStage === 0) {
                ...
                if (rangeStage0CostMode)
                    range_cost_warning_applied = true;
    ...
    if (input.currentStage >= 1 &&
        ...
        reject_reason = "EDGE_FAIL_FEE";
        final_decision = "REJECT";
        supplemental_reasons.push(input.currentStage >= 2 ? "EDGE_FAIL_FEE_STAGE3_STRICT" : "EDGE_FAIL_FEE_STAGE2_STRICT");
    }
    const minVol = input.config.paperMinEdgeVolatilityMove;
    /** Stage 1만 기대 변동성 게이트(expected move) 하한 추가 완화; Stage 2+는 기본 대비 강화 */
    const effectiveMinVol = input.currentStage === 0 ? minVol * 0.52 : input.currentStage === 1 ? minVol * 1.1 : minVol * 1.18;
    ...
    const rangeStage0SignalActive = input.currentStage === 0 &&
        input.regime === "RANGE" &&
    ...
    if (blockedRegime && blockedRegime.until > input.now) {
        ...
        const nearEdgeRangeCandidate = input.currentStage === 0 &&
            input.regime === "RANGE" &&
            typeof sn.boxPos === "number" &&
            (sn.boxPos <= 0.38 || sn.boxPos >= 0.62);
        const streakSuspend = blockedRegime.reason.includes("mode_loss_streak") || blockedRegime.reason.includes("highway_range_streak");
        const relaxModeCooldown = nearEdgeRangeCandidate &&
            streakSuspend &&
        ...
        if (relaxModeCooldown) {
            ...
            const engineBlockedOnlyByLossSuspend = input.risk?.engineBlocked === true &&
                streakSuspend &&
            ...
            const upperRiskHit = (input.risk?.engineBlocked === true && !engineBlockedOnlyByLossSuspend) ||
                (input.risk?.crashState !== undefined && input.risk.crashState !== "NONE") ||
            ...
            elapsedMs = input.now - (meta?.closedAt ?? 0);
            const previewReentryActive = (meta?.closedAt ?? 0) > 0 &&
                waitMs > 0 &&
                elapsedMs < waitMs;
    ...
    const rangeReversalGlobalReentryBypass = input.rangeReversalImmediateSwitch != null &&
        intentSide === input.rangeReversalImmediateSwitch.preferredSide &&
        input.regime === "RANGE" &&
        input.currentStage === 0 &&
        sn != null &&
        typeof sn.boxPos === "number" &&
        ((input.rangeReversalImmediateSwitch.preferredSide === "short" && (0, types_2.classifyRangeZone)(sn.boxPos) === "upper") ||
            (input.rangeReversalImmediateSwitch.preferredSide === "long" && (0, types_2.classifyRangeZone)(sn.boxPos) === "lower"));
    if (input.lastCloseMetaBySymbol && input.reentryCooldownMs > 0 && intentSide) {
        ...
        const relaxEligible = input.currentStage === 0 &&
            input.regime === "RANGE" &&
            meta != null &&
            !scaledFromClose &&
            isRangeStage0ReentryRelaxCloseReason(meta.closeReason);
        if (relaxEligible && waitMs > 0) {
            const relaxed = Math.max(RANGE_STAGE0_REENTRY_RELAX_MIN_MS, Math.floor(waitMs * RANGE_STAGE0_REENTRY_RELAX_MULT));
            if (relaxed < waitMs) {
                waitMs = relaxed;
                reentry_cooldown_applied = true;
        ...
        if (rangeStage0 && meta != null && waitMs > 0) {
            const boxReformed = typeof sn.rangeCycleCount === "number" &&
            ...
            if (directionFlipped || boxReformed || (enoughTimeElapsed && edgeCandidate)) {
                const relaxedByStructure = Math.max(RANGE_STAGE0_REENTRY_RELAX_MIN_MS, Math.floor(waitMs * (directionFlipped ? 0.45 : 0.6)));
        ...
        if (lastClose > 0 && elapsedMs < waitMs && !rangeReversalGlobalReentryBypass) {
            ...
            if (reentry_cooldown_reason)
                supplemental_reasons.push(`REENTRY_COOLDOWN_REASON_${reentry_cooldown_reason}`);
    ...
    if (final_decision === "REJECT") {
        ...
        return ret({
            ...
            adaptive_decision: "N/A",
            supplemental_reasons,
            stage1_result_code: (reject_reason?.startsWith("EDGE") ? "STAGE1_BLOCKED_EDGE" : "STAGE1_BLOCKED_RISK"),
            required_move_pct,
            shortfall_pct
        }, {
            intentSide,
    ...
    if (useRangeStage0Engine && (!executorDecision || !executorDecision.entry_allowed)) {
        ...
        const rangeFinalBlockReason = rangeBlocked === "RANGE_GATE_BLOCK_LOW_CONFIDENCE" && signal_state === "NONE"
            ? "RANGE_SIGNAL_NONE"
        ...
        const rangeStage1Code = rangeFinalBlockReason.startsWith("RANGE_RISK_BLOCK_")
            ? "STAGE1_BLOCKED_RISK"
            ...
            rangeFinalBlockReason === "RANGE_RISK_BLOCK_REENTRY"
                ...
                : rangeFinalBlockReason.startsWith("RANGE_RISK_BLOCK_")
                    ...
                    : rangeFinalBlockReason === "RANGE_SIGNAL_NONE"
                        ? "SIGNAL_NONE"
                        : "EDGE_FAIL_EXPECTANCY";
        if (reject_reason === "RISK_COOLDOWN" || reject_reason === "RISK_FAIL_REENTRY")
            risk_state = "COOLDOWN";
        ...
        if (zone === "upper" && sn.signal === "paper_long_candidate") {
            ...
            console.log("RANGE_REVERSAL_BOTTLENECK_PROOF", {
                ...
                reversal_3tier_result: {
                    ...
                    is_extreme_edge: zone === "upper" ? boxPos >= 0.74 : boxPos <= 0.26
                },
                metrics: {
                    rangeConfidence: sn.rangeConfidence,
                    boxCohesion01: sn.boxCohesion01,
    ...
    if (!executorDecision || !executorDecision.entry_allowed) {
        ...
        const stage1BlockCode = input.currentStage === 0
        ...
        if (reject_reason === "RISK_COOLDOWN")
            risk_state = "COOLDOWN";
        const invalidTier = executorDecision?.detail?.highway_invalid_tier ?? null;
        ...
        const baseCandidateExists = workingSignal === "paper_long_candidate" ||
        ...
        const useRangeStage0Engine = overrideRegime === "RANGE" &&
            input.currentStage === 0 &&
            !shouldBypassRangeStage0;
        const hasAuthorityEnterFromV2 = authority.decision === "ENTER" && authority.source === "v2";
        const authorityExpectancySoftPassEligible = hasAuthorityEnterFromV2 &&
            !severeRiskLock &&
            (authority.stageMarginKrw ?? 0) > 0 &&
            (authority.side === "long" || authority.side === "short");
        if (reject_reason === "EDGE_FAIL_EXPECTANCY" && authorityExpectancySoftPassEligible) {
            ...
            if (executorDecision?.entry_allowed) {
                ...
                if (typeof br === "string" && br.length > 0)
                    supplemental_reasons.push(`EXEC_BLOCKED_${br.toUpperCase()}`);
                ...
                return ret({
                    ...
                    target_stage: null,
                    supplemental_reasons,
                    stage1_result_code: stage1BlockCode,
                    legacy_block_reason,
                    legacy_regime_gate,
                    legacy_gate_source,
                    override_by_legacy,
    ...
    const last10Net = typeof input.risk?.detail?.last10_net_usd === "number" && Number.isFinite(input.risk.detail.last10_net_usd)
        ? input.risk.detail.last10_net_usd
        : 0;
    ...
    if (aiIn) {
        ...
        if (input.currentStage === 0 &&
            ...
            const tierTag = typeof rangeReversalTier === "string" && rangeReversalTier.length > 0
                ? rangeReversalTier.toUpperCase()
                : "UNKNOWN";
            supplemental_reasons.push(`RANGE_REVERSAL_TIER_${tierTag}_SIZE_REDUCED`);
        }
        const authorityExpectancySoftPassSizeMult = reject_reason === "AUTHORITY_EXPECTANCY_SOFT_PASS" ? 0.5 : 1.0;
        ...
        const authorityAdaptiveSoftPassSizeMult = reject_reason === "AUTHORITY_ADAPTIVE_SOFT_PASS" ? 0.5 : 1.0;
         ...
         */
        const feeDragRangePriorityProtected = input.regime === "RANGE" &&
            input.currentStage === 0 &&
        ...
        if (costWarningStage1 && input.currentStage === 0 && !feeDragRangePriorityProtected) {
            ...
            if (rc !== null && rc > 0 && emU !== null) {
                ...
                const extremeTail = emRatio <= c.paperFeeDragBlockEmRatioMax &&
                ...
                if (extremeTail && c.paperFeeDragTailSizeMult < 1) {
                    ...
                    const weakTail = shortfall_pct >= c.paperFeeDragWeakShortfallPctMin &&
        ...
        const stage1RangeExplorePath = stage1ExploreSoftExec ||
        ...
        const stage1RangeAdaptiveSoftExplore = input.currentStage === 0 &&
            input.regime === "RANGE" &&
        ...
        const highwayDet = executorDecision?.detail;
        const highwayAiRaw = highwayDet?.aiScoreRaw;
        const highwayStateOk = highwayDet?.state === types_1.HighwayTrendState.VALID;
        const highwayCoreFinalOk = !highwayAiRaw ||
            (highwayAiRaw.coreState === types_1.HighwayTrendState.VALID && highwayAiRaw.finalState === types_1.HighwayTrendState.VALID);
        const scoreSourceTrendCore = highwayDet?.scoreSource === "trend_core_default";
        let trendVolumeRatioMinOverride = null;
        if (input.adaptiveMode === "trend") {
            ...
            if (tier === "strong_highway")
                supplemental_reasons.push("ADAPTIVE_TREND_VOLUME_MIN_RELAXED_HIGHWAY_STRONG");
            if (tier === "weak_highway_edge_ok")
                supplemental_reasons.push("ADAPTIVE_TREND_VOLUME_MIN_RELAXED_HIGHWAY_WEAK_EDGE");
        }
        else {
            trendVolumeRelaxProof = null;
        }
        const trendEmaSoftGate = authority.decision === "ENTER" &&
            authority.source === "v2" &&
            input.adaptiveMode === "trend";
        const effectiveV2ModeForLog = input.regime === "RANGE" || input.routingActiveEngine === "RANGE"
            ? "engine_v2"
            : (0, op_mode_1.parseEngineV2OpModeFromEnv)(process.env.ORBITALPHA_ENGINE_V2_MODE);
        console.log("[ENTRY_EXECUTION_AUTHORITY_TRACE]", {
            symbol: String(sym),
            authority_owner: authority.source,
            authority_decision: authority.decision,
            configured_v2_mode: (0, op_mode_1.parseEngineV2OpModeFromEnv)(process.env.ORBITALPHA_ENGINE_V2_MODE),
        ...
        adaptive = (0, live_entry_pipeline_1.runFuturesAdaptiveEntry)({
            ...
            baseSizeUsd: (0, live_position_sizing_1.computePaperSizingAnchorUsd)(input.config) * dynamicSizeMult,
        ...
        console.log("[ADAPTIVE_RESULT_SUMMARY]", {
            ...
            candidate_strength: sn?.candidateStrength ?? null,
    ...
    const hasAuthorityEnterFromV2 = authority.decision === "ENTER" &&
        authority.source === "v2" &&
        (authority.side === "long" || authority.side === "short") &&
        (authority.stageMarginKrw ?? 0) > 0;
    const hardBlockedForExecution = !!severeRiskLock ||
        risk_state === "HARD_BLOCK";
    if (adaptive && !adaptive.ok) {
        const af = adaptive;
        ...
        if (authorityBypassesPolicyVeto) {
            ...
            adaptiveOk = true;
            adaptiveDetailOut = adaptive.detail ?? null;
            const bypassSide = authority.side === "long" || authority.side === "short"
                ? authority.side
                : intentSide === "long" || intentSide === "short"
                    ? intentSide
                    : null;
            const authUsdt = typeof authority.stageMarginKrw === "number" && Number.isFinite(authority.stageMarginKrw) && authority.stageMarginKrw > 0
                ? (authority.stageMarginKrw / 1400)
                : 0;
            const anchorMult = (0, live_position_sizing_1.computePaperSizingAnchorUsd)(input.config) * dynamicSizeMult;
            if (bypassSide != null) {
                adaptive = {
                    ok: true,
                    direction: bypassSide,
                    sizeUsd: Math.max(live_position_sizing_1.MIN_POSITION_SIZE_USD, authUsdt > 0 ? authUsdt : anchorMult),
    ...
    const authorityAdaptiveSoftPassSizeMult = reject_reason === "AUTHORITY_ADAPTIVE_SOFT_PASS" ? 0.5 : 1.0;
    const authorityExpectancySoftPassSizeMult = reject_reason === "AUTHORITY_EXPECTANCY_SOFT_PASS" ? 0.45 : 1.0;
    const initialEntryQty = adaptive?.ok === true && typeof adaptive.sizeUsd === "number" && Number.isFinite(adaptive.sizeUsd) && sn.lastPrice > 0
    ...
    if (initialEntryQty <= 0) {
        ...
        return ret({
            ...
            adaptiveOk: false,
            adaptiveDetail: adaptiveDetailOut,
            adaptiveResult: adaptiveOk ? adaptive : null,
            aiGatePassed: true
        });
    }
    final_decision = "ENTER";
    reject_reason = null;
    execution_state = "PAPER_READY";
    const forceEnterAdaptive = adaptive?.detail["stage1_adaptive_force_enter"] ?? adaptive?.detail["stage1_adaptive_soft_explore"];
    if (forceEnterAdaptive === "direction_none")
        supplemental_reasons.push("STAGE1_EXPLORE_ADAPTIVE_DIRECTION_NONE");
    if (forceEnterAdaptive === "ema_flat")
        supplemental_reasons.push("STAGE1_EXPLORE_ADAPTIVE_EMA_FLAT");
    ...
    return ret({
        ...
        stage1_result_code: stage1ResultCodeOverride ?? (reject_reason === "AUTHORITY_EXPECTANCY_SOFT_PASS"
        ...
        price: sn.lastPrice,
        stopLoss: (() => {
            const slThresh = (0, regime_exit_1.stopLossPctForRegime)(input.regime);
        ...
        original_signal_state: signal_state,
        final_signal_state: signal_state,
        ...
        diag_short_rejected_reasons: null,
        diag_btc_bias: (() => {
            const btcB = (0, live_market_mode_1.btcBiasFromModeDetail)(input.adaptiveDetail || {});
        ...
        adaptiveOk: true,
        adaptiveDetail: adaptiveDetailOut,
        adaptiveResult: (adaptiveOk && adaptive) ? adaptive : null,
        aiGatePassed: true
    });
