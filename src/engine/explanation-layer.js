...
exports.buildPaperExplanation = buildPaperExplanation;
function shortMarketJudgment(mm) {
    switch (mm) {
        case "RANGE":
            return "횡보 우세";
        case "TREND":
...
function compactEngineLine(routing) {
    const eng = routing.activeEngine;
    const role = eng === "RANGE" ? "양방향·왕복" : eng === "TREND" ? "돌파·스위칭" : "대기";
    const entry = routing.newEntryPolicy === "full" ? "전량" : routing.newEntryPolicy === "reduced" ? "축소" : "보류";
 ...
 */
function buildPaperExplanation(input) {
