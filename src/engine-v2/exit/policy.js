...
exports.evaluateV2ExitPolicy = evaluateV2ExitPolicy;
function resolvePosition(args) {
    ...
    if (longPos && shortPos) {
        ...
        return { pos: longPos, side: "long", dual: true };
    }
    if (longPos)
        return { pos: longPos, side: "long", dual: false };
    if (shortPos)
        return { pos: shortPos, side: "short", dual: false };
    return { pos: null, side: "none", dual: false };
}
function urgencyFromAction(action, critical) {
    if (critical)
        return "CRITICAL";
    if (action === "FULL_EXIT")
        return "HIGH";
    if (action === "REDUCE" || action === "PARTIAL_TAKE_PROFIT")
        return "MID";
    return "LOW";
}
function evaluateV2ExitPolicy(args) {
    ...
    const shockAgainst = (side === "long" && args.judgment.shockPhase === "DOWN_SHOCK") ||
        (side === "short" && args.judgment.shockPhase === "UP_SHOCK");
    let action = "HOLD";
    let reason = "NO_EXIT_SIGNAL";
    let reduceRatio = 0;
