...
exports.calculateRegimeConfidence = calculateRegimeConfidence;
function calculateRegimeConfidence(judgment, input) {
    ...
    if (judgment.subtype === "RANGE_MID_CHOP")
        score -= 8;
    if (judgment.subtype === "RANGE_LOWER_REACTION" || judgment.subtype === "RANGE_UPPER_REACTION") {
        if ((sn.rangeConfidence || 0) >= 0.7)
            score += 3;
    }
    if (judgment.subtype === "TREND_EXHAUSTION")
        score -= 12;
    if (judgment.subtype === "TRANSITION_CONFLICT")
