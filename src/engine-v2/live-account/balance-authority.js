...
exports.deriveLiveBalanceAuthority = deriveLiveBalanceAuthority;
...
function parseWalletFromPayload(payload) {
    const out = {
        ...
        available_fallback_reason: null,
    };
    if (!payload)
        return out;
...
function computePaperEstimatedUsage(positions, walletBalanceUsdt) {
    const lines = positions.map((p) => {
        const lev = Number.isFinite(p.leverage) && p.leverage > 0 ? p.leverage : 1;
        const isV2 = p.isV2Authority === true;
    ...
    const leverage = walletBalanceUsdt != null && walletBalanceUsdt > 0
        ? notional / walletBalanceUsdt
        : (lines.length === 0 ? 0 : null);
    return { used_margin, notional, leverage, lines };
}
function parseOkxPositions(payload) {
    if (!payload || !Array.isArray(payload))
        return { used_margin: 0, notional: 0, lines: [], parse_source: "none" };
    const lines = [];
    let totalMargin = 0;
    let totalNotional = 0;
    ...
    for (const p of payload) {
        const parsedKey = (0, okx_position_sync_1.okxSwapRowToLedgerKey)(p);
        if (!parsedKey)
            continue;
...
function deriveLiveBalanceAuthority(input) {
    ...
    const ready = parsedWallet.walletBalanceUsdt != null &&
    ...
    const okxEffectiveLeverage = (input.okxPositionsPayload && parsedWallet.walletBalanceUsdt != null && parsedWallet.walletBalanceUsdt > 0)
    ...
    let live_balance_block_reason = null;
    if (allBalanceFieldsZero) {
        live_balance_block_reason = "AVAILABLE_BALANCE_ZERO";
    }
    const okxBalanceError = live_balance_block_reason ??
