...
exports.calculateRiskSizing = calculateRiskSizing;
/**
 * Tier 5: Risk & Sizing Policy (Refined)
 * Adjusts size based on regime and confidence.
 */
function calculateRiskSizing(judgment, confidence, executor, input) {
    const { config } = input;
    ...
    let isBlocked = false;
    let blockReason;
    let leverageProfile = "BASE";
    let leverageBlockReason = null;
    ...
    const entryQualityGrade = qualityScore >= 90 ? "S" : qualityScore >= 80 ? "A" : "B";
    ...
    const marketSnapshotReady = snapshot != null &&
    ...
    const paperReadinessBlockReasons = [];
    if (state.serverTradeEnabled === false)
        paperReadinessBlockReasons.push("SERVER_TRADE_DISABLED");
    if (state.closeOnlyMode === true)
        paperReadinessBlockReasons.push("CLOSE_ONLY_MODE");
    if (state.killSwitch === true || state.killSwitchActive === true)
        paperReadinessBlockReasons.push("KILL_SWITCH");
    if (state.reconcileSafeMode === true || state.reconcileSafeModeActive === true)
        paperReadinessBlockReasons.push("RECONCILE_SAFE_MODE");
    if (riskModeUpper === "HALT")
        paperReadinessBlockReasons.push("RISK_MODE_HALT");
    if (dailyLossGuardTriggered)
        paperReadinessBlockReasons.push("DAILY_LOSS_GUARD");
    if (!marketSnapshotReady)
        paperReadinessBlockReasons.push("MARKET_SNAPSHOT_NOT_READY");
    if (!positionStateReady)
        paperReadinessBlockReasons.push("POSITION_STATE_NOT_READY");
    if (!v2InputReady)
        paperReadinessBlockReasons.push("V2_INPUT_NOT_READY");
    ...
    const maxOrderNotionalUsdt = config.okxLiveMaxOrderNotionalUsdt ?? (state.liveMaxOrderNotionalUsdt != null ? Number(state.liveMaxOrderNotionalUsdt) : null);
    const maxAddonNotionalUsdt = config.okxLiveMaxAddonNotionalUsdt ?? (state.liveMaxAddonNotionalUsdt != null ? Number(state.liveMaxAddonNotionalUsdt) : null);
    const maxSymbolNotionalUsdt = config.okxLiveMaxSymbolNotionalUsdt ?? (state.liveMaxSymbolNotionalUsdt != null ? Number(state.liveMaxSymbolNotionalUsdt) : null);
    const maxAccountNotionalUsdt = config.okxLiveMaxAccountNotionalUsdt ?? (state.liveMaxAccountNotionalUsdt != null ? Number(state.liveMaxAccountNotionalUsdt) : null);
    const maxAddonCount = config.okxLiveMaxAddonCount ?? (state.liveMaxAddonCount != null ? Number(state.liveMaxAddonCount) : null);
    ...
    if (!limitsConfigured && !state.okxLiveEnabled) {
