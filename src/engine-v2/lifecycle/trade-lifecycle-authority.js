...
exports.deriveTradeLifecycleAuthority = deriveTradeLifecycleAuthority;
const types_1 = require("../../models/types");
const probe_tp_policy_1 = require("../exit/probe-tp-policy");
function resolveCooldownType(input) {
    ...
    if (reason.includes("risk") || reason.includes("halt") || reason.includes("daily_loss"))
        return "risk_halt";
    if (reason.includes("reentry_fail") || reason.includes("fail_reentry"))
        return "fail_reentry";
    if (input.cooldownState.reentryBlocked || reason.includes("reentry") || reason.includes("cooldown"))
        return "time_reentry";
    return "none";
}
function resolveLifecycleStage(input) {
    ...
    if (cooldownType !== "none")
        return "cooldown";
    if (input.position != null && (input.unrealizedPnlPct ?? 0) > 0.003 && input.regime === "RANGE")
        return "partial";
    return "position_state";
}
function deriveTradeLifecycleAuthority(input) {
    const proofReasons = [];
    const trueInconsistencyReasons = [];
    const knownShadowGaps = [];
    const cooldownType = resolveCooldownType(input);
    const lifecycleStage = resolveLifecycleStage(input);
    const lifecycleAuthorityOwner = input.authoritySource === "v2" && input.adoptedEngine === "V2"
        ? "v2"
    ...
    const executionOwner = isV2Owner ? "v2_executor" : "paper_engine";
    ...
    const positionStateExecutionOwner = isV2Owner ? "v2_executor" : "paper_engine";
    ...
    const rangeEdge = typeof boxPos === "number" && Number.isFinite(boxPos) && (boxPos <= 0.15 || boxPos >= 0.85);
    const rangeMid = typeof boxPos === "number" && Number.isFinite(boxPos) && (0, types_1.classifyRangeZone)(boxPos) === "mid";
    const trendHealthy = input.rawMetricsSummary.trendWeaknessScore <= 0.55;
    let result_givebackPct;
    let result_guardThresholdPct;
    let result_guardAction;
    let addOnAllowed = null;
    let newStopPrice;
    let nextAddonNotional;
    let partialAction = "none";
    let partialReason = null;
    let exitAction = "none";
    let exitReason = null;
    let reduceRatio = null;
    let tp1TriggeredResult;
    let tp2TriggeredResult;
    let tp1PxResult;
    let tp2PxResult;
    ...
    const isProbe = (0, probe_tp_policy_1.isProbeEntryReason)(v2EntryReason);
    if (isProbe && input.position != null && input.markPrice != null) {
        const pos = input.position;
        const side = input.side;
        const stopPx = input.currentStopPrice ?? pos.ledger_stop_px ?? 0;
        if (stopPx > 0) {
            const probePlan = (0, probe_tp_policy_1.calculateProbeTpPlan)(input.symbol, side, v2EntryReason, pos.entryPrice, stopPx);
            if (probePlan != null) {
                ...
                if (!alreadyFilled) {
                    const timeStop = (0, probe_tp_policy_1.evaluateProbeTimeStop)(input.symbol, side, probePlan, input.markPrice, barsHeld, alreadyFilled);
                ...
                if (alreadySubmitted && !alreadyFilled && exitAction === "none") {
                    const trigger = (0, probe_tp_policy_1.detectProbeTP1Trigger)(input.symbol, side, probePlan, input.markPrice, alreadySubmitted, alreadyFilled);
                    if (trigger?.triggered) {
                        // triggered=true: TP1 가격 도달 감지.
                        // fill 확인/ledger 반영/본절 이동/재정렬은 executor가 OKX fill API 확인 후 수행.
                        tp1PxResult = probePlan.tp1Price;
                        proofReasons.push(`PROBE_TP1_TRIGGERED|markPx=${input.markPrice.toFixed(4)}|tp1Px=${probePlan.tp1Price.toFixed(4)}|fillConfirmed=false|AWAIT_OKX_FILL_API`);
                    }
                }
                // Stage 7: TP1 submitted 장시간 미체결 경보
                if (alreadySubmitted && !alreadyFilled) {
                    (0, probe_tp_policy_1.detectProbePendingStale)(input.symbol, side, probePlan, alreadySubmitted, alreadyFilled, barsHeld);
                }
                // Stage 1: TP1 미제출 상태 → 제출 결정
                if (!alreadySubmitted && !alreadyFilled && exitAction === "none" && partialAction === "none") {
                    const submitDecision = (0, probe_tp_policy_1.evaluateProbeTP1Submit)(input.symbol, side, probePlan, totalQty, alreadySubmitted, alreadyFilled);
    ...
    if (input.position != null) {
        if (input.regime === "RANGE") {
            ...
            nextAddonNotional = input.finalAddonNotionalUsdt ?? Math.min(remainingSymbolRoom, remainingGlobalRoom, addonMaxByProfit, liveMaxOrderNotional);
            console.info(JSON.stringify({
            ...
            const pyramidPass = trendHealthy &&
                pnlPct >= 0.002 &&
            ...
            if (pyramidPass && !guardTriggered)
                proofReasons.push("PROFIT_LOCKED_PYRAMID_ACTIVE");
        }
        else {
            addOnAllowed = false;
            partialAction = "none";
    ...
    if (input.directionalShockState === "DOWN" &&
        input.side === "long" &&
    ...
    const legacyInterventionDetected = !isV2Owner ||
    ...
    const result = {
        ...
        exitReason,
        reduceRatio,
        newStopPrice: newStopPrice ?? input.suggestedStopPrice ?? undefined,
        invalidationPx: input.suggestedInvalidationPx ?? null,
