...
exports.normalizePositionSideUpper = normalizePositionSideUpper;
exports.normalizePositionSideLower = normalizePositionSideLower;
exports.normalizeAuthoritySide = normalizeAuthoritySide;
exports.normalizeAuthorityDecision = normalizeAuthorityDecision;
exports.buildV2LegacyAdapter = buildV2LegacyAdapter;
exports.reconcileV2Decision = reconcileV2Decision;
exports.deriveExecutionAuthority = deriveExecutionAuthority;
exports.deriveExecutionAuthorityFromEnvelope = deriveExecutionAuthorityFromEnvelope;
exports.buildV2ShadowParityPayload = buildV2ShadowParityPayload;
exports.resolveSymbolDecisionEnvelope = resolveSymbolDecisionEnvelope;
const index_1 = require("./index");
const envelope_1 = require("./execution/envelope");
/**
 * LEGACY NORMALIZATION HELPERS (Phase 4 Independence)
 * Ensures consistent behavior across engine boundaries.
 */
function normalizePositionSideUpper(side) {
...
function normalizePositionSideLower(side) {
    if (side === "long" || side === "LONG")
        return "long";
    if (side === "short" || side === "SHORT")
        return "short";
...
function normalizeAuthoritySide(side) {
    const s = normalizePositionSideLower(side);
    if (s === "none")
        return null;
    return s;
}
function normalizeAuthorityDecision(decision) {
    if (decision === undefined || decision === null)
        return "SKIP";
    const d = String(decision).trim().toUpperCase();
    if (d === "ENTER" || d === "SKIP" || d === "REJECT" || d === "DISABLED")
        return d;
    return "SKIP";
}
/**
 ...
 */
function buildV2LegacyAdapter(res) {
    return {
        decision: {
            ...
            required_cost_usd: typeof res.decision?.required_cost_usd === "number" && Number.isFinite(res.decision.required_cost_usd)
        ...
        executorDecision: res.executorDecision ? {
            ...
            total_cost: typeof res.executorDecision.total_cost === "number" && Number.isFinite(res.executorDecision.total_cost)
 ...
 */
function reconcileV2Decision(v1, v2, engineMode) {
    const legacy_result = {
        ...
        size: typeof v1.decision?.required_cost_usd === "number" && Number.isFinite(v1.decision.required_cost_usd)
    ...
    return {
        ...
        v2_result: v2,
        adopted_result,
        mismatch: legacy_result.decision !== v2.decision ||
 ...
 */
function deriveExecutionAuthority(selector) {
    ...
    return {
        ...
        nonBypassableHardBlockPresent: useV2 && v2Risk.blockReason != null ? new Set([
            "KILL_SWITCH_ACTIVE",
...
function deriveExecutionAuthorityFromEnvelope(envelope) {
    return {
        ...
        leverageProfile: envelope.leverageProfile == null ? "BASE" : envelope.leverageProfile,
        ...
        entryQualityGrade: envelope.entryQualityGrade == null ? "B" : envelope.entryQualityGrade,
        ...
        nonBypassableHardBlockPresent: envelope.hardBlockReason != null && new Set([
            "KILL_SWITCH_ACTIVE",
            "SERVER_TRADE_DISABLED",
            "CLOSE_ONLY_MODE",
            "RISK_MODE_HALT",
 ...
 */
function buildV2ShadowParityPayload(symbol, ts, selectorResult) {
    return {
        symbol,
        ts,
 ...
 */
function resolveSymbolDecisionEnvelope(input) {
    ...
    // 4. Execution
    const v2Input = (0, index_1.adaptV2Input)(symbol, fetchedAt, snapshotAdapter, configAdapter, state, buildV2LegacyAdapter(legacyDecision), snapshotAdapter.candles);
    const v2ResRaw = (0, index_1.runEngineV2)(v2Input);
    const v2Res = state.reconcileSafeMode === true &&
    ...
    const legacyComparison = {
        legacyDecision: legacyDecision.decision.final_decision,
    ...
    let executionEnvelope = (0, envelope_1.buildV2ExecutionAuthorityEnvelope)({
    ...
    const authority = v2Mode === "engine_v2"
    ...
    const selectorMismatch = v1_dec !== v2_dec ||
        v1Side !== v2Side ||
    ...
    if (v2Mode === "engine_v2" && executionEnvelope.authoritySource !== "v2_execution_envelope")
        invariantFailures.push("AUTHORITY_SOURCE_MISMATCH");
    if (v2Mode === "engine_v2" && !runtimeDecisionMatchesV2)
        invariantFailures.push("RUNTIME_DECISION_MISMATCH_V2");
    if (v2Mode === "engine_v2" && !runtimeSideMatchesV2)
        invariantFailures.push("RUNTIME_SIDE_MISMATCH_V2");
    if (v2Mode === "engine_v2" && !runtimeSizeMatchesV2)
        invariantFailures.push("RUNTIME_SIZE_MISMATCH_V2");
    if (hardBlockEnterConflict)
        invariantFailures.push("HARDBLOCK_ENTER_CONFLICT");
