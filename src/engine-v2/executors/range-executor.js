...
exports.executeRangeRegime = executeRangeRegime;
 ...
 */
function executeRangeRegime(input, judgment) {
    ...
    const isDistorted = judgment.metadata?.isDistorted === true;
    const isDrifting = judgment.metadata?.isDrifting === true;
    const distortionFactor = judgment.metadata?.distortionFactor ?? 0;
    const bhSlope = judgment.metadata?.bhSlope ?? 0;
    const blSlope = judgment.metadata?.blSlope ?? 0;
    const execMeta = (judgment.metadata ?? {});
    const readBool = (v) => v === true || v === "true";
    let signal = "NONE";
    let side = "none";
    let reason = "Initial state";
    let recheckSuggested = false;
    let reversalConfirmed = readBool(execMeta.reversal_confirmed) || judgment.reversalConfirmed === true;
    ...
    const boxZone = (0, types_1.classifyRangeZone)(currentBoxPos);
    ...
    if (isDownStructure || isUpStructure) {
        ...
        const primarySide = isDownStructure ? "short" : "long";
        const blockedSide = isDownStructure ? "long" : "short";
    ...
    if (isVolDown || isVolUp) {
        ...
        const primarySide = isVolDown ? "short" : "long";
        const blockedSide = isVolDown ? "long" : "short";
    ...
    if (judgment.shockPhase === "DOWN_SHOCK" || judgment.shockPhase === "UP_SHOCK") {
        ...
        const primarySide = judgment.shockPhase === "DOWN_SHOCK" ? "short" : "long";
        const blockedSide = judgment.shockPhase === "DOWN_SHOCK" ? "long" : "short";
    ...
    if (side === "long") {
        ...
        if (tp1 <= entryPx)
            tp1 = entryPx + minProfitDistance;
        tp2 = Math.max(boxHigh, tp1 + minProfitDistance);
        if (tp2 <= tp1)
            tp2 = tp1 + minProfitDistance;
    }
    else if (side === "short") {
    ...
    const finalSide = signal === "NONE" ? "none" : side;
