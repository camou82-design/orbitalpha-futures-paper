...
exports.executeTransitionRegime = executeTransitionRegime;
 ...
 */
function executeTransitionRegime(input, judgment) {
    ...
    const isMidZone = (0, types_1.classifyRangeZone)(boxPos) === "mid";
    ...
    if (!hasPosition && isTransitionRegime) {
        ...
        if (candles.length >= 20) {
            ...
            if (topOverextended && shortAllow && directionalShockState !== "UP") {
                const shortConfirms = [];
                if (lastPrice < swingHigh)
                    shortConfirms.push("failed_high");
                if (lastCandle.high - Math.max(lastCandle.open, lastCandle.close) > (Math.max(lastCandle.open, lastCandle.close) - Math.min(lastCandle.open, lastCandle.close)))
                    shortConfirms.push("upper_wick");
                if (lastCandle.close < prevCandle.low)
                    shortConfirms.push("break_prev_low");
                if (sn.ema20 && lastPrice < sn.ema20)
                    shortConfirms.push("below_ema20");
                if (rsi < calculateRSI(closes.slice(0, -1)))
                    shortConfirms.push("rsi_turn");
                if (bb && prevCandle.high > bb.upper && lastPrice < bb.upper)
                    shortConfirms.push("bb_reentry");
                ...
                if (shortConfirms.length >= MIN_FAILURE_CONFIRMATIONS && !lateChaseBlocked) {
                    ...
                    if (lastPrice < swingHigh * 1.0005) {
                        ...
                        return {
                            ...
                            metadata: { failure_layer: "top_failure", confirms: shortConfirms.length, rsi }
                        };
                    }
                }
                    ...
                    return {
                        ...
                        stopPrice,
                        invalidationPx: stopPrice,
                        metadata: { failure_layer: "retest_failure", type: "short" }
                    };
                }
            ...
            if (bottomOverextended && longAllow && directionalShockState !== "DOWN") {
                const longConfirms = [];
                if (lastPrice > swingLow)
                    longConfirms.push("failed_low");
                if (Math.min(lastCandle.open, lastCandle.close) - lastCandle.low > (Math.max(lastCandle.open, lastCandle.close) - Math.min(lastCandle.open, lastCandle.close)))
                    longConfirms.push("lower_wick");
                if (lastCandle.close > prevCandle.high)
                    longConfirms.push("break_prev_high");
                if (sn.ema20 && lastPrice > sn.ema20)
                    longConfirms.push("above_ema20");
                if (rsi > calculateRSI(closes.slice(0, -1)))
                    longConfirms.push("rsi_turn");
                if (bb && prevCandle.low < bb.lower && lastPrice > bb.lower)
                    longConfirms.push("bb_reentry");
                ...
                if (longConfirms.length >= MIN_FAILURE_CONFIRMATIONS && !lateChaseBlocked) {
                    ...
                    if (lastPrice > swingLow * 0.9995) {
                        ...
                        return {
                            ...
                            stopPrice,
                            invalidationPx: stopPrice,
                            metadata: { failure_layer: "bottom_failure", confirms: longConfirms.length, rsi }
                        };
                    }
                }
                else if (lateChaseBlocked && retestConfirmed) {
                    ...
                    return {
                        ...
                        stopPrice,
                        invalidationPx: stopPrice,
                        metadata: { failure_layer: "retest_failure", type: "long" }
                    };
                }
                else if (lateChaseBlocked) {
                    console.info(JSON.stringify({
    ...
    const isWhipsawExempt = (() => {
        if (subtype === "WHIPSAW_SOFT_WATCH") {
            const isShockDownTarget = directionalShockState === "DOWN" &&
                shortAllow &&
                emaGap < 0 &&
                (0, types_1.classifyRangeZone)(boxPos) === "lower";
            ...
            const isMidShockDownTarget = directionalShockState === "DOWN" &&
                shortAllow &&
                (0, types_1.classifyRangeZone)(boxPos) === "mid";
            if (isMidShockDownTarget) {
                ...
                const boxMidLost = judgment?.diagnostics?.fastTrendShift?.box_mid_lost === true ||
                ...
                const lowerBreakdownHold = judgment?.diagnostics?.fastTrendShift?.box_lower_breakdown_hold === true ||
                ...
                const earlyStopPrice = judgment?.diagnostics?.early_probe?.allowed ? judgment.diagnostics.early_probe.stopPrice : null;
    ...
    let transitionSetupType = "NONE";
    let transitionAction = "REJECT";
    let transitionPrimarySide = "none";
    let transitionCounterSide = "none";
    let transitionWatchOnly = true;
    let transitionConfirmRequired = true;
    let transitionRejectReason = "TRANSITION_CONFLICT_NO_TRADE";
    let transitionConfirmBasis = "insufficient";
    let transitionPreflightSafetyPassed = false;
    let transitionPreflightBlockReason = null;
    ...
    if (subtype === "SHOCK_REACTION_DOWN" || directionalShockState === "DOWN") {
        ...
        if (!shortAllow) {
            ...
            const isMidWhipsawRetest = subtype === "WHIPSAW_SOFT_WATCH" &&
            ...
            const stopPriceVal = fastStopPrice != null && Number.isFinite(fastStopPrice) ? fastStopPrice :
                (earlyStopPrice != null && Number.isFinite(earlyStopPrice) ? earlyStopPrice : null);
            const hasCondition = breakdownRetestFailure || boxMidLost || lowerBreakdownHold;
            const downMomentumConfirmed = shortAllow &&
                emaGap < 0 &&
                qualityScore >= 70 &&
            ...
            const emaGapOnlySafety = qualityScore >= 72 &&
                reviewingTicks >= 2 &&
        ...
        if (!longAllow) {
            ...
            const upMomentumConfirmed = longAllow &&
                emaGap > 0 &&
                qualityScore >= 70 &&
                trendWeaknessScore < 0.65 &&
