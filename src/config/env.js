...
exports.getPaperLoopIntervalMs = getPaperLoopIntervalMs;
exports.getEngineConfig = getEngineConfig;
const path = __importStar(require("node:path"));
const dotenv = __importStar(require("dotenv"));
const entry_gate_config_1 = require("../strategy/entry-gate-config");
function parseBool(v, fallback) {
    if (v === undefined)
        return fallback;
    const x = v.trim().toLowerCase();
    if (["1", "true", "yes", "y", "on"].includes(x))
        return true;
    if (["0", "false", "no", "n", "off"].includes(x))
        return false;
    return fallback;
}
...
function parseLogLevel(v) {
    ...
    if (x === "debug" || x === "info" || x === "warn" || x === "error")
        return x;
    return "info";
}
function parseSymbols(v) {
    const raw = (v ?? "BTCUSDT,ETHUSDT").split(",").map((s) => s.trim()).filter(Boolean);
    return raw.length > 0 ? raw : ["BTCUSDT", "ETHUSDT"];
}
function parseHorizonPriority(v, fallback) {
    if (!v || v.trim() === "")
        return fallback;
    const parts = v
        .split(",")
...
function getPaperLoopIntervalMs(env = process.env) {
    const raw = env.ORBITALPHA_PAPER_LOOP_INTERVAL_MS;
    const parsedDefault = raw === undefined || raw === "" ? PAPER_LOOP_INTERVAL_DEFAULT_MS : Number(raw);
    const parsed = !Number.isFinite(parsedDefault) ? PAPER_LOOP_INTERVAL_DEFAULT_MS : Math.floor(parsedDefault);
    const clamped = Math.min(PAPER_LOOP_INTERVAL_MAX_MS, Math.max(PAPER_LOOP_INTERVAL_MIN_MS, parsed));
    let delayReason;
...
function getEngineConfig(env = process.env) {
    ...
    const defaultMoveMult = paperEntryRelaxed ? 1.05 : entry_gate_config_1.ENTRY_GATE_CONFIG.minMoveVsCostMultiplier;
    ...
    const paperReentryCooldownMsParsed = paperReentryCooldownMsRaw === undefined || paperReentryCooldownMsRaw.trim() === ""
        ? 900_000
        : parseInt(paperReentryCooldownMsRaw, 10);
    const paperReentryCooldownMs = !Number.isFinite(paperReentryCooldownMsParsed) || paperReentryCooldownMsParsed < 0
        ? 900_000
    ...
    const rangeRebalanceMinHoldMsParsed = rangeRebalanceMinHoldMsRaw === undefined || String(rangeRebalanceMinHoldMsRaw).trim() === ""
        ? 600_000
        : parseInt(String(rangeRebalanceMinHoldMsRaw), 10);
    const rangeRebalanceMinHoldMs = !Number.isFinite(rangeRebalanceMinHoldMsParsed) || rangeRebalanceMinHoldMsParsed < 0
        ? 600_000
        : Math.min(86_400_000, rangeRebalanceMinHoldMsParsed);
    const rangeRebalanceBoxBreakConfirmTicks = parseIntClamped(env.ORBITALPHA_RANGE_REBALANCE_BOX_BREAK_CONFIRM_TICKS, 3, 1, 8);
    ...
    const rangeRebalanceTrailMaxArmedNoLockMsParsed = rangeRebalanceTrailMaxArmedNoLockMsRaw === undefined || String(rangeRebalanceTrailMaxArmedNoLockMsRaw).trim() === ""
        ? 0
        : parseInt(String(rangeRebalanceTrailMaxArmedNoLockMsRaw), 10);
    const rangeRebalanceTrailMaxArmedNoLockMs = !Number.isFinite(rangeRebalanceTrailMaxArmedNoLockMsParsed) || rangeRebalanceTrailMaxArmedNoLockMsParsed < 0
        ? 0
    ...
    const paperModeHardSuspendMsParsed = paperModeHardSuspendMsRaw === undefined || String(paperModeHardSuspendMsRaw).trim() === ""
        ? 1_200_000
        : parseInt(String(paperModeHardSuspendMsRaw), 10);
    const paperModeHardSuspendMs = !Number.isFinite(paperModeHardSuspendMsParsed) || paperModeHardSuspendMsParsed < 0
        ? 1_200_000
    ...
    const paperModeSuspendMsParsed = paperModeSuspendMsRaw === undefined || paperModeSuspendMsRaw.trim() === "" ? 3_600_000 : parseInt(paperModeSuspendMsRaw, 10);
    const paperModeSuspendMs = !Number.isFinite(paperModeSuspendMsParsed) || paperModeSuspendMsParsed < 0 ? 3_600_000 : Math.min(86_400_000, paperModeSuspendMsParsed);
    ...
    let paperAccountEquityUsd = null;
    if (paperAccountEquityUsdRaw !== undefined && String(paperAccountEquityUsdRaw).trim() !== "") {
        ...
        if (Number.isFinite(eq) && eq > 0)
            paperAccountEquityUsd = Math.min(1_000_000_000, Math.max(0.01, eq));
    }
    let paperEntryNotionalTargetFrac = parseNumber(env.ORBITALPHA_PAPER_ENTRY_NOTIONAL_TARGET_FRAC, 1);
    if (!Number.isFinite(paperEntryNotionalTargetFrac) || paperEntryNotionalTargetFrac <= 0)
        paperEntryNotionalTargetFrac = 1;
    ...
    const aiBlockEvaluationHorizonPriorityMins = parseHorizonPriority(env.ORBITALPHA_AI_BLOCK_EVAL_HORIZON_PRIORITY, [30, 15, 5]);
    const paperEngineModeRaw = (env.ORBITALPHA_PAPER_ENGINE_MODE ?? "PAPER_TEST").trim().toUpperCase();
    const paperEngineMode = paperEngineModeRaw === "SAFE" || paperEngineModeRaw === "RESEARCH" ? paperEngineModeRaw : "PAPER_TEST";
    ...
    if (!Number.isFinite(paperFeeDragTailSizeMult) || paperFeeDragTailSizeMult <= 0)
        paperFeeDragTailSizeMult = 0.72;
    ...
    const okxAuthMode = okxLiveEnabled && okxExchangeAuthOptIn
        ? "live"
    ...
    const okxAuthReady = okxExchangeAuthOptIn &&
        ((okxAuthMode === "live" &&
            okxApiKey.length > 0 &&
