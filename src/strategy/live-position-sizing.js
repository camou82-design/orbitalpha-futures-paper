...
exports.MIN_POSITION_SIZE_USD = void 0;
exports.computePaperSizingAnchorUsd = computePaperSizingAnchorUsd;
exports.calculateAdaptivePositionSize = calculateAdaptivePositionSize;
exports.MIN_POSITION_SIZE_USD = 15;
/**
 * Paper sizing anchor: `paperAccountEquityUsd * paperEntryNotionalTargetFrac` when equity is set; else `paperBaseSizeUsd`.
 */
function computePaperSizingAnchorUsd(config) {
    const frac = Number.isFinite(config.paperEntryNotionalTargetFrac) && config.paperEntryNotionalTargetFrac > 0
        ? Math.min(1, config.paperEntryNotionalTargetFrac)
        : 1;
    if (config.paperAccountEquityUsd != null &&
    ...
    const base = Number.isFinite(config.paperBaseSizeUsd) && config.paperBaseSizeUsd > 0 ? config.paperBaseSizeUsd : 100;
 ...
 */
function calculateAdaptivePositionSize(input) {
    let confMult = 1;
    if (input.confidenceTier === "top")
        confMult = 1;
    else if (input.confidenceTier === "high")
        confMult = 0.75;
    else if (input.confidenceTier === "mid")
        confMult = 0.5;
    else
        confMult = 0.25;
    let volMult = 1.0;
    if (input.volumeRatioProxy !== undefined) {
        ...
        if (vol >= 8.0)
            volMult = 0.15; // User rule: 0.1x ~ 0.2x
        else if (vol >= 4.5)
            volMult = 0.3; // User rule: 0.25x ~ 0.4x
    ...
    return {
        ...
        detail: {
            ...
            min_size_usd: exports.MIN_POSITION_SIZE_USD,
            volume_mult: volMult
        }
    };
}
