...
/** Paper futures adaptive engine — 3-way regime from BTC 5m structure + range. */
export type FuturesMarketMode = "trend" | "sideways" | "risk_off";
 ...
 */
export function detectFuturesMarketMode(input: Readonly<{ btcCandles5m: readonly Candle[] }>): Readonly<{
  ...
  const bias: "up" | "down" | "flat" =
    e12 > e26 * 1.0012 ? "up" : e12 < e26 * 0.9988 ? "down" : "flat";
...
export function btcBiasFromModeDetail(detail: Record<string, unknown>): "up" | "down" | "flat" {
  const b = detail.bias;
  if (b === "up" || b === "down" || b === "flat") return b;
  return "flat";
}
