...
exports.evaluateRegimeEntry = evaluateRegimeEntry;
const types_1 = require("../models/types");
function isDir(signal) {
 ...
 */
function evaluateRegimeEntry(ctx) {
    const dir = isDir(ctx.signal);
    if (dir === null)
        return { ok: false, reason: "no_signal", detail: {} };
    ...
    if (ctx.regime === "RANGE") {
        ...
        const zone = (0, types_1.classifyRangeZone)(ctx.boxPos);
        // Middle forbidden.
        if (zone === "mid") {
    ...
    const breakoutUp = hasBox ? ctx.lastPrice >= ctx.boxHigh * 1.0006 : false;
    const breakoutDown = hasBox ? ctx.lastPrice <= ctx.boxLow * 0.9994 : false;
    ...
    if (dir === "long") {
        if (!emaAlignedLong)
            return { ok: false, reason: "trend_not_aligned", detail: { sub: "ema_long_not_aligned" } };
    ...
    if (dir === "short") {
        if (!emaAlignedShort)
            return { ok: false, reason: "trend_not_aligned", detail: { sub: "ema_short_not_aligned" } };
