...
exports.trendExecutorEvaluateEntry = trendExecutorEvaluateEntry;
exports.trendExecutorEvaluateExit = trendExecutorEvaluateExit;
function intentDirection(signal) {
...
function trendExecutorEvaluateEntry(input) {
    ...
    const breakoutUp = hasBox ? input.lastPrice >= input.boxHigh * 1.0006 : null;
    const breakoutDown = hasBox ? input.lastPrice <= input.boxLow * 0.9994 : null;
    const breakout_state = breakoutUp === null || breakoutDown === null ? "none" : breakoutUp ? "breakout_up" : breakoutDown ? "breakout_down" : "none";
    ...
    const pullback_state = pullbackLong === null || pullbackShort === null
        ? "none"
        : dir === "long"
            ? pullbackLong
                ? "pullback_ok"
    ...
    const breakoutOk = dir === "long" ? breakout_state === "breakout_up" : dir === "short" ? breakout_state === "breakout_down" : false;
    const pullbackOk = pullback_state === "pullback_ok";
    const currentStage = input.currentStage ?? 0;
    //      ?                      ?      
    let guidance = "";
    if (dir === "long") {
        if (pullback_state === "pullback_ok")
...
function trendExecutorEvaluateExit(input) {
    ...
    return {
        ...
        action: "hold",
        reason: null,
        guidance: input.partialExitStage === 0 ? "hold_primary_tp_watch" : "scale_out_progress_watch",
        next_action: input.partialExitStage === 0 ? "WAIT_FOR_PARTIAL_TP_TRIGGER" : "MANAGE_TRAILING_OR_TP",
