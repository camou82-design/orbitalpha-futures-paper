...
exports.TREND_VOLUME_RELAX_WEAK_MAX_REQUIRED_MOVE_PCT = exports.TREND_VOLUME_RELAX_WEAK_QUALITY_FOR_STRONG_MIN = exports.TREND_VOLUME_RELAX_WEAK_MIN_QUALITY_SCORE = exports.TREND_POLICY_MIN_VOLUME_RATIO_PROXY_HIGHWAY_WEAK_RELAX = exports.TREND_POLICY_MIN_VOLUME_RATIO_PROXY_HIGHWAY_STRONG_RELAX = exports.TREND_POLICY_MIN_VOLUME_RATIO_PROXY = exports.STRONG_SCORE = exports.ENTRY_MIN_SCORE = void 0;
exports.decidePositionDirection = decidePositionDirection;
exports.evaluateEntryPolicy = evaluateEntryPolicy;
exports.calculatePositionSize = calculatePositionSize;
const live_market_mode_1 = require("./live-market-mode");
/** 최소 품질 점수 — 진입 파이프라인·방향 결정 공통 하한 (수수료 대비 애매한 진입 감소). */
exports.ENTRY_MIN_SCORE = 35;
/** 횡보 모드에서 weak 후보는 더 높은 점수 요구. */
const SIDEWAYS_WEAK_MIN_SCORE = 72;
/** TREND BTC 역풍 완화·하이웨이 strong 연동 등과 동일 하한(72). */
exports.STRONG_SCORE = 72;
/** TREND 모드 `evaluateEntryPolicy`: 최소 거래량 비율(스냅샷 `volumeRatioProxy`). trend-executor minVol(1.02)과 정렬. */
exports.TREND_POLICY_MIN_VOLUME_RATIO_PROXY = 1.02;
/**
 * Stage1 TREND: 하이웨이 코어 VALID + strong 후보 완화 시 하한(여전히 neutral 이상).
 * `paper-symbol-decision`에서만 override로 전달.
 */
exports.TREND_POLICY_MIN_VOLUME_RATIO_PROXY_HIGHWAY_STRONG_RELAX = 1.0;
/**
 ...
 */
exports.TREND_POLICY_MIN_VOLUME_RATIO_PROXY_HIGHWAY_WEAK_RELAX = 1.01;
/** weak 완화 최소 품질(Highway·엣지 보완 가정). */
exports.TREND_VOLUME_RELAX_WEAK_MIN_QUALITY_SCORE = 65;
/** weak이어도 이 점수 이상이면 엣지 양호 시 볼륨 하한을 strong과 동일(1.0)으로 허용. */
exports.TREND_VOLUME_RELAX_WEAK_QUALITY_FOR_STRONG_MIN = 70;
/** weak 완화: `required_move_pct`(유효비용×100) 상한 — 비용 부담 큰 틱 제외. */
exports.TREND_VOLUME_RELAX_WEAK_MAX_REQUIRED_MOVE_PCT = 0.4;
/** 횡보에서 EMA 분리가 너무 작으면 애매한 구간으로 진입 차단 (|ema20-ema60|/ema60). */
const SIDEWAYS_MIN_EMA_REL_SEP = 0.0035;
/**
 * 시장 모드 + BTC 편향 + 후보 신호로 롱/숏/관망 결정.
 */
function decidePositionDirection(input) {
    const btcB = (0, live_market_mode_1.btcBiasFromModeDetail)(input.modeDetail);
    ...
    if (input.mode === "risk_off") {
        if (wantLong) {
            if (input.signalStrengthScore >= exports.STRONG_SCORE && btcB === "up" && input.candidateStrength === "strong") {
                return "long";
            }
            return "none";
        }
    ...
    if (input.mode === "sideways") {
        ...
        if (wantLong) {
            ...
            return input.signalStrengthScore >= exports.ENTRY_MIN_SCORE ? "long" : "none";
        }
        if (wantShort) {
            if (btcB === "up")
                return "none";
            ...
            return input.signalStrengthScore >= exports.ENTRY_MIN_SCORE ? "short" : "none";
        }
    }
    /* trend — 약한 점수면 방향 자체를 주지 않음 */
    if (wantLong || wantShort) {
        if (input.signalStrengthScore < exports.ENTRY_MIN_SCORE)
            return "none";
    }
    if (wantLong) {
        if (btcB === "down" && input.signalStrengthScore < exports.STRONG_SCORE)
            return "none";
        return "long";
    }
    if (wantShort) {
        if (btcB === "up" && input.signalStrengthScore < exports.STRONG_SCORE)
            return "none";
        return "short";
    }
    return "none";
...
function evaluateEntryPolicy(input) {
    ...
    if (input.mode === "trend") {
        ...
        const trendVolMin = typeof input.trendVolumeRatioMinOverride === "number" &&
            Number.isFinite(input.trendVolumeRatioMinOverride) &&
            input.trendVolumeRatioMinOverride > 0
            ? input.trendVolumeRatioMinOverride
            : exports.TREND_POLICY_MIN_VOLUME_RATIO_PROXY;
        const trendVolFloor = input.trendEmaSoftGate === true ? Math.min(trendVolMin, 1.0) : trendVolMin;
            ...
            return {
                ...
                detail: {
                    ...
                    entry_policy_proof: {
                        ...
                        default_min_volume_ratio_proxy: exports.TREND_POLICY_MIN_VOLUME_RATIO_PROXY,
                        volume_ratio_proxy_actual: input.volumeRatioProxy,
                        shortfall_ratio: trendVolFloor - input.volumeRatioProxy,
                        trend_volume_min_override_applied: trendVolMin !== exports.TREND_POLICY_MIN_VOLUME_RATIO_PROXY,
                        ...
                        not_in_this_stage: {
                            ...
                            volume_support_score: "Highway `evaluateAiHighwayQuality`; adaptive entry uses `snap.volumeRatioProxy` (1m candle-derived proxy, different formula)"
                        }
                    }
                }
            };
    ...
    if (input.direction === "long") {
        if (input.signalStrengthScore >= exports.STRONG_SCORE) {
    ...
    if (!emaAlignedShort && input.signalStrengthScore < exports.STRONG_SCORE) {
        return {
            ok: false,
            blockMessage: "blocked_no_structure",
