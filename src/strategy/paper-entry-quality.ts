...
export function computePaperEntryQualityScore(input: Readonly<{
  ...
  if (!input.trendOk || input.ema20 === null || !Number.isFinite(input.ema20) || input.ema20 <= 0) return 0;

  let score = 35;
  if (input.ema60 !== null && Number.isFinite(input.ema60) && input.ema20 > input.ema60) score += 25;
