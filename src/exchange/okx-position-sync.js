...
exports.okxSwapRowToLedgerKey = okxSwapRowToLedgerKey;
exports.buildLedgerOkxPositionSyncSnapshot = buildLedgerOkxPositionSyncSnapshot;
const NOTIONAL_TOLERANCE_USD = 1.0;
const PRICE_TOLERANCE_RATIO = 0.0005; // 0.05%
function okxSwapRowToLedgerKey(row) {
    ...
    if (!Number.isFinite(posNum) || Math.abs(posNum) <= 0)
        return null;
...
function buildLedgerOkxPositionSyncSnapshot(paperOpens, okxPayload, instrumentMap) {
    const paperMap = new Map();
    const paper_positions_preview = [];
    for (const p of paperOpens) {
        ...
        paperMap.set(key, p);
        const paperBaseQty = typeof p.baseQty === "number" && Number.isFinite(p.baseQty) && p.baseQty > 0
            ? p.baseQty
            : p.sizeUsd / Math.max(1e-12, p.entryPrice || 1);
        const paperNotional = typeof p.notionalUsd === "number" && Number.isFinite(p.notionalUsd) ? p.notionalUsd : p.sizeUsd;
    ...
    for (const row of okxPayload) {
        ...
        const safeStr = (v) => typeof v === "string" && v.trim() !== "" ? v.trim() : undefined;
    ...
    let sync_status = "ALIGNED";
    let detail = null;
    const mismatched_keys = [];
    const ignored_external_manual_keys = [];
    if (okx_nonzero_position_count === 0 && paper_open_position_count === 0) {
        ...
        for (const k of okxMap.keys())
            mismatched_keys.push(k);
    }
    else if (okx_nonzero_position_count === 0 && paper_open_position_count > 0) {
        ...
        if (onlyOkx.length > 0 || onlyPaper.length > 0) {
            ...
            for (const k of onlyOkx)
                mismatched_keys.push(k);
            for (const k of onlyPaper)
                mismatched_keys.push(k);
        }
        ...
        for (const [key, okxPos] of okxMap.entries()) {
            ...
            const paperNotional = typeof paperPosData.notionalUsd === "number" && Number.isFinite(paperPosData.notionalUsd)
            ...
            const hasExplicitPaperBase = typeof paperBaseLedger === "number" && Number.isFinite(paperBaseLedger) && paperBaseLedger > 0;
            if (okxBaseQty !== undefined &&
            ...
            if (mismatchAtThisKey) {
                if (!mismatched_keys.includes(key))
        ...
        for (const k of mismatched_keys) {
            ...
            if (p?.lifecycleState === "EXTERNAL_MANUAL_POSITION") {
                if (!ignored_external_manual_keys.includes(k))
                    ignored_external_manual_keys.push(k);
            }
        }
